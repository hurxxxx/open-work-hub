...
Open Work Hub AI capability registry는 단순 `OpenAI function` 목록에서 **MCP-first capability platform** 으로 전환됐다. 현재 구현은 다음 구조를 전제로 한다.
...
- executable write tool은 `OPEN_WORK_HUB_AI_WRITE_TOOLS_ENABLED=true` 일 때에만 discovery 대상이 된다.
...
- write capability 추가 시 `OPEN_WORK_HUB_AI_WRITE_TOOLS_ENABLED` off/on 양쪽 discovery test를 함께 유지한다.
