...
이 문서는 신규 AI 포털의 구현 기준 구조를 고정한다. 레거시 프로토타입의 기능 흐름만 참고하고, 새 구현은 `문서 RAG`, `PLM 조회`, `문서 작성 지원`, `관리자/RBAC`, `PMS`를 중심으로 설계한다.
...
- 데스크톱 기본형은 `좌측 사이드바 메뉴 + 상단 유틸리티 바 + 중앙 작업면 + 필요 시 우측 컨텍스트 pane` 이다.
- 홈은 마케팅 hero가 아니라 앱 셸 안의 `전역 검색`, `최근 작업`, `즐겨찾기 워크플로`, `할당 작업` 중심 시작점으로 구성한다.
- 검색은 중요한 진입점이지만 정보구조를 대체하지 않는다.
- 기본 작업면은 `중앙 결과`, `우측 근거/상세/액션 pane` 또는 `좌측 조건`, `중앙 결과`, `우측 근거/액션` 3패널 구조를 우선한다.
- 카드형 섹션은 최소화하고 표, 리스트, 슬라이드오버, 드로어, 인라인 상태 전환을 우선한다.
- 시각 스타일은 과장된 AI 데모 사이트보다 안정적인 enterprise tool 언어를 우선한다.

### 디자인 기준서

- 상세 디자인 방향: [docs/architecture/enterprise-portal-design-direction.md](/Users/edward/projects/doowon/docs/architecture/enterprise-portal-design-direction.md)
