...
`/projects/ai-do/ai-do-inference-gateway`는 AI-DO 메인 API/worker와 분리된 범용 추론
게이트웨이다. 소스는 별도 Git 레포
`http://128.1.253.101:8929/dwdcc/ai-do-inference-gateway.git`에서 관리한다.
...
ASR은 AI-DO 메인 API의 `asr_cohere_model` 기본값인 `transcribe-v1`을 공개
...
공개 모델명은 `AI_DO_INFERENCE_GATEWAY_ASR_PUBLIC_MODEL`로 바꿀 수 있다.
...
`.env`의 `AI_DO_INFERENCE_GATEWAY_HF_TOKEN_FILE`로 다른 경로를 지정한다. 토큰 원문은
...
| Endpoint | Role |
| --- | --- |
| `http://128.1.253.103:18080` | AI-DO application용 internal LB endpoint |
...
```text
102: /etc/systemd/system/ai-do-local-ai-backend.service, vllm-qwen36-docker.service
103: /etc/systemd/system/ai-do-local-ai-backend.service, vllm-qwen36-docker.service
103 HAProxy: local_ai_api :18080 -> 127.0.0.1:18081, 192.168.100.102:18080
Required tasks: embedding, reranker, docling, asr
```

운영 unit 이름은 현재 `ai-do-local-ai-backend.service`이고 working directory는
`/srv/local-ai-backend`다. repository의 신규 설치 helper가 만드는
`ai-do-inference-gateway.service`와 이름이 다르므로 서버 상태 확인과 장애 대응에는 실제
...
| env | 기본값 | disabled 시 |
| --- | --- | --- |
| `AI_DO_INFERENCE_GATEWAY_EMBEDDING_ENABLED` | `true` | `/v1/embeddings` 503 |
| `AI_DO_INFERENCE_GATEWAY_RERANKER_ENABLED` | `true` | `/v1/rerank`, `/v2/rerank` 503 |
| `AI_DO_INFERENCE_GATEWAY_DOCLING_ENABLED` | `true` | `/v1/convert/source` 503 |
| `AI_DO_INFERENCE_GATEWAY_ASR_ENABLED` | `true` | `/v1/audio/transcriptions` 503 |
...
- AI-DO portal server `128.1.253.101`
...
접근 복구 전 103 규칙 snapshot은 `/etc/ufw.pre-open-local-dev-*`에 있다. AI-DO 운영
...
```bash
cd /projects/ai-do/ai-do-inference-gateway
...
AI-DO repo 루트에서는 wrapper script로 같은 작업을 호출할 수 있다.
...
```bash
ssh dgx-spark-102 'systemctl is-enabled ai-do-local-ai-backend.service; systemctl is-active ai-do-local-ai-backend.service'
ssh dgx-spark-103 'systemctl is-enabled ai-do-local-ai-backend.service; systemctl is-active ai-do-local-ai-backend.service'
...
직접 주지 않으면 `AI_DO_INFERENCE_GATEWAY_EMBEDDING_QUERY_PROMPT_NAME` 기본값 `query`를 사용한다.
...
`ai-do-local-ai-backend-watchdog.timer`를 함께 설치하고 20초마다 localhost `/health`를
...
cold start를 막는다. 배포 파일은 `ops/systemd/system/ai-do-local-ai-backend-*`가 정본이다.

계획 점검 중에는 먼저 `/etc/ai-do/inference-gateway-watchdog.disabled`를 만들고 watchdog
...
ssh dgx-spark-102 'systemctl is-enabled ai-do-local-ai-backend-watchdog.timer; systemctl list-timers ai-do-local-ai-backend-watchdog.timer --no-pager'
ssh dgx-spark-103 'systemctl is-enabled ai-do-local-ai-backend-watchdog.timer; systemctl list-timers ai-do-local-ai-backend-watchdog.timer --no-pager'
ssh dgx-spark-102 'systemctl show ai-do-local-ai-backend.service -p Restart -p RestartUSec -p TimeoutStopUSec'
ssh dgx-spark-103 'systemctl show ai-do-local-ai-backend.service -p Restart -p RestartUSec -p TimeoutStopUSec'
```
...
- 이 백엔드는 AI-DO 메인 API/worker와 독립된 범용 inference service다.
- AI-DO 메인 API/worker는 RAG embedding/rerank/OCR과 ASR provider adapter를 통해
