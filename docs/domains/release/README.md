...
Do not create a full product Docker image merely to release the independent Codex Console: use its [release builder](../../apps/codex-console/README.md#배포-완료-확인). Temporary verification images must be removed by their creating operation after verification, retaining failed-test evidence rather than full image copies. Product release validation remains required when its own image/runtime changes.
