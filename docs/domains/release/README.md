...
- Route the external HTTPS proxy to `OPEN_WORK_HUB_APP_BIND_HOST:OPEN_WORK_HUB_APP_PORT`. `OPEN_WORK_HUB_APP_FORWARDED_ALLOW_IPS` lists only the exact proxy IPs.
...
- `pnpm app:prod:deploy` builds and verifies the revision image, applies migrations, replaces the app runtime, then requires local and public health, readiness, revision, bootstrap, and login-shell checks.
