...
In the pinned official image, the `hermes` user's container home (`~`) is `/opt/data`. Open Work Hub mounts a private named volume there, so `/opt/data/profiles/terminal` is the container's user-home data and does not read or write the host machine's `/opt` directory. Generated project files remain on the separate private `/workspace` volume.
