...
Open Work Hub의 기본 AI 동작은 내부 원본을 읽고, 검색하고, 요약하는 보조 기능이다. 원본 시스템을 실제로 변경하는 write 동작은 기능 플래그, 사용자 승인, 원본 ACL, 감사 로그가 모두 충족될 때만 실행한다.
...
- `OPEN_WORK_HUB_AI_WRITE_TOOLS_ENABLED`의 기본값은 `false`다. 비활성 상태에서는 write tool을 registry와 AI surface에 노출하지 않는다.
