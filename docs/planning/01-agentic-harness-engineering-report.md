...
> 현재 v1 실행 계획과 기본 모델 선택은 [`02-evidence-first-agent-runtime.md`](../../plans/02-evidence-first-agent-runtime.md)가 우선한다. v1 canonical 모델은 `Qwen/Qwen3.6-35B-A3B`, 로컬 MLX checkpoint는 `mlx-community/Qwen3.6-35B-A3B-4bit`로 고정한다.
...
- 실제 Doowon 적용 설계는 [`02-evidence-first-agent-runtime.md`](../../plans/02-evidence-first-agent-runtime.md)에서 별도 실행 계획으로 관리한다.
