...
| LIVE-10 | In Community, B creates a company post and keeps its detail open. A opens it and comments. Before reloading or changing focus in B's browser, verify the open detail. Then background and restore B's post tab while recording post-detail requests, and mark the notification read.                                                                 | Community has no workspace selector. B's detail updates without reload and one canonical notification appears. Bringing the tab forward causes one detail refresh even if both `visibilitychange` and `focus` fire; read state persists after reload.                                                                                                                  |
...
| CAP-62 | Run WCAG A/AA checks on launcher, chooser, company/workspace apps, bulk-delete triggers, confirmation dialogs, and denial. | No critical/serious violations; labels, headings, focus, destructive-action contrast, and scope badges meet their contracts. |
