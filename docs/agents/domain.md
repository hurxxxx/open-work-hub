...
| 변경 위험과 MR 검증 깊이 | [구현 검증 하네스](vibe-coding-harness.md) |
| 공용 UI 재사용 | [UI 컴포넌트](ui-components.md) |
| 유사 기능의 공유 모델 여부 | [조합형 추상화](composable-abstractions.md) |
| Codex MR review 동작 | [로컬 Codex MR 리뷰](local-codex-review.md) |
| GitLab Issue 사용 | [Issue Tracker](issue-tracker.md) |
| GitLab Issue 라벨 판단 | [Triage Labels](triage-labels.md) |
...
GitLab `origin`의 Issue/MR이 사이트 변경 추적의 기준이다. GitHub `upstream`은 원본 코드 수신용이며
사이트 작업 지시나 병합 상태의 정본으로 사용하지 않는다.
