...
| Native `.rules`          | Allow GitHub PR operations within explicit user scope; forbid GitHub issue mutations, direct upstream pushes, and direct production Compose mutations                  |
...
[Native rules load at Codex startup](https://learn.chatgpt.com/docs/agent-configuration/rules#create-a-rules-file). Start a new Codex session after changing `.codex/rules/project.rules`; `codex execpolicy check` validates the candidate file, not the running session's loaded policy. An allow rule does not grant task authorization or override a more restrictive rule from another active layer. If a running session still denies an authorized PR command, report the required restart instead of changing command forms or using another tool to evade the denial.
