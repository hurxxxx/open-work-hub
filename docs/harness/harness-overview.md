...
- 입력, 시스템 지시, 허용 도구, 금지 응답, 출력 스키마, citation 규칙을 묶은 계약

### `WorkflowProfile`

- 어떤 모델/검색/도구 조합을 사용할지 정의하는 실행 프로필
...
- 대표 질의, 기대 결과, 평가 기준, 실행 결과를 표현하는 테스트 단위
...
`ScenarioSpec 확정 -> PromptContract/WorkflowProfile 갱신 -> EvalCase 작성/갱신 -> Offline Eval -> Scorecard -> Release Gate`

### 서비스 루프

`Request -> Trace/Span 기록 -> Online Eval -> Alert/Feedback -> Failure Triage -> EvalCase 편입 -> 다음 Release Gate`
...
- 계획/계약 정리: `PromptContract`, `WorkflowProfile`, `ServiceGuardrailPolicy` 영향을 정리한다.
- 구현: 도메인 경계 안에서만 수정한다.
...
- Prompt 또는 workflow를 바꾸면 해당 시나리오 문서와 eval spec도 같이 바꾼다.
