...
PMS에서 자연어 기반 생성 기능을 사용하더라도 승인된 구조와 권한 안에서만 요약, 초안, 상태 제안을 수행하도록 한다.

## input schema

| 필드 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `request_type` | enum | yes | `project-summary`, `task-update`, `issue-triage`, `milestone-update` |
| `source_ids` | string[] | no | 참조 프로젝트/이슈/작업/마일스톤 ID |
...
- 없으면 최근 관련 프로젝트/작업/이슈/마일스톤을 검색하되 권한 필터를 먼저 적용한다.
