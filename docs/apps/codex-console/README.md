...
프록시를 함께 맞춘다. 일반 JSON 요청의 128 KiB 제한은 유지된다.
...
`codex-console migrate`를 실행하고 `current`를 전환해 별도로 재시작한다. 첨부 기능은
`console_0002` migration이 필요하며 기존 작업·로그인·계획 기록을 보존한다. 이전 서버는
...
- 요구사항·계획은 읽기 전용 sandbox와 승인 불허 정책을 사용한다. 구현 버튼은 최신 계획
  버전에 묶인다. 요구사항 변경은 기존 계획 승인을 무효화한다.
...
- 백엔드·Codex 종료 또는 접수 응답 유실은 `uncertain`이다. workspace lease를 유지하고
  승인 요청을 폐기한다. **실행 상태 확인**은 공식 thread를 재개·조회하며 사용자 요청을
  자동 재전송하지 않는다. 이전 CLI 대화는 원래 실행을 종료한 후에만 가져온다.
...
- 서버 로그에 원본 프롬프트·토큰·upstream 오류 본문을 기록하지 않는다. diff 조회는 비밀 파일,
