...
- 공식 `codex app-server` **0.155.1**의 stdio 프로토콜을 사용한다. Codex가 구독 인증,
...
현재 사용자로 `codex login status`가 성공하는 Codex 0.155.1이 필요하다.
...
OWH 개발 서버나 운영 앱만 재시작해도 콘솔에는 반영되지 않는다. 콘솔 배포가 요청된 경우
다음 항목까지 완료해야 배포 완료로 보고한다.
