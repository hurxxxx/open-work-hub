...
Codex Console 구현을 요청하면 로컬 변경 또는 검증만으로 범위를 제한하지 않은 한 소스의
`dev` 통합과 전용 콘솔 릴리스 교체를 하나의 전달 범위로 본다. 이 경우 다음 항목까지
완료해야 작업 완료로 보고한다. 권한 범위와 `dev -> main` 승인은 루트 `AGENTS.md`를 따른다.
