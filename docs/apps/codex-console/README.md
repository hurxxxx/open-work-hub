...
않는다. 별도 요구사항 단계와 탭은 제공하지 않는다. 계획은 최대 100,000자로 콘솔 DB에만
...
`console_0007` migration까지 필요하다. 0007은 저장된 계획 버전이 없는 완료 작업의 최신
native `plan` item을 계획 문서로 복원한다. 이전 구조화 응답이면 계획 문서 본문을 우선하고,
없으면 native plan item의 답변을 사용한다. 기존 요구사항 이력은 삭제하지 않지만 현재 API와
화면에는 투영하지 않는다. 0004에서 업그레이드할 때 기존 계획 operation에는
...
새 JSON 요청이나 형식을 입증할 수 없는 응답은 Markdown으로 추정하지 않는다. 계획 응답의
실행 출처와 이전 실행 경로, 권한 전환 제출 직전 확인된 sandbox를 저장하며 기존
