...
새 작업의 기본 실행 모드는 **일반 대화**다. 질문·설명·상태 확인을 native default 모드와
읽기 전용 권한으로 수행하며 요구사항·계획 문서를 생성하거나 갱신하지 않는다. 이전 계획
승인도 일반 질문으로 무효화하지 않는다. 파일 수정은 입력 내용을 유지한 채 **구현**으로
전환하고 최신 계획을 승인한다. 별도 Git 작업은 아래 **병합 요청**의 명시적 범위를 사용한다.

입력창의 **실행 모드**에서 일반 대화·요구사항·구현 계획·구현을 선택한다. 요구사항·계획은 native plan
모드와 읽기 전용 권한으로 실행하고, 구현은 저장된 최신 계획을 확인한 뒤 native default
모드로 실행한다. 모델·추론 강도는 다음 메시지에 적용한다. 이미 실행 중인 턴에는 보충 지시를
보낼 수 있으며 모델·모드·권한 변경은 해당 턴이 끝난 뒤 적용한다.

구현 권한의 기본값은 **필요할 때 승인 요청**이다. **YOLO · 전체 권한**을 명시적으로 선택하면
공식 `approvalPolicy: never`, `sandboxPolicy: dangerFullAccess`로 실행한다. 승인 창에서 모델과
YOLO 선택을 다시 표시한다. YOLO는 서버 계정이 접근할 수 있는 파일·네트워크에 대한 샌드박스
제한과 개별 승인 요청을 없앤다. 계획 모드는 YOLO 선택과 관계없이 읽기 전용이다. 각 작업은
마지막으로 실행한 모델·강도·실효 권한을 DB에 보관해 새로 접속해도 표시한다.

**현재 실행 상태**에는 모드·모델·권한, 현재 명령과 공식 계획 도구의 단계별 진행 상태가
표시된다. 단계 완료 수는 Codex가 보고한 값이며 시간이나 전체 구현 완성도를 추정하지 않는다.
작업은 서버에서 실행하므로 페이지 이동·탭 종료·웹 로그아웃으로 중단하지 않는다. **중단**을
누를 때만 명시적으로 실행을 멈춘다. 서버·Codex 프로세스 자체가 재시작되면 아래의 복구 절차가
필요하다. SSE 재연결과 별개로 10초마다 저장 상태를 확인하고 페이지 복귀 때 즉시 갱신한다.

연결 유실로 상태가 불확실하면 **실행 상태 확인**으로 공식 이력과 저장 상태를 맞춘다.
그 다음 **중단 지점부터 계속**을 선택한다. 구현 재개에서는 현재 diff와 유지할 변경을 확인하고
같은 native thread·작업 디렉터리·파일을 사용해 남은 계획을 수행한다. 기존 요청을 자동 재전송하지
않으며 새 요청 ID로 이어서 실행한다. 계획을 수정했다면 최신 버전을 다시 확인해야 한다.
실행 전에 실패한 요청은 **전송하지 못한 요청 불러오기**로 본문을 입력창에 복원할 수 있다.
첨부는 필요한 파일을 다시 선택한다. 본문 복원만으로 재실행하지 않는다.

Codex 0.154.0의 이미 열린 thread는 `thread/resume`에서 이전 실행의 sandbox와 cwd를
반환할 수 있다. 콘솔은 이전에 부여한 작업 폴더 쓰기 권한 또는 명시적 YOLO만 재개 응답으로
허용하고, 새 `turn/start`에 선택 모드의 sandbox·승인 정책·저장된 작업 경로를 항상 지정한다.
일반 대화·계획으로 돌아오면 읽기 전용으로 낮춘다. 알 수 없거나 기록보다 넓은 재개 권한은
계속 차단한다. thread 응답의 이전 cwd를 현재 작업 경로로 사용하지 않는다.

## 브랜치 상태와 병합 요청

작업 상단에 실제 작업 폴더·브랜치·HEAD, 스테이징/미스테이징/새 파일 수·충돌·추적 브랜치와
앞선/뒤처진 커밋 수를 표시한다. 브랜치가 없는 워크트리는 **브랜치 없음 (detached HEAD)**으로
표시한다. 상태는 10초마다, 페이지 복귀 및 수동 새로고침 때 조회한다. 원격 비교는 로컬에
저장된 추적 참조 기준이며 자동 fetch나 브랜치 변경은 하지 않는다. 조회 시각은 원격 최신성의
보장이 아니다. 변경 파일 수만으로 작업의 미병합 여부를 판단하지 않는다.

**병합 요청**에서 저장소에 존재하는 대상 참조와 **MR/PR 생성까지** 또는 **검사·리뷰 후
병합까지**를 선택한다. **채팅창에 요청문 넣기**는 기존 초안을 보존해 요청문을 추가하고 구현
모드로 전환한다. 전송할 때 표시된 Git 범위·모델·권한을 확인하면 같은 세션에서 실행한다.
Git 요청은 저장된 구현 계획과 별도의 명시적 승인으로 기록된다. 일반 구현의 계획 승인 규칙은
유지한다. 서버는 작업의 저장된 경로에서 Git 상태·대상 참조를 재검사하고, 선택 이후 상태가
바뀌거나 충돌·대상 삭제가 발견되면 전송을 차단한다. 임의 cwd나 클라이언트가 만든 Git 상태를
실행 기준으로 받지 않는다.

요청문에는 기존 반영 여부·MR/PR 확인, 무관한 변경 보존, 필요하면 별도 소스 브랜치 준비,
저장소의 remote·보호 브랜치·검사·리뷰 정책 준수를 포함한다. 선택한 Git 작업의 커밋·푸시·
MR/PR 작성까지만 기본 승인되며, 병합은 선택한 범위일 때만 포함된다. 병합 범위의 자동 완성
요청문에는 병합 완료 및 최신 변경 반영을 확인한 뒤 이번 작업의 로컬·원격 소스 브랜치를
정리하고 해당 원격 추적 정보를 갱신하는 지시도 기본 포함한다. 보호·대상·유지용 브랜치, 다른 작업이 사용하는 브랜치,
미커밋·미병합 변경은 보존하고 정리 결과와 제외 사유를 보고하도록 한다. 현재 세션의 작업
디렉터리는 유지한다. MR/PR 생성만 선택한 요청문에는 병합·브랜치 정리를 포함하지 않는다.
운영 배포·강제 푸시·변경 폐기·지속 통합 브랜치 삭제는 포함하지 않는다.
UI 버튼은 Git 명령을 직접 실행하지 않는다.
...
6. 새 작업의 일반 대화 기본값과 질문 후 문서가 변경되지 않는지, 모델 목록·계획/구현 전환·
   기본 승인/YOLO 표시를 확인한다. 브랜치/분리 HEAD와 변경 상태, 병합 대상·범위 선택 및
   요청문 삽입만으로 실행되지 않는지도 확인한다. 검증용 작업을 시작한 후 탭을
   닫았다가 같은 task URL로 돌아와 완료 상태와 진행 단계가 복원되는지 확인한다. 중단된 검증용
   작업의 diff를 확인하고 **중단 지점부터 계속**하여 기존 파일이 유지되는지도 확인한다.
...
`console_0004` migration까지 필요하다. 첨부·문서 실행 출처와 모델·권한·진행 상태를 저장하며 기존
...
- 기본 구현은 작업 디렉터리의 workspace-write sandbox를 사용한다. YOLO의 명시적 선택은
