...
When exploring the codebase, use current code, tests, the owning domain/app documents, and accepted decisions in root `adr/` to build a clear model of the relevant modules.
...
- [ ] The hypothesis that turned out correct is recorded in the requested handoff, commit, or pull-request description so the next debugger learns

**Then ask: what would have prevented this bug?** If architectural work is a separate
user-requested outcome, route the findings to `improve-codebase-architecture`; do not
load that skill automatically during the bug fix.
