---
name: open-alm-mr-review-validation
description: Review Open ALM GitLab merge requests for mergeability and project contract risk. Use when the user explicitly asks to review an MR, decide whether it can merge, or address existing MR review findings. Do not use for ordinary implementation, local code reading, or a pre-commit self-review.
---
...
4. On `/projects/open-alm/dev`, keep the checkout on `dev`. Add the worktree skill only
