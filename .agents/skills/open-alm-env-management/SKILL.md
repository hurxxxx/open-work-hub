...
- The runtime `.env` keysets for `/projects/open-alm/dev`, `/projects/open-alm/prod`, `.env.local`, and `.env.example` must stay identical unless a deliberate exception is documented.
...
- Project-owned keys use `OPEN_ALM_*`. External/tooling keys may use their established names such as `COMPOSE_PROJECT_NAME`, `VITE_*`, `GITLAB_*`, and `HF_TOKEN`.
...
```bash
bash .agents/skills/open-alm-env-management/scripts/env-inventory.sh /projects/open-alm/dev
```
...
```bash
bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh status --profile production
...
2. Classify the key:
   - `OPEN_ALM_*`: normal app/service config.
...
5. Compare and update Secure Files:
   ```bash
   bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh status --profile production
   bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh upload --profile production --source /projects/open-alm/prod/.env
   bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh status --profile local
   bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh upload --profile local --source /path/to/.env.local
   ```
...
- Canonical Secure File profile names are:
  - `production` -> `.env.production`, normally sourced from `/projects/open-alm/prod/.env`
  - `local` -> `.env.local`, normally sourced from a developer-safe `.env.local`
- `.secure` is the System Operations Information Ledger (`시스템 운영 정보 대장`), not a runtime env file. Its remote GitLab Secure File is canonical; `/projects/open-alm/dev/.secure` is a Git-ignored `0600` working copy. Never source it.
...
```bash
bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh status --secure-file-name .secure --source /projects/open-alm/dev/.secure
bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh download --secure-file-name .secure --target /projects/open-alm/dev/.secure
stat -c '%a %n' /projects/open-alm/dev/.secure
```
