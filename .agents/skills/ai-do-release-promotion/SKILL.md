---
name: ai-do-release-promotion
description: Promote AI-DO changes from dev to production through GitLab merge requests, validation checks, and production checkout deployment. Use when preparing a release, promoting dev to main, or coordinating production rollout.
---
...
1. Work on `/projects/ai-do/dev` branch `dev`.
...
6. After merge, update `/projects/ai-do/prod` to `origin/main`.
...
- Merge with a full source SHA guard, for example:
  ```bash
  SOURCE_SHA="$(git -C /projects/ai-do/dev rev-parse origin/dev)"
  glab mr merge <iid> --sha "$SOURCE_SHA" --auto-merge=false --message "Merge branch 'dev' into 'main'" --yes
  ```
- In `/projects/ai-do/prod`, deploy only the remote merge result:
  ```bash
