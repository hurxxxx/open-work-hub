---
name: open-alm-production-operations
description: "Operate the Open ALM production checkout safely: render/start/restart/status/smoke, run migrations deliberately, and validate rollback readiness. Use when deploying, restarting, checking, or rolling back production on /projects/open-alm/prod."
---
...
- Run production commands only from `/projects/open-alm/prod`.
...
- If production `.env` keys or values are also in scope, add
  `open-alm-env-management` and update the `.env.production` GitLab Secure File
  before restart.
...
```bash
git -C /projects/open-alm/prod branch --show-current
git -C /projects/open-alm/prod status --short
...
- If dev-login appears in prod bootstrap, stop and disable `OPEN_ALM_API_ALLOW_DEV_ADMIN_LOGIN`.
