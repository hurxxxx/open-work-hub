---
name: open-alm-development-environment
description: Manage Open ALM development environments, including the shared server dev infra and local developer app setup through published dev infra ports. Use when setting up local development, starting dev services, checking dev infra, or onboarding developers.
---
...
- Server dev checkout lives at `/projects/open-alm/dev`.
- Server dev checkout uses `OPEN_ALM_ENV_PROFILE=dev` and API port `8001`.
- `/projects/open-alm/dev` is the live development server checkout and must stay on the `dev` branch. On-server development uses this checkout directly unless the user explicitly requests a parallel worktree branch. Local developer machines create normal feature branches in their current checkout instead of using `/projects/open-alm/worktrees`.
...
- PostgreSQL is a native service/database in server dev and tests, not a Docker container. Do not pull or run PostgreSQL Docker images for Open ALM. If `pgvector` is missing, install/enable the native PostgreSQL `vector` extension and keep the planned pgvector path instead of falling back to Qdrant or non-vector behavior.
...
cd /projects/open-alm/dev
git pull --ff-only origin dev
systemctl --user restart open-alm-dev-app.service
pnpm dev:smoke
```

If `https://dev.open-alm.example/` stays on "세션 확인 중", treat it as an API/session
bootstrap failure first. Check the API proxy and service logs before changing
code:

```bash
curl -sS -D - --max-time 15 https://dev.open-alm.example/api/v1/auth/bootstrap-status
systemctl --user --no-pager --plain status open-alm-dev-app.service
journalctl --user -u open-alm-dev-app.service --since '10 minutes ago' --no-pager
```
...
repair, run `alembic upgrade head`, restart `open-alm-dev-app.service`, and run
`pnpm dev:smoke`.

## Local Developer

Use `.env.local` (GitLab Secure File, `OPEN_ALM_*` contract per `.env.example`) at the repo root.
...
management task too. Add `open-alm-env-management`, then authenticate `glab` if
needed and download/update the `.env.local` GitLab Secure File without printing
values:

```bash
bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh status --profile local
bash .agents/skills/open-alm-env-management/scripts/secure-env-files.sh download --profile local --target .env.local
```
...
The fetch script auto-installs glab via winget when missing, configures the open-alm GitLab host (HTTP on :8929), reads `GLAB_PAT`/`GITLAB_TOKEN` from the environment for non-interactive auth (falls back to `glab auth login`), and downloads the secure file named `.env.local` into the repo root. PAT generation remains a human step at the GitLab UI.
