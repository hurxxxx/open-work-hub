...
4. The owning document under `docs/domains/`, `docs/apps/`, or `docs/product/`.
5. Accepted decisions under the repository-root `adr/` directory.

Use `docs/product/` for product intent. If dated or historical material is encountered elsewhere, it does not override current code or owner documentation.

Do not create parallel glossary files or nested ADR directories. Open Work Hub records domain language in the relevant current/domain/app document and architecture decisions in root `adr/`.
