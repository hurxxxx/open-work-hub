...
def test_platform_extension_bootstrap_registers_core_adapters_after_reset() -> None:
    ...
    try:
        ...
        assert "legacy_issues" in snapshot.conversation_scope_refs
        ...
        expected_retrieval_partition_adapter_ids = (
            ...
            if KNOWLEDGE_SOURCE_REGISTRY_RETIRED
            else (
                "docs",
                "files",
                "knowledge",
...
def test_default_conversation_scope_adapters_reregister_after_reset() -> None:
    ...
    assert get_conversation_scope_adapter("legacy_issues") is not None
...
def test_web_search_workloads_register_as_external_only() -> None:
    ...
    try:
        ...
        assert registry.resolve_llm_workload("ppt.design").allowed_routes == (
            "local",
            "external",
        )
    finally:
...
def test_default_source_access_adapters_reregister_after_reset() -> None:
    ...
    policy = SourceAclPolicy(
        db=SimpleNamespace(),
        workspace=SimpleNamespace(id="ws-1"),
        user=SimpleNamespace(id="user-1", primary_org_unit_id=None),
        workspace_role=None,
    )
