...
def dev_login(client: TestClient, account_key: str = "administrator") -> dict:
    ...
    definition = _NAMED_USERS.get(account_key)
...
def create_company_user_session(
    client: TestClient,
    *,
    login_id: str,
    email: str,
    ...
    admin = _administrator_session(client)
    _ensure_user(client, admin["token"], login_id=login_id, email=email, full_name=full_name)
    response = client.post(
        "/api/v1/auth/login", json={"login_id": login_id, "password": TEST_USER_PASSWORD}
    )
    assert response.status_code == 200, response.text
    return response.json()
...
def _administrator_session(client: TestClient) -> dict:
    with get_session_factory()() as db:
        initial_accounts = not are_dev_login_accounts_seeded(db)
...
def configure_company_app_access(db, app_ids=None):
    ...
    for app_id in app_ids if app_ids is not None else [a.app_id for a in iter_app_catalog()]:
        ...
        db.get(AppAccessPolicy, app_id).audience = "all"
    db.commit()
