...
TEST_RESOURCE_PREFIX = "ai-do-api-test-"
TEST_INDEX_PREFIX = "ai_do_api_test_"
TEST_DATABASE_PREFIX = "ai_do_test_"
...
def _approved_dev_values(name: str) -> set[str]:
    ...
    for env_file in _env_files():
        ...
        profile = str(values.get("AI_DO_ENV_PROFILE") or "").strip().lower()
...
def _has_non_production_ack() -> bool:
    return os.getenv("AI_DO_TEST_NON_PRODUCTION_ACK", "") == _NON_PRODUCTION_ACK


def _run_token() -> str:
    run_id = os.getenv(
        "AI_DO_API_TEST_RUN_ID",
...
def stale_resource_cutoff() -> datetime:
    raw_hours = os.getenv("AI_DO_TEST_RESOURCE_TTL_HOURS", "")
    try:
        hours = int(raw_hours) if raw_hours else _STALE_RESOURCE_TTL_HOURS
    except ValueError as error:
        raise RuntimeError("AI_DO_TEST_RESOURCE_TTL_HOURS must be an integer.") from error
    if hours < 1:
        raise RuntimeError("AI_DO_TEST_RESOURCE_TTL_HOURS must be at least 1.")
    return datetime.now(timezone.utc) - timedelta(hours=hours)


def _assert_non_production(selected: dict[str, str]) -> None:
    profile = (
        os.getenv("AI_DO_ENV_PROFILE") or _file_value("AI_DO_ENV_PROFILE") or ""
    ).strip().lower()
...
def assert_non_production_postgres_dsn(dsn: str) -> None:
    ...
    approved = _approved_dev_values("AI_DO_POSTGRES_DSN")
    ...
    prod_env = _workspace_root().parent / "prod" / ".env"
    raw_prod_dsn = _env_values(prod_env).get("AI_DO_POSTGRES_DSN")
    if raw_prod_dsn:
    ...
    with psycopg.connect(plain_dsn) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE d.datname LIKE 'ai_do_test_%'
                  AND NOT EXISTS (
...
class IntegrationInfra:
    ...
    def load(cls) -> IntegrationInfra:
        redis_url = _value("AI_DO_TEST_REDIS_URL", "AI_DO_API_COLLAB_REDIS_URL")
        minio_endpoint = _validate_http_endpoint(
            "MinIO endpoint",
            _value("AI_DO_TEST_MINIO_ENDPOINT", "AI_DO_MINIO_ENDPOINT"),
        )
        opensearch_url = _validate_http_endpoint(
            "OpenSearch endpoint",
            _value("AI_DO_TEST_OPENSEARCH_URL", "AI_DO_OPENSEARCH_URL"),
        )
        _assert_non_production(
            {
                "AI_DO_API_COLLAB_REDIS_URL": redis_url,
                "AI_DO_MINIO_ENDPOINT": minio_endpoint,
                "AI_DO_OPENSEARCH_URL": opensearch_url,
            }
        )
        ...
        return cls(
            ...
            minio_access_key=_value(
                "AI_DO_TEST_MINIO_ACCESS_KEY", "AI_DO_MINIO_ACCESS_KEY"
            ),
            minio_secret_key=_value(
                "AI_DO_TEST_MINIO_SECRET_KEY", "AI_DO_MINIO_SECRET_KEY"
            ),
    ...
    def _postgres_template_dsn(self) -> str | None:
        configured = os.getenv("AI_DO_TEST_POSTGRES_TEMPLATE_DSN")
        if configured:
            assert_non_production_postgres_dsn(configured)
            return configured
        root = _workspace_root()
        template = _file_value(
            "AI_DO_POSTGRES_DSN", (root / ".env", root / ".env.local")
        )
        if template:
