...
def _workspace():
    return SimpleNamespace(id="workspace-1", key="workspace-1")
...
def test_workspace_keyword_search_adapters_declare_registered_index_hooks() -> None:
    ensure_search_entity_adapters_registered()
    ensure_search_index_hooks_registered()
...
def test_workspace_backfill_rejects_projection_identity_drift(
    document: dict[str, str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter = SimpleNamespace(
        entity_types=("doc",),
        load_workspace_documents=lambda db, *, workspace: [document],
    )
    monkeypatch.setattr(
        search_projections,
    ...
    with pytest.raises(SearchProjectionIdentityError):
        search_projections.all_workspace_search_documents(
            SimpleNamespace(),
            workspace=_workspace(),
        )


def test_workspace_backfill_rejects_projection_without_acl_fields(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter = SimpleNamespace(
        entity_types=("doc",),
        load_workspace_documents=lambda db, *, workspace: [
            {
...
def test_workspace_keyword_search_rejects_workspace_without_active_source(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(search_service, "resolve_workspace_enabled_app_ids", lambda db, _id: [])
    monkeypatch.setattr(
        search_service,
        "_search_client",
    ...
    with pytest.raises(HTTPException) as error:
        search_service.query_workspace_keyword_search(
            SimpleNamespace(),
            workspace=_workspace(),
            user=_user(),
            request=_request(),
    ...
    assert error.value.headers["X-Open-Work-Hub-Error-Code"] == ("search.workspace_keyword_search_disabled")
...
def test_workspace_keyword_search_returns_empty_for_disallowed_requested_entities(
    ...
    response = search_service.query_workspace_keyword_search(
        SimpleNamespace(),
        workspace=_workspace(),
        user=_user(),
...
def test_workspace_keyword_search_accepts_empty_workspace_index(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        "for_workspace",
        staticmethod(lambda db, *, workspace, user: FakePolicy()),
    )
...
def test_partitioned_keyword_query_uses_partition_then_source_acl_and_hydration(
    ...
    class StagingClient:
        ...
        def search(self, query: KeywordSearchQuery) -> KeywordSearchResult:
            ...
            return KeywordSearchResult(
                hits=(KeywordSearchHit(document=stale_row, score=1.0),)
            )
    ...
    monkeypatch.setattr(
        ...
        lambda db, *, rows, execution_workspace: [
            {
                **rows[0],
                ...
                "metadata": {
                    ...
                    "managed_workspace_id": "workspace-before-publication",
                },
            }
        ],
    )
...
def test_workspace_keyword_search_reports_missing_global_index(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class FakeSearchClient:
        def index_exists(self) -> bool:
...
def test_disabled_source_retains_indexed_documents_for_reenable(
    ...
    first_response = search_service.query_workspace_keyword_search(
        SimpleNamespace(),
        workspace=_workspace(),
        user=_user(),
        request=_request(),
    ...
    reenabled_response = search_service.query_workspace_keyword_search(
        SimpleNamespace(),
        workspace=_workspace(),
        user=_user(),
        request=_request(),
