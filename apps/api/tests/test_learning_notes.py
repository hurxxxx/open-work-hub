...
def test_any_user_can_create_and_edit_their_own_private_note(client: TestClient) -> None:
    ...
    _, alice_token = _create_member(client, admin["token"], email="alice@open-work-hub.local")
...
def test_private_notes_are_hidden_from_everyone_else_including_admin(
    ...
    admin = _bootstrap_admin_session(client)
    _, alice_token = _create_member(client, admin["token"], email="alice@open-work-hub.local")
    _, bob_token = _create_member(client, admin["token"], email="bob@open-work-hub.local")
...
def test_public_notes_are_listed_and_readable_by_everyone(client: TestClient) -> None:
    ...
    alice_id, alice_token = _create_member(
        client, admin["token"], email="alice@open-work-hub.local", full_name="Alice"
    )
    _, bob_token = _create_member(client, admin["token"], email="bob@open-work-hub.local", full_name="Bob")
