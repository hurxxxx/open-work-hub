...
from ai_do_api.domains.ai.legacy_provider_import import (
    LegacyProviderImportError,
    import_legacy_external_llm_providers,
)
from ai_do_api.domains.ai.model_credentials import decrypt_api_key, encrypt_api_key
...
def test_preview_reports_changes_without_writing(
    ...
    monkeypatch.setenv("AI_DO_LLM_OPENAI_API_KEY", "preview-secret")
    monkeypatch.setenv("AI_DO_LLM_OPENAI_DEFAULT_MODEL", "preview-model")
...
def test_apply_uses_descriptor_endpoint_and_is_idempotent(
    ...
    _remove_provider(db_factory, "openai")
    monkeypatch.setenv("AI_DO_LLM_OPENAI_API_KEY", "apply-secret")
    monkeypatch.setenv("AI_DO_LLM_OPENAI_DEFAULT_MODEL", "imported-chat-model")
...
def test_apply_never_overwrites_existing_database_values(
    ...
    monkeypatch.setenv("AI_DO_LLM_ANTHROPIC_API_KEY", "environment-secret")
    monkeypatch.setenv("AI_DO_LLM_ANTHROPIC_BASE_URL", "https://environment.example.test")
    monkeypatch.setenv("AI_DO_LLM_ANTHROPIC_DEFAULT_MODEL", "environment-model")
...
def test_missing_encryption_root_rolls_back_the_transaction(
    ...
    _remove_provider(db_factory, "openai")
    monkeypatch.setenv("AI_DO_LLM_OPENAI_API_KEY", "rollback-secret")
    monkeypatch.setenv("AI_DO_LLM_OPENAI_DEFAULT_MODEL", "rollback-model")
