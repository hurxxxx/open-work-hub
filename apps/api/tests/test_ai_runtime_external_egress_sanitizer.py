...
def test_external_egress_sanitizer_blocks_credentials() -> None:
    sanitization = build_external_egress_sanitization(
        "API_KEY=sk-test-secret 를 포함해서 공개 자료를 검색해줘."
    )

    assert sanitization.removed_entity_types == ["credential"]
    assert sanitization.blocked_entity_types == ["credential"]
...
def test_external_egress_sanitizer_removes_internal_identifiers() -> None:
    text = "INTERNAL-PROJECT-2400 공개 접근성 요구사항을 검색해줘."

    sanitization = build_external_egress_sanitization(text)

    assert sanitization.removed_entity_types == ["internal_identifier"]
    assert sanitization.blocked_entity_types == []
    assert "INTERNAL-PROJECT-2400" not in sanitization.sanitized_prompt
    assert sanitization.sanitized_query == "공개 접근성 요구사항을 검색해줘"


def test_external_egress_sanitizer_preserves_public_prompt_when_no_tokens() -> None:
    prompt = "공개 규격만 기준으로 초기 분석해줘"

    assert detect_sensitive_entity_types(prompt) == []
    assert sanitize_external_prompt(prompt) == prompt
    assert sanitize_external_search_query(prompt) == prompt


def test_external_egress_sanitizer_removes_internal_id_and_truncates_prompt() -> None:
    prompt = f"공개 규격 검토 INTERNAL-REPORT-123 {'x' * 1300}"

    sanitized = sanitize_external_prompt(prompt)

    assert "INTERNAL-REPORT-123" not in sanitized
