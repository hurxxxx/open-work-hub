...
def _create_video_session(client: TestClient, token: str, title: str) -> dict:
...
def test_video_chat_session_create_list_and_join_token(client: TestClient) -> None:
    ...
    assert session["room_name"].startswith("open-work-hub-")
    ...
    token_response = client.post(
        f"/api/v1/video-chat/sessions/{session['id']}/join-token",
        headers=_auth_headers(admin["token"]),
    )
    ...
    assert claims["sub"].startswith(f"{admin['user']['id']}:")
...
def test_video_chat_join_token_uses_remote_request_host_for_dev_livekit_url(
    ...
    admin = _bootstrap_admin_session(client)
    session = _create_video_session(client, admin["token"], "Remote browser")
...
def test_video_chat_join_token_prefers_configured_public_livekit_url(
    ...
    try:
        admin = _bootstrap_admin_session(client)
        session = _create_video_session(client, admin["token"], "Public URL")
...
def test_video_chat_join_token_identity_is_unique_per_join_request(
    ...
    admin = _bootstrap_admin_session(client)
    session = _create_video_session(client, admin["token"], "Identity")
    ...
    identity_prefix = f"{admin['user']['id']}:"
...
def test_video_chat_ended_session_cannot_issue_join_token(client: TestClient) -> None:
    admin = _bootstrap_admin_session(client)
    session = _create_video_session(client, admin["token"], "Close me")

    end_response = client.post(
        f"/api/v1/video-chat/sessions/{session['id']}/end",
        headers=_auth_headers(admin["token"]),
    )
...
def test_video_chat_non_owner_cannot_end_session(client: TestClient) -> None:
    admin = _bootstrap_admin_session(client)
    session = _create_video_session(client, admin["token"], "Owner only")
...
def test_video_chat_recording_and_captions_are_feature_flagged(client: TestClient) -> None:
    admin = _bootstrap_admin_session(client)
    session = _create_video_session(client, admin["token"], "Feature flags")

    recording = client.post(
        f"/api/v1/video-chat/sessions/{session['id']}/recording/start",
        headers=_auth_headers(admin["token"]),
    )
