...
def _reset_test_database(engine) -> None:
    ...
    if not configured_database.startswith("ai_do_test_"):
        raise RuntimeError(
    ...
    with engine.begin() as connection:
        ...
        if database != configured_database or not database.startswith("ai_do_test_"):
...
def _native_postgres_template_dsn() -> str:
    root = _workspace_root()
    configured = _env_file_value("AI_DO_TEST_POSTGRES_TEMPLATE_DSN")
    if not configured:
    ...
    if not configured:
        raise RuntimeError(
            "Native PostgreSQL tests require AI_DO_POSTGRES_DSN or "
            "AI_DO_TEST_POSTGRES_TEMPLATE_DSN."
        )
...
def _create_native_test_database(template_dsn: str, database: str) -> str:
    ...
    with psycopg.connect(admin_dsn.replace("postgresql+psycopg://", "postgresql://", 1)) as conn:
        ...
        with conn.cursor() as cursor:
            ...
            marker = f"ai-do-test-created-at={datetime.now(timezone.utc).isoformat()}"
            cursor.execute(
...
def _native_test_database(template_dsn: str, *, role: str) -> Iterator[str]:
    database = f"ai_do_test_{TEST_RUN_TOKEN}_{role}_{uuid.uuid4().hex[:8]}"
...
def _migrate_application_test_database(dsn: str) -> None:
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        monkeypatch.setenv("AI_DO_POSTGRES_DSN", dsn)
...
def _initialize_application_test_database(dsn: str) -> None:
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        monkeypatch.setenv("AI_DO_POSTGRES_DSN", dsn)
        monkeypatch.setenv("AI_DO_API_AUTO_MIGRATE", "0")
        monkeypatch.setenv("AI_DO_MAIL_CREDENTIAL_ENCRYPTION_KEY", "test-mail-credential-key")
        monkeypatch.setenv(
...
def _ensure_ai_analysis_reader_role(dsn: str) -> None:
    ...
    try:
        with psycopg.connect(plain_dsn) as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT pg_has_role(current_user, 'ai_do_analysis_reader', 'MEMBER')"
                )
...
def _required_settings_env(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    monkeypatch.setenv("AI_DO_POSTGRES_DSN", TEST_POSTGRES_DSN)
    monkeypatch.setenv("AI_DO_MAIL_CREDENTIAL_ENCRYPTION_KEY", "test-mail-credential-key")
    monkeypatch.setenv(
        "AI_DO_AI_MODEL_CREDENTIAL_ENCRYPTION_KEY",
        "test-ai-model-credential-key",
...
def in_memory_object_storage(monkeypatch: pytest.MonkeyPatch) -> None:
    from ai_do_api.domains.diagrams import storage as diagram_storage
    from ai_do_api.domains.dm import attachment_storage
    from ai_do_api.domains.files import storage_adapter
...
def _configure_test_application_environment(
    ...
    *,
    ...
    minio_bucket: str = "ai-do-test-unused",
) -> None:
    monkeypatch.setenv("AI_DO_POSTGRES_DSN", postgres_dsn)
    monkeypatch.setenv("AI_DO_API_SESSION_TTL_HOURS", "1")
    monkeypatch.setenv("AI_DO_API_ALLOW_DEV_ADMIN_LOGIN", "1")
    monkeypatch.setenv("AI_DO_LLM_HEALTHCHECK_ON_STARTUP", "0")
    monkeypatch.setenv("AI_DO_OPF_HEALTHCHECK_ON_STARTUP", "0")
    monkeypatch.setenv("AI_DO_OPF_REQUIRED", "0")
    monkeypatch.setenv("AI_DO_OPF_SERVICE_BASE_URL", "")
    monkeypatch.setenv("AI_DO_API_AUTO_MIGRATE", "0")
    monkeypatch.setenv("AI_DO_API_COLLAB_REDIS_URL", "redis://127.0.0.1:1/0")
    monkeypatch.setenv("AI_DO_API_REALTIME_REDIS_URL", "redis://127.0.0.1:1/0")
    monkeypatch.setenv("AI_DO_MINIO_ENDPOINT", minio_endpoint)
    monkeypatch.setenv("AI_DO_MINIO_ACCESS_KEY", minio_access_key)
    monkeypatch.setenv("AI_DO_MINIO_SECRET_KEY", minio_secret_key)
    monkeypatch.setenv("AI_DO_MINIO_BUCKET", minio_bucket)
    monkeypatch.setenv("AI_DO_MAIL_CREDENTIAL_ENCRYPTION_KEY", "test-mail-credential-key")
    monkeypatch.setenv(
        "AI_DO_AI_MODEL_CREDENTIAL_ENCRYPTION_KEY",
        "test-ai-model-credential-key",
    ...
    monkeypatch.setenv("AI_DO_LLM_LOCAL_PROVIDER", "mlx-lm")
    ...
    monkeypatch.setenv("AI_DO_IMAGE_ENABLED", "0")
...
def _build_test_application(
    ...
    *,
    ...
    from ai_do_api.external_runtime import (
    ...
    try:
        if reused_app:
            ...
            test_client = _build_client(
                ...
                minio_bucket=minio.bucket if minio else "ai-do-test-unused",
            )
        with test_client:
            yield test_client
    finally:
