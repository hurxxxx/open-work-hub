...
from open_work_hub_api.static_serving import apply_static_response_headers
...
def prepare_open_work_hub_desktop_update_dirs(settings: Settings) -> dict[str, Path]:
    update_dirs = open_work_hub_desktop_update_dirs(settings)
    for update_dir in update_dirs.values():
        update_dir.mkdir(parents=True, exist_ok=True)
    return update_dirs


def mount_open_work_hub_desktop_update_feeds(
    app: FastAPI,
    settings: Settings,
    update_dirs: dict[str, Path] | None = None,
) -> None:
    resolved_update_dirs = (
        update_dirs if update_dirs is not None else open_work_hub_desktop_update_dirs(settings)
    )
    ...
    for platform in policy.platforms:
        app.mount(
            ...
            name=f"open-work-hub-desktop-updates-{platform}",
        )
