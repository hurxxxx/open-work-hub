...
The wire contract is versioned by event type. Consumers must ignore unknown
types so new publishers can be added without breaking older clients.
"""
...
ENVELOPE_SCHEMA_ADAPTER: TypeAdapter[Any] = TypeAdapter(AgentEventEnvelope)


PUBLISHED_EVENT_TYPES: dict[str, type[_EnvelopeBase]] = {
...
def make_envelope(
    ...
    *,
    ...
    cls = PUBLISHED_EVENT_TYPES.get(event_type)
    if cls is None:
...
def envelope_schema() -> dict[str, Any]:
    ...
    return ENVELOPE_SCHEMA_ADAPTER.json_schema()
