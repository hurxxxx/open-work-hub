...
from open_work_hub_api.domains.ai.model_credentials import AiModelCredentialError, encrypt_api_key
...
def _read_environment(environment: Mapping[str, str]) -> _LegacyImageEnvironment | None:
    ...
    provider_id = (
        normalize_image_provider_id(environment.get("OPEN_WORK_HUB_IMAGE_PROVIDER")) or "openai"
    )
    provider_keys = _provider_api_keys(environment)
    provider_specific_field = (
        "OPEN_WORK_HUB_IMAGE_"
    ...
    api_key = (
        ...
        or environment.get("OPEN_WORK_HUB_IMAGE_API_KEY", "").strip()
    )
    return _LegacyImageEnvironment(
        provider_id=provider_id,
        endpoint_url=environment.get("OPEN_WORK_HUB_IMAGE_BASE_URL", "").strip().rstrip("/"),
        supervisor_model_id=environment.get("OPEN_WORK_HUB_IMAGE_SUPERVISOR_MODEL", "").strip(),
        generation_model_id=environment.get("OPEN_WORK_HUB_IMAGE_MODEL", "").strip(),
        api_key=api_key,
...
def _provider_api_keys(environment: Mapping[str, str]) -> dict[str, str]:
    raw = environment.get("OPEN_WORK_HUB_IMAGE_PROVIDER_API_KEYS", "").strip()
    if not raw:
        return {}
    try:
