...
from open_alm_api.api_registry import register_api_routers
from open_alm_api.client_build import ClientBuildGuardMiddleware, read_frontend_build_id
from open_alm_api.core.db import get_session_factory, init_db
...
from open_alm_api.core.runtime_diagnostics import (
...
from open_alm_api.open_alm_desktop_updates import (
    mount_open_alm_desktop_update_feeds,
    prepare_open_alm_desktop_update_dirs,
)
from open_alm_api.frontend import mount_frontend
from open_alm_api.domains.ai.runtime.registry_validation import RuntimeRegistryValidationError
...
from open_alm_api.external_runtime import ApiExternalRuntime, ProductionApiExternalRuntime
from open_alm_api.openapi_contract import stable_operation_id
from open_alm_api.platform_extensions import initialize_platform_extensions
from open_alm_api.version import RUNTIME_REVISION, VERSION as APP_VERSION
...
def create_app(
    *,
    ...
    open_alm_desktop_update_dirs = prepare_open_alm_desktop_update_dirs(settings)
    telemetry_enabled = bootstrap_telemetry(
        service_name="open-alm-api",
    ...
    telemetry_tracer = get_tracer("open_alm_api.http")
    ...
    async def add_instance_headers(request, call_next) -> FastAPIResponse:
        ...
        response.headers["X-Open ALM-Instance-Id"] = settings.instance_id
        trace_id = current_trace_id()
        if trace_id is not None:
            response.headers["X-Open ALM-Trace-Id"] = trace_id
        return response
    ...
    mount_open_alm_desktop_update_feeds(app, settings, open_alm_desktop_update_dirs)
