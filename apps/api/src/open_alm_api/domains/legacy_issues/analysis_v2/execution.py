...
def postgres_scope_binder(scope: AnalysisSqlScope) -> ScopeBinder:
    ...
    settings = (
        ("open_alm.legacy_issue_workspace_id", scope.workspace_id),
        ("open_alm.legacy_issue_partition_ids", ",".join(scope.partition_ids)),
        ("open_alm.legacy_issue_module_keys", ",".join(scope.module_keys)),
        ("open_alm.legacy_issue_revision_ids", ",".join(scope.revision_ids)),
        ("open_alm.legacy_issue_checklist_ids", ",".join(scope.checklist_ids)),
    )

    def _bind(connection: Connection) -> None:
        connection.exec_driver_sql("SET LOCAL ROLE open_alm_analysis_reader")
