...
본문(corporate-v2는 N장, A4는 1장 고정) 구조의 슬라이드 JSON을 LLM이 생성하도록 안내한다.
"""
...
from open_alm_api.domains.ppt_generator.families import (
    DEFAULT_FAMILY,
    SLIDE_RANGE_MAP,
    resolve_family,
)
...
# ============================================================
# 각 layout의 data 스키마 — Open ALM v2 명세, 4가지 본문 패턴 (16:9)
# ============================================================
_LAYOUT_SCHEMA = """\
...
# Open ALM 사내 진행보고 "하우스 스타일"(SPEC_house_style.md §5) — 블록 데이터 모델 스키마.
...
9) outline — **번호가 붙는 계층 목록**. 대항목 아래 하위 항목이 여러 개 나열되는 내용일 때.
   ...
   - 예시(이 형태를 그대로 따르세요):
     {"type":"outline","section":"AI TFT팀 주요 활동 내용","items":[
       {"text":"고성능 PC 확보하여 반복 루틴 업무 개선 프로그램 Open ALM 자체 개발중","level":1},
...
- 과장·홍보성 수식어 금지. Open ALM 실무 보고서처럼 **건조하고 사실 위주**로.
...
_FAMILY_SCHEMAS = {
    "corporate-v2": _LAYOUT_SCHEMA,
    "corporate-house": _HOUSE_SCHEMA,
...
# ============================================================
# Open ALM 양식(자유 본문) — Open ALM 표지/본문 틀 + Claude 본문 콘텐츠
#   Claude 는 "본문 칸(1244×754px)에 들어갈 콘텐츠"만 만든다. 표지·제목·날짜·로고·워터마크·
#   저작권 같은 틀은 절대 만들지 않는다(서버 corporate_frame 가 감쌈).
#   본문 디자인은 v3.9.1 §14 Information Design(Stripe+Bloomberg+Linear) 철학을 적용.
# ============================================================
from open_alm_api.domains.ppt_generator.design.corporate_frame import (  # noqa: E402
...
def build_corporate_free_prompt(
    content: str,
    n_slides: int,
    language: str,
    tone: str,
    ...
    """Open ALM 양식 본문 콘텐츠 생성 프롬프트 (system, user).

    Claude 는 본문 칸 전용 HTML 콘텐츠만 출력. 서버가 Open ALM 표지/본문 틀로 감싼다.
    """
    bw, bh = _BODY_W_PX, _BODY_H_PX
    body_n = max(1, min(14, (n_slides or 6)))
    design_authority = _BRANDLOGY_MD or _BRANDLOGY_SCHEMA
    sys_prompt = f"""당신은 Open ALM(OPEN ALM) 경영 보고서 본문을 디자인하는 프레젠테이션 디자이너입니다.
...
- 본문 슬라이드 {body_n}장. 각 슬라이드는 정확히 다음 형태의 <section> 하나:
    <section class="corporate-body" data-title="슬라이드 제목" data-subtitle="(선택) 한 줄 부제">
...
═══════════════════════════════════════════════════════════
[Open ALM 본문 규칙 — 위 디자인 시스템과 충돌 시 이쪽 우선]
═══════════════════════════════════════════════════════════
...
- **색 오버라이드**: 브랜드 블루(#1456f0 등) 대신 **Open ALM 네이비 #1B2C6B + 강조 레드 #D7261E**(강조 최대 2회)
...
{content}
---
위 자료로 Open ALM 양식 본문 {body_n}장을 위 형식의 HTML 문서 하나로 출력하세요.
표지·제목 띠·로고·워터마크·저작권은 만들지 말고, 각 <section class="corporate-body" data-title> 의
본문 콘텐츠({bw}×{bh}px)만 디자인하세요. <head> 에 <title> 로 발표 전체 제목을 넣으세요."""
    return sys_prompt, user_prompt


# ============================================================
# Open ALM 양식 (Qwen 자동) — 디자인은 서버 §14 템플릿, 내용만 Qwen JSON
# ============================================================
# Qwen 은 레이아웃을 직접 그리지 않는다. 슬라이드마다 (1) 패턴을 고르고 (2) 그 패턴이
# 요구하는 필드만 JSON 으로 채운다. 실제 §14 HTML 은 design/corporate_templates.py 가 만든다.

_QWEN_PATTERNS_GUIDE = """\
이 양식은 Open ALM **사양 비교 / 공용화 현황** 문서입니다. 슬라이드 패턴(pattern)을 내용에 맞게 고르세요:
...
def build_corporate_qwen_outline_messages(
    content: str,
    n_body: int,
    language: str,
    tone: str,
    ...
    """Open ALM(Qwen 자동) 1단계 — 덱 아웃라인(제목 + 슬라이드별 pattern/brief) JSON 메시지."""
    sys = f"""당신은 Open ALM(OPEN ALM) 경영 보고서를 기획하는 컨설턴트입니다.
...
def build_corporate_qwen_fill_messages(
    deck_title: str,
    slide_index: int,
    slide_total: int,
    slide_title: str,
    ...
    """Open ALM(Qwen 자동) 2단계 — 한 슬라이드의 내용을 패턴 스키마에 맞춰 JSON 으로 채운다."""
    pattern = pattern if pattern in _QWEN_FILL_SCHEMAS else "bullets"
    schema = _QWEN_FILL_SCHEMAS[pattern]
    sys = f"""당신은 Open ALM(OPEN ALM) 보고서의 한 슬라이드 **내용을 채우는** 작성자입니다.
...
# ── 세미나 참석 보고 양식 (corporate-seminar) — 단일 본문 ───────────
...
def build_corporate_seminar_fill_messages(
    content: str,
    language: str,
    tone: str,
    instructions: str | None,
) -> list[dict]:
    """세미나 참석 보고(corporate-seminar) — 단일 본문 페이지 내용을 JSON 으로 채운다."""
    sys = f"""당신은 Open ALM(OPEN ALM) **세미나/출장 참석 보고서**의 본문을 작성하는 작성자입니다.
디자인·HTML·좌표는 만들지 마세요 — 아래 JSON 스키마의 필드만 정확히 채웁니다(서버가 디자인함).
...
_MEETING_FILL_SCHEMA = """\
{
  ...
  "team": "주관팀(예: AI TFT)",
  "author": "작성자(예: 최무겸 대리)",
  "place": "장소(예: Open ALM 연구1동 201호)",
  "date": "26.06.18 (목)",
  "time": "15:00~16:00",
  "attendees": [
    {"org": "Open ALM", "members": "이기정 상무, 남강우 이사, 류성훈 과장"},
    {"org": "Open ALM전자", "members": "박창화 이사, 강신덕 책임"}
  ],
...
def build_corporate_meeting_fill_messages(
    content: str,
    language: str,
    tone: str,
    instructions: str | None,
) -> list[dict]:
    """회의록(corporate-meeting) — 회의 메모/문서를 Open ALM 회의록 양식 JSON 으로 정리."""
    sys = f"""당신은 Open ALM(OPEN ALM) **회의록**을 정리하는 작성자입니다.
...
{content}
---
위 내용을 Open ALM 회의록 양식 JSON 으로 정리하세요. 결정 사항과 Action Item 을 명확히 구분해 담으세요."""
    return [{"role": "system", "content": sys}, {"role": "user", "content": user}]
...
def build_corporate_seminar_trip_messages(
    content: str,
    language: str,
    tone: str,
    instructions: str | None,
    ...
    sys = f"""당신은 Open ALM(OPEN ALM) **출장(세미나 참석) 일정표**를 작성하는 작성자입니다.
...
# ── 교육 보고서 양식 (corporate-education) — 단일 본문 표 ───────────
...
def build_corporate_education_fill_messages(
    content: str,
    language: str,
    tone: str,
    instructions: str | None,
) -> list[dict]:
    """교육 보고서(corporate-education) — 단일 본문 표(구분/내용/비고)를 JSON 으로 채운다."""
    sys = f"""당신은 Open ALM(OPEN ALM) **교육 참가 보고서**의 본문을 작성하는 작성자입니다.
...
def build_education_result_messages(
    ...
    sys = f"""당신은 Open ALM(OPEN ALM) **교육 결과 보고서**의 본문을 작성하는 작성자입니다.
...
- lines 는 해당 소항목의 설명(각 줄 한 문장 정도, 2~6줄). Open ALM 결과보고서 양식대로 **줄 앞 마커로 계층**을 표현하세요:
...
def _build_house_messages(
    ...
    """Open ALM 하우스 스타일(진행보고) 블록 JSON 생성 메시지.
    ...
    sys_prompt = f"""당신은 Open ALM AI TFT 의 사내 진행보고 PPT 자동 작성 어시스턴트입니다.
주어진 주제·자료를 바탕으로 "Open ALM 사내 진행보고 하우스 스타일"(연파랑 헤더 전체격자표 + 파란 강조 +
...
def build_house_enrich_messages(
    ...
    sys_prompt = f"""당신은 Open ALM 사내 진행보고 "하우스 스타일" PPT 의 **내용 보강** 편집자입니다.
...
def build_house_fill_messages(
    ...
    sys_prompt = f"""당신은 Open ALM 사내 진행보고 "하우스 스타일" PPT 슬라이드의 **내용 채우기** 편집자입니다.
...
def build_messages(
    ...
    if family == "corporate-v2":
        ...
        family_intro = "Open ALM 표준 양식 v2 (16:9, corporate-blue/red, Pretendard)"
        body_rule = (
            "- 모든 본문 슬라이드는 TAKEAWAY (OPEN ALM 시사점) 필드를 반드시 채우세요. "
        ...
        family_intro = f"Open ALM {fam['name']} (A4 가로, corporate-blue/red, Pretendard)"
        body_rule = (
    ...
    sys_prompt = f"""당신은 Open ALM AI TFT의 보고서 자동 작성 어시스턴트입니다.
...
def compute_slide_count(
    ...
    *,
    ...
    """corporate-v2 본문 슬라이드 수 자동 산정 (콘텐츠 길이 기반).
...
__all__ = [
    ...
    "build_corporate_free_prompt",
    "build_corporate_qwen_outline_messages",
    "build_corporate_qwen_fill_messages",
    "build_corporate_seminar_fill_messages",
    "build_corporate_seminar_trip_messages",
    "build_corporate_education_fill_messages",
