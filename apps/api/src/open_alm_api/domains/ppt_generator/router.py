...
from open_alm_api.domains.conversations.app_scope_adapters import PPT_JOB_SCOPE_REF
from open_alm_api.domains.ppt_generator.app_catalog import PPT_ASSISTANT_WORKSPACE_APP
from open_alm_api.domains.ppt_generator import service
from open_alm_api.domains.ppt_generator.families import FAMILIES, resolve_family
from open_alm_api.domains.ppt_generator.source_images import (
    extract_source_images as source_image_extract,
)
from open_alm_api.domains.ppt_generator.source_images import (
    render_schedule_images as schedule_image_render,
)
from open_alm_api.domains.ppt_generator.models import (
    PptJob,
    PptTemplatePreviewImage,
    utcnow_naive,
)
from open_alm_api.domains.ppt_generator.schemas import (
    CancelResponse,
    ChatEditRequest,
    ChatEditResponse,
    FamilyInfo,
...
async def generate(
    ...
    if fam.get("design") in ("corporate-seminar", "corporate-education"):
        try:
    ...
    if fam.get("design") == "corporate-education":
...
def download(
    ...
    filename = quote("Open ALM_PPT.pptx")
    return Response(
        content=data,
