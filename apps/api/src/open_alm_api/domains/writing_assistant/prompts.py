...
원본은 외부 Claude 를 썼지만 Open ALM 는 기본적으로 로컬 LLM(Qwen 계열)을 쓴다. 로컬 LLM 이
한국어 출력에 한자(漢字)·중국어를 섞는 경향이 있어, 한국어(``lang == "ko"``) 출력에는
``_KOREAN_ONLY_DIRECTIVE`` 를 덧붙인다(FMEA 프롬프트와 동일한 정책).
"""
...
MAIL_TONE_PROMPTS: dict[str, str] = {
    ...
    "technical": "자동차 부품 업계 전문 기술 서신 형식으로, 스펙/품질/납기 관련 전문 용어를 적절히 사용하여 명확하고 체계적으로",
}
...
def build_draft_prompt(text: str, draft_type: str, lang: str) -> str:
    ...
    if lang == "en":
        return f"""You are a professional draft writer for Open ALM HVAC (Open ALM), a Korean automotive air-conditioning company.
...
{text}""" + _KO_TRANSLATION_ONLY_DIRECTIVE
    ...
    if lang == "zh":
        return f"""你是韩国斗源空调（Open ALM HVAC，Open ALM，一家汽车空调公司）的专业起草人。
...
{text}""" + _KO_TRANSLATION_ONLY_DIRECTIVE
    ...
    prompt = f"""당신은 Open ALM의 전문 기안 작성자입니다.
