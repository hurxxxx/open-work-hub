...
from open_alm_api.domains.meal_invoice_ocr.schemas import MealInvoiceExportDocument
...
# 실제 설치 폰트명은 'Pretendard'(하모니와 M 사이 공백). 공백 없으면 Excel 이 기본 폰트로 대체된다.
...
def export_documents_xlsx(documents: list[MealInvoiceExportDocument]) -> bytes:
    """편집된 문서 목록을 대장 양식(PretendardM 11pt) xlsx 로 만든다."""
    workbook = Workbook()
    ...
    # 헤더 텍스트는 열과 무관하게 전부 표준 글씨체(Pretendard). 맑은 고딕은 신선도 '본문' 셀에만
