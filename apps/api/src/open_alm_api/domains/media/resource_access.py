...
from open_alm_api.domains.docs.models import DocMeetingAccess, NativeDocPage, NativeDocUserShare
from open_alm_api.domains.media.models import MediaFile
from open_alm_api.domains.media.service import MEDIA_ID_PATTERN
...
MEDIA_RESOURCE_PPT_TEMPLATE_PREVIEW = "ppt_template_preview"
SUPPORTED_MEDIA_LINK_RESOURCE_TYPES = frozenset(
    {
        ...
        MEDIA_RESOURCE_PPT_TEMPLATE_PREVIEW,
    }
)
...
def can_resolve_media(db: Session, user: User, media: MediaFile) -> bool:
    ...
    if media.resource_type == MEDIA_RESOURCE_PPT_TEMPLATE_PREVIEW:
        return _can_access_ppt_template_preview(db, user, media.resource_id)
...
def ensure_media_link_resource_access(
    ...
    if resource_type == MEDIA_RESOURCE_PPT_TEMPLATE_PREVIEW:
        _ensure_ppt_template_preview_access(db, user, resource_id)
        return
...
def _can_access_ppt_template_preview(
    db: Session,
    user: User,
    preview_id: str | None,
) -> bool:
    ...
    from open_alm_api.domains.ppt_generator.models import PptTemplatePreviewImage

    preview = db.scalar(
        select(PptTemplatePreviewImage).where(
            PptTemplatePreviewImage.id == preview_id,
    ...
    if is_platform_admin_user(user, db):
        return True
    return resolve_workspace_role(db, user, preview.workspace_id) is not None


def _ensure_ppt_template_preview_access(db: Session, user: User, preview_id: str) -> None:
    if _can_access_ppt_template_preview(db, user, preview_id):
        return
    raise localized_http_exception(
        status_code=403,
...
def _can_access_community_post_resource(
    ...
    from open_alm_api.domains.community.models import CommunityPost
...
def _can_access_community_comment_resource(
    ...
    from open_alm_api.domains.community.models import CommunityComment
