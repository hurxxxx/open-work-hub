...
from open_alm_api.domains.auth.app_bar_preferences import serialize_app_bar_layout
...
from open_alm_api.domains.auth.workspace_bootstrap_projection import (
    project_workspace_bootstrap_apps,
)
from open_alm_api.domains.auth.security import derive_login_id_from_email, hash_password, new_id
...
DEV_LOGIN_PASSWORD = "Open ALM!dev1234"
...
def _ensure_seed_org_units(db: Session) -> dict[str, OrgUnit]:
    existing = {org.slug: org for org in db.scalars(select(OrgUnit)).all()}
    ensured: dict[str, OrgUnit] = {}
    for definition in DEFAULT_ORG_UNIT_SEEDS:
        org = existing.get(definition["slug"])
        if org is None:
            org = OrgUnit(
                id=new_id(),
            ...
            db.add(org)
            db.flush()
        org.hr_source_system = None
        org.hr_domain_num = None
        org.hr_depart_num = None
        org.hr_org_code = None
        org.hr_parent_org_code = None
        org.hr_org_level = None
        org.hr_org_order = None
        org.hr_last_synced_at = None
        parent_slug = definition["parent_slug"]
        parent = ensured.get(parent_slug) if parent_slug else None
        org.parent_id = parent.id if parent else None
        org.active = True
        ensured[org.slug] = org
        existing[org.slug] = org

    return ensured


def _ensure_root_org(db: Session) -> OrgUnit:
    root_org = _ensure_seed_org_units(db).get("hq")
    if root_org is None:
        raise RuntimeError("Root org unit seed is missing.")
    return root_org
