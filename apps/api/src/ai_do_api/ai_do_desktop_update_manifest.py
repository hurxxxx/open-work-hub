...
AI_DO_DESKTOP_UPDATE_MANIFEST_PATH = (
    WORKSPACE_ROOT / "packages" / "contracts" / "ai-do-desktop-update-feed.manifest.json"
)
...
class DesktopUpdateManifestProjection:
    ...
    def platform_config(self, platform: str) -> DesktopUpdatePlatformManifest:
        ...
        if config is None:
            raise KeyError(f"Unsupported AI-DO desktop update platform: {platform}")
        return config
...
@lru_cache(maxsize=1)
def ai_do_desktop_update_manifest() -> dict[str, Any]:
    return validate_ai_do_desktop_update_manifest(
        json.loads(AI_DO_DESKTOP_UPDATE_MANIFEST_PATH.read_text(encoding="utf-8"))
    )


@lru_cache(maxsize=1)
def ai_do_desktop_update_manifest_projection() -> DesktopUpdateManifestProjection:
    return project_ai_do_desktop_update_manifest(ai_do_desktop_update_manifest())


def validate_ai_do_desktop_update_manifest(value: object) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise AiDoDesktopUpdateManifestError("AI-DO desktop update manifest must be an object.")
    ...
    if not isinstance(platform_order, list) or not platform_order:
        raise AiDoDesktopUpdateManifestError("AI-DO desktop update manifest platformOrder is invalid.")
    if not isinstance(platforms, dict):
        raise AiDoDesktopUpdateManifestError("AI-DO desktop update manifest platforms is invalid.")

    seen: set[str] = set()
    for platform in platform_order:
        if not isinstance(platform, str) or not platform:
            raise AiDoDesktopUpdateManifestError("AI-DO desktop update platform name is invalid.")
        if platform in seen:
            raise AiDoDesktopUpdateManifestError(
                f"AI-DO desktop update platform is duplicated: {platform}."
            )
        seen.add(platform)
        config = platforms.get(platform)
        if not isinstance(config, dict):
            raise AiDoDesktopUpdateManifestError(
                f"AI-DO desktop update platform config is missing: {platform}."
            )
        _validate_platform_config(platform, config, served_artifact_content_types)
    return value


def project_ai_do_desktop_update_manifest(
...
def _validate_platform_config(
    ...
    _manifest_env_name(
        ...
        "installerUrlEnv",
        platform=platform,
        prefix="VITE_AI_DO_DESKTOP_INSTALLER_URL_",
    )
    ...
    if not isinstance(stable_copies, list) or not any(
        ...
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop installerStableCopy must reference a stable copy target for {platform}."
        )
...
def _validate_manifest_rules(
    ...
    *,
    ...
    if not isinstance(value, list):
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop {field} must be a list for {platform}."
        )
    for index, rule in enumerate(value):
        if not isinstance(rule, dict):
            raise AiDoDesktopUpdateManifestError(
                f"AI-DO desktop {field}[{index}] must be an object for {platform}."
            )
        has_template = "template" in rule
        has_prefix = "prefixTemplate" in rule
        if has_template == has_prefix:
            raise AiDoDesktopUpdateManifestError(
                f"AI-DO desktop {field}[{index}] must define one matcher for {platform}."
            )
        if has_template:
        ...
        if "suffix" in rule:
            ...
            if not suffix.startswith("."):
                raise AiDoDesktopUpdateManifestError(
                    f"AI-DO desktop suffix is invalid for {platform}: {suffix}."
                )
        if require_target:
...
def _manifest_string(config: Mapping[str, object], field: str, *, platform: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop {field} is invalid for {platform}."
        )
    return value
...
def _manifest_env_name(
    ...
    *,
    platform: str,
    prefix: str = "AI_DO_DESKTOP_",
) -> str:
    value = _manifest_string(config, field, platform=platform)
    if not value.startswith(prefix):
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop {field} is invalid for {platform}: {value}."
        )
    return value
...
def _manifest_relative_path(config: Mapping[str, object], field: str, *, platform: str) -> str:
    ...
    if "\\" in value or value.startswith("/") or value.startswith("~"):
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop {field} must be a relative POSIX path for {platform}."
        )
    parts = value.split("/")
...
def _manifest_feed_path_prefix(config: Mapping[str, object], field: str) -> str:
    ...
    if not isinstance(value, str) or not value.startswith("/") or value.startswith("//"):
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop {field} must be an absolute path prefix."
        )
    if "\\" in value or "?" in value or "#" in value or value.endswith("/"):
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop {field} must be a clean absolute path prefix."
        )
    parts = value.split("/")[1:]
...
def _manifest_suffix_list(value: object, field: str, platform: str) -> tuple[str, ...]:
    if not isinstance(value, list) or not value:
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop {field} must be a non-empty list for {platform}."
        )
    suffixes: list[str] = []
    seen: set[str] = set()
    for index, suffix in enumerate(value):
        if (
            ...
            raise AiDoDesktopUpdateManifestError(
                f"AI-DO desktop {field}[{index}] is invalid for {platform}."
            )
...
def _manifest_content_type_map(value: object, field: str) -> dict[str, str]:
    if not isinstance(value, dict):
        raise AiDoDesktopUpdateManifestError(f"AI-DO desktop {field} must be an object.")
    ...
    for suffix, content_type in value.items():
        if (
            ...
            raise AiDoDesktopUpdateManifestError(
                f"AI-DO desktop {field} suffix is invalid: {suffix}."
            )
        if (
            ...
            raise AiDoDesktopUpdateManifestError(
                f"AI-DO desktop {field} content type is invalid for {suffix}."
            )
...
def _validate_served_artifact_content_types(
    ...
    for suffix in suffixes:
        ...
        if not isinstance(suffix, str) or suffix not in served_artifact_content_types:
            raise AiDoDesktopUpdateManifestError(
                f"AI-DO desktop servedArtifactContentTypes is missing {suffix} for {platform}."
            )
...
def _assert_manifest_file_name(value: str, field: str, *, platform: str) -> None:
    if (
        ...
        raise AiDoDesktopUpdateManifestError(
            f"AI-DO desktop {field} must be a safe file name for {platform}."
        )


def ai_do_desktop_update_platforms() -> tuple[str, ...]:
    return ai_do_desktop_update_manifest_projection().platforms


def ai_do_desktop_update_platform_config(platform: str) -> DesktopUpdatePlatformManifest:
    return ai_do_desktop_update_manifest_projection().platform_config(platform)


def ai_do_desktop_update_metadata_files() -> dict[str, str]:
    return ai_do_desktop_update_manifest_projection().metadata_files


def ai_do_desktop_update_artifact_suffixes() -> dict[str, frozenset[str]]:
    return ai_do_desktop_update_manifest_projection().artifact_suffixes


def ai_do_desktop_update_attachment_suffixes() -> dict[str, frozenset[str]]:
    return ai_do_desktop_update_manifest_projection().attachment_suffixes


def ai_do_desktop_update_content_types() -> dict[str, str]:
    return dict(ai_do_desktop_update_manifest_projection().served_artifact_content_types)


def ai_do_desktop_update_feed_path_prefix() -> str:
    return ai_do_desktop_update_manifest_projection().feed_path_prefix


def ai_do_desktop_update_base_url_env_name() -> str:
    return ai_do_desktop_update_manifest_projection().update_base_url_env


def ai_do_desktop_trusted_update_origins_env_name() -> str:
    return ai_do_desktop_update_manifest_projection().trusted_update_origins_env


def ai_do_desktop_update_default_dir(platform: str) -> str:
    default_dir = ai_do_desktop_update_platform_config(platform).default_dir
    return str(WORKSPACE_ROOT / default_dir)


def ai_do_desktop_update_dir_env_name(platform: str) -> str:
    return ai_do_desktop_update_platform_config(platform).update_dir_env


def ai_do_desktop_update_dir_values(env: Mapping[str, str | None] | None = None) -> dict[str, str]:
    env_values = env or {}
    return {
        platform: _ai_do_desktop_update_dir_value(platform, env_values)
        for platform in ai_do_desktop_update_platforms()
    }


def _ai_do_desktop_update_dir_value(
    platform: str,
    env: Mapping[str, str | None],
) -> str:
    env_name = ai_do_desktop_update_dir_env_name(platform)
    configured = env.get(env_name)
    if configured is None:
        return ai_do_desktop_update_default_dir(platform)
...
def ai_do_desktop_installer_url_env_name(platform: str) -> str:
    return ai_do_desktop_update_platform_config(platform).installer_url_env


def ai_do_desktop_installer_stable_copy(platform: str) -> str:
    return ai_do_desktop_update_platform_config(platform).installer_stable_copy
