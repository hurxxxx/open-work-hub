...
from ai_do_api.domains.auth.models import OrgUnit, Team, User, Workspace
from ai_do_api.domains.auth.security import new_id
from ai_do_api.domains.files import service as files_service
from ai_do_api.domains.files import storage_adapter as file_storage
from ai_do_api.domains.files.archive_planner import safe_filename
from ai_do_api.domains.files.external_access import is_current_platform_admin
...
from ai_do_api.domains.files.rag_sync import enqueue_file_retrieval_sync
from ai_do_api.domains.rag.contracts import RagSyncOperation
from ai_do_api.domains.retrieval.models import RetrievalPartition, RetrievalPartitionState


ExternalFileGrantType = Literal["company", "workspace", "user", "org_unit", "team"]
EXTERNAL_FILE_GRANT_TYPES = frozenset({"company", "workspace", "user", "org_unit", "team"})
...
def _resolve_grants(
    ...
    *,
    ...
    if targets := targets_by_type.get("org_unit"):
        resolved_by_type["org_unit"] = set(
            db.scalars(
                select(OrgUnit.id).where(OrgUnit.id.in_(targets), OrgUnit.active.is_(True))
            ).all()
        )
