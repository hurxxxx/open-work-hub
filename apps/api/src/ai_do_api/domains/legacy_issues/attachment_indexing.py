...
from ai_do_api.core.llm import LlmRuntimeError
...
from ai_do_api.domains.rag.providers.local_document_rendering import (
...
def _extract_with_vision_model(
    ...
    *,
    ...
    suffix = _attachment_suffix(filename=filename, content_type=content_type)
    try:
        with tempfile.TemporaryDirectory(prefix="ai-do-legacy-attachment-") as tmp:
            work_dir = Path(tmp)
...
def _celery_client() -> Celery:
    ...
    return create_fail_fast_celery_publisher(
        "ai_do_api_legacy_issue_attachment_index",
