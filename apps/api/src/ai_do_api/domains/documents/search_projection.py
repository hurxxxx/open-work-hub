...
class SearchDocumentsResponse(BaseModel):
    scenario_id: str = "documents-demo"
    query_profile: str = "fixture lexical demo"
...
def _matches_query(hit: SearchDocumentHit, terms: list[str]) -> bool:
    haystack = " ".join([hit.title, hit.summary, hit.citation]).lower()
...
def _build_grounded_answer(query: str, hits: list[SearchDocumentHit]) -> GroundedAnswer | None:
    ...
    return GroundedAnswer(
        ...
        key_points=[f"{hit.title}: {hit.summary}" for hit in selected_hits],
        citations=[
...
def build_search_documents_response(
    ...
    hits = [
        ...
        if _matches_filters(hit, payload.filters) and _matches_query(hit, terms)
    ]
