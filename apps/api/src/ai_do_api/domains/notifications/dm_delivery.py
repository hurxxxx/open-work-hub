...
from ai_do_api.domains.dm.models import DmConversation, DmMessage
from ai_do_api.domains.notifications.models import NotificationDmDelivery
from ai_do_api.domains.pms.models import Notification


AI_DO_BOT_USER_ID = "ai-do-notification-bot"
AI_DO_BOT_LOGIN_ID = "ai-do-bot"
AI_DO_BOT_EMAIL = "ai-do-bot@ai-do.local"
AI_DO_BOT_NAME = "AI-DO Bot"
...
def ensure_notification_bot_user(db: Session) -> User:
    bot = db.get(User, AI_DO_BOT_USER_ID)
    if bot is not None:
        _normalize_bot_user(bot)
        return bot

    bot = User(
        id=AI_DO_BOT_USER_ID,
        login_id=AI_DO_BOT_LOGIN_ID,
        email=AI_DO_BOT_EMAIL,
        full_name=AI_DO_BOT_NAME,
        display_name=AI_DO_BOT_NAME,
...
def _normalize_bot_user(bot: User) -> None:
    bot.login_id = AI_DO_BOT_LOGIN_ID
    bot.email = AI_DO_BOT_EMAIL
    bot.full_name = AI_DO_BOT_NAME
    bot.display_name = AI_DO_BOT_NAME
    bot.status = "system"
