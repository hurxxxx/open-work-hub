...
(doowon-v2 16:9 / a4-progress / a4-exec-kpi / a4-scorecard / a4-minutes 양식은
...
from ai_do_api.domains.ppt_generator.design.builders_a4 import SLIDE_H as SLIDE_H_A4
from ai_do_api.domains.ppt_generator.design.builders_a4 import SLIDE_W as SLIDE_W_A4
from ai_do_api.domains.ppt_generator.design.builders_brandlogy import (
...
from ai_do_api.domains.ppt_generator.design.builders_house import SLIDE_H as SLIDE_H_HOUSE
from ai_do_api.domains.ppt_generator.design.builders_house import SLIDE_W as SLIDE_W_HOUSE

_ASSETS_DIR = Path(__file__).parent / "design" / "assets"
LOGO_PATH_A4 = str(_ASSETS_DIR / "doowon_wordmark.png")
...
FAMILIES: dict[str, dict] = {
    # 두원 사내 진행보고 "하우스 스타일"(SPEC_house_style.md) 데이터 드리븐 렌더러.
    ...
    # ("자유 양식 (Brandlogy)", "두원 양식(자유 본문, doowon-free)", "사양 비교(doowon-qwen)"
    #  양식은 카탈로그에서 제거됨. 생성 분기·프롬프트·렌더러 코드는 세미나 등이 재사용하므로
    #  남겨둔다. 다시 노출하려면 아래 형태의 family 항목을 복원하면 된다.)
    # 세미나 참석 보고 — 두원 표지/본문 틀 + Qwen 자동채움. 본문 1장(정보헤더+주요내용표+소감).
    # 생성은 design=="doowon-seminar" 분기(단일 채움 콜).
    "doowon-seminar": {
        "name": "세미나 & 출장 보고서",
        "aspect": "A4",
        "slide_w": SLIDE_W_A4,
        ...
        "fixed_body_n": 2,
        "logo_path": LOGO_PATH_A4,
        "design": "doowon-seminar",
    },
    # 교육 참가 보고 — 두원 표지/본문 틀 + Qwen 자동채움. 본문 1장(구분/내용/비고 표).
    # 생성은 design=="doowon-education" 분기(단일 채움 콜).
    "doowon-education": {
        "name": "교육 참가 보고서",
        "aspect": "A4",
        "slide_w": SLIDE_W_A4,
        ...
        "fixed_body_n": 1,
        "logo_path": LOGO_PATH_A4,
        "design": "doowon-education",
    },
    # 회의록 — 두원 세로 A4 회의록 양식. 회의 메모·문서를 머리표와 회의내용으로 정리.
    # 생성은 design=="doowon-meeting" 분기(단일 채움 콜, 표지 없는 세로 1장 HTML).
    "doowon-meeting": {
        "name": "회의록",
        "aspect": "A4",
        "slide_w": SLIDE_W_A4,
        ...
        "fixed_body_n": 1,
        "logo_path": LOGO_PATH_A4,
        "design": "doowon-meeting",
    },
}

DEFAULT_FAMILY = "doowon-house"
