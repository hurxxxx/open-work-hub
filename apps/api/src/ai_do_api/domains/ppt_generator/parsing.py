...
from ai_do_api.domains.ppt_generator.families import DEFAULT_FAMILY, resolve_family
...
def parse_doowon_qwen_outline(
    text: str | None, n_body: int
) -> tuple[str, list[dict]]:
    """두원(Qwen 자동) 아웃라인 응답 → (덱 제목, [{title, pattern, brief}]).
    ...
    if isinstance(raw, list):
        for s in raw:
            ...
            if pattern not in _DOOWON_QWEN_PATTERNS:
                pattern = "bullets"
            slides.append({
...
def parse_doowon_qwen_slide(text: str | None, fallback: dict) -> dict:
    """두원(Qwen 자동) 슬라이드 채움 응답 → 슬라이드 dict. 실패 시 fallback(title/pattern) 사용."""
    try:
        obj = parse_json_object(text)
    except Exception:
        obj = {}
...
__all__ = [
    ...
    "parse_doowon_qwen_outline",
    "parse_doowon_qwen_slide",
