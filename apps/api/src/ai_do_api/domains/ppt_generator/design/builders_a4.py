# -*- coding: utf-8 -*-
"""
pptgen2_design_a4.py - 두원공조 A4 경영진 보고 양식 v3.5 (pptgen2 A4 패밀리)

캔버스: A4 landscape (27.517 × 19.05 cm)
폰트: 현대하모니 M (HDharmony M) — Arial Bold 예외 4곳
공통 chrome:
...
# 폰트 (§4)
# ============================================================
# PowerPoint(GDI) 등록명은 '현대하모니 M'(공백 포함). latin/ea/cs 모두 이 이름으로 통일해
# 폰트박스에 '현대하모니 M'으로 표시되게 한다. 굵기는 bold 속성으로 처리(별도 'B' 폰트명 미사용).
FONT_KR = '현대하모니 M'        # 현대하모니 M (PowerPoint 등록명)
FONT_KR_ALT = '현대하모니 M'    # ea(한글) typeface
FONT_KR_BOLD = '현대하모니 M'   # 굵은 텍스트도 동일 폰트명 + bold 속성
FONT_EN = 'Arial'
...
# ============================================================
COVER_WATERMARK_TEXT = 'DOOWON Climate Control  Confidential Documents'  # "Control" "Confidential" 사이 더블 스페이스
...
# 렌더 보정으로 나눈다. 네이티브 헤더(apply_body_chrome)와 HTML 헤더(doowon_frame)가
...
def apply_body_chrome(slide, header_title, logo_path=None, date_str=None, *,
    ...
    title_fixed_pt 를 주면 자동 축소 없이 그 크기로 제목을 고정한다(예: 두원 덱 24pt).
    """
    # 날짜는 항상 오늘 기준 (LLM 이 넣은 DATE 는 무시)
    date_str = datetime.now().strftime('%Y.%m.%d')

    # S1 — ▣ 제목 (현대하모니M Bold). 기본은 24pt 에서 길면 자동 축소하지만,
    ...
    # L3 — DOOWON 로고 (gap 안: x=22.50, h=0.40, w 자동, top=1.65)
    if logo_path:
        try:
...
def build_a4_cover_slide(prs, data, logo_path=None):
    ...
    data 필드:
      ...
      - AUTHOR: 작성자 (기본 '㈜두원공조 기술연구소 AI TFT')
    ...
    # 날짜는 항상 오늘 기준 (LLM 이 넣은 DATE 는 무시)
    date_str = datetime.now().strftime('%Y.%m.%d')
    author = data.get('AUTHOR', '㈜두원공조 기술연구소 AI TFT')
    ...
    # 3) 표지 제목 (현대하모니M Bold 24pt #333333) — 24pt 기본.
