"""두원 양식(사양 비교) — 슬라이드 JSON → 두원 사양비교/공용화 현황 본문 HTML 렌더러.

참고 정본: ``HTML - PPT 연동 확인.pptx`` (두원 자동차 쿨링모듈 사양비교/공용화 현황).
Qwen 35B 는 **내용(JSON)만** 만들고, 이 모듈이 그 JSON 을 두원 사양비교 문서 스타일의
본문 ``inner_html`` 로 결정론적으로 렌더한다. 결과는 두원 본문 칸(1244×754px)을 채우는
단일 루트 블록이며 ``doowon_frame.body_section_html`` 의 ``.dw-bodyzone`` 에 들어간다.
...
from ai_do_api.domains.ppt_generator.design.doowon_frame import BODY_H_PX, BODY_W_PX
...
# 폰트 스택 주의:
#  - 브라우저(DirectWrite)는 typographic family 이름 'HDharmony'/'현대하모니'(공백 없음)로만 인식.
#  - PowerPoint(GDI)는 등록명 '현대하모니 M'(공백 포함)으로 인식 → html_to_pptx 가 첫 토큰을 사용.
#  따라서 '현대하모니 M' 을 첫째(=pptx용)로, 그 뒤 'HDharmony'/'현대하모니'(=브라우저용)를 둔다.
FONT = "'현대하모니 M','HDharmony','현대하모니','Pretendard','Malgun Gothic',sans-serif"
...
# ── 회의록 (doowon-meeting) — A4 세로 단일 슬라이드, 자체완결형 HTML ──────────
MEETING_W_CM = 21.0
MEETING_H_CM = 29.7
...
def render_meeting_minutes_deck(slide: dict) -> str:
    ...
    foot = (
        ...
        f"Doowon Climate Control Co., Ltd. · This information is exclusive property of "
        f"Doowon Corporation.</div>"
    )
    title_head = (
        "<div style=\"font-family:'현대하모니 M','HDharmony','현대하모니',"
    ...
    css = (
        ...
        "body{font-family:'현대하모니 M','HDharmony','현대하모니','Pretendard',"
...
def render_section(slide: dict) -> dict:
    """슬라이드 JSON → doowon_frame 섹션 dict {title, subtitle, inner_html}."""
    return {
