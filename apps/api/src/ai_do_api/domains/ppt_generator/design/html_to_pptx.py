...
_BROWSER_ENV_VAR = "AI_DO_PPT_BROWSER_PATH"
_BROWSER_PATHS = (
    # Windows
...
class _Builder:
    ...
    def _style_run(self, run, it):
        ...
        face = _first_font_family(it.get("fontFamily")) or "현대하모니 M"
        f.name = face
...
def html_deck_to_editable_pptx_bytes(html: str, slide_w_px: int = 1280,
    ...
    slide_w_cm/slide_h_cm: 출력 pptx 캔버스 물리 크기(기본 27×16.75cm, 두원 양식은 27.517×19.05).
    """
...
def doowon_deck_to_pptx_bytes(
    ...
    *,
    ...
    """두원 양식 .pptx — 표지/본문 틀은 builders_a4 로 네이티브 작성하고, 본문 콘텐츠만
    ...
    from ai_do_api.domains.ppt_generator.design import builders_a4 as b4
    from ai_do_api.domains.ppt_generator.design import doowon_frame as df
    ...
    with tempfile.TemporaryDirectory(prefix="pptdoowon_") as directory:
