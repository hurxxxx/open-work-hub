"""두원공조 양식(chrome) HTML 합성기.

Claude 가 만든 "본문 콘텐츠"(틀 없는 자유 디자인)를 두원공조 표지/본문 chrome 으로 감싸
...
핵심 분리:
  ...
  - 이 모듈이 그 콘텐츠를 두원 표지 1장 + 본문 N장(각: 틀 + 본문 칸에 콘텐츠)으로 감싼다.
...
# ── §3 고정 문자열 (절대 변경 금지) ──────────────────────────
COVER_WATERMARK_TEXT = "DOOWON Climate Control  Confidential Documents"
...
_DEFAULT_AUTHOR = "㈜두원공조 기술연구소 AI TFT"
_DEFAULT_VERSION = "Ver 1"

_LOGO_PATH = Path(__file__).parent / "assets" / "doowon_wordmark.png"
...
body {{ font-family: '현대하모니 M','HDharmony','현대하모니','Pretendard','Malgun Gothic',sans-serif; }}
...
def _logo_img(left_px: int, top_px: int, height_px: int) -> str:
    uri = _logo_data_uri()
    if not uri:
        return ""
    return (
        f'<img class="dw-abs" src="{uri}" alt="DOOWON" '
        f'style="left:{left_px}px;top:{top_px}px;height:{height_px}px;width:auto;display:block;">'
    )
...
def cover_section_html(title: str, author: str, version: str, date_str: str) -> str:
    ...
    # width:auto → 우측 정렬 위해 right 로 배치
    uri = _logo_data_uri()
    if uri:
        right = round(0.30 * PXCM)
        parts.append(
            f'<img class="dw-abs" src="{uri}" alt="DOOWON" '
            f'style="right:{right}px;top:{logo_top}px;height:{logo_h}px;width:auto;display:block;">'
        )
...
def body_section_html(title: str, subtitle: str, inner_html: str, date_str: str) -> str:
    ...
    parts.append(
        ...
        f'transform:rotate(-30deg);">{_esc(DCC_WATERMARK_TEXT)}</div>'
    )
