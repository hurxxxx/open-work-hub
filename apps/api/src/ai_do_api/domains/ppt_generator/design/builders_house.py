# -*- coding: utf-8 -*-
"""두원 사내 진행보고 "하우스 스타일" 데이터 드리븐 렌더러 (python-pptx).
...
# §4.1 캔버스 & 좌표계
# ============================================================
# A4(가로) 용지 — 두원 a4/세미나 양식과 동일 치수(27.517 × 19.05cm).
SLIDE_W_CM = 27.517
SLIDE_H_CM = 19.05
...
# §4.3 타이포그래피
# ============================================================
# SPEC §4.3 은 Pretendard 를 지정하나, 기존 PPT 시스템 전체가 사내 표준 폰트 '현대하모니 M'
...
def _draw_frame(slide, data: dict, logo_path=None) -> None:
    ...
    ▣ 제목(좌상) + 우상단 날짜 + 가로선 사이 DOOWON 로고 + 사선 DCC 워터마크 +
    하단 영문 저작권 + Confidential 배지. (하우스 고유의 'N. 제목 [태그]'·'N/total' 푸터는 제거)
    """
    from ai_do_api.domains.ppt_generator.design.builders_a4 import apply_body_chrome
...
# 글자 폭 추정 계수(em). 현대하모니 M 한글 실측 ≈ 1.0em 이라 1.1 로 약간만 여유(실제보다 살짝
...
# 줄바꿈·열폭·side-by-side 판정 공용 글자폭 — 현대하모니 M **COM 실측**: 한글 1.03em, ASCII 0.51~0.59.
...
_WRAP_KO_EM = 1.03
_WRAP_ASCII_EM = 0.6
# 공백·문장부호는 글자보다 훨씬 좁다(현대하모니 M 실측: 공백 0.30 / 쉼표·콜론 0.31 / 괄호 0.34em).
...
_CELL_VPAD = 0.24
# 12pt 셀 한 줄의 실제 높이 근사(현대하모니 M). 실측(COM) 기준 상향 — set≥실제여야 PPT 가 행을
