# -*- coding: utf-8 -*-
"""
pptgen2_design.py — Doowon PPT Design System v2

두원공조 AI TFT 전용 16:9 프레젠테이션 빌더.
캔버스 17.778" × 10.0" (1920×1080 px 등비), 단일 폰트 "현대하모니 M",
doowon-blue 메인 + doowon-red 액센트.

본 모듈은 두원공조 사내 표준 PPT 양식을 정확히 재현하기 위한 사양이며,
...
# ============================================================
# 컬러 토큰 — Doowon v2 명세 1:1
# ============================================================
COLORS = {
    # 두원 브랜드 코어
    'doowon_blue':         RGBColor(0x1E, 0x2A, 0x8E),
    'doowon_blue_deep':    RGBColor(0x14, 0x1C, 0x6B),
    'doowon_blue_soft':    RGBColor(0x3B, 0x4A, 0xB8),
    'doowon_red':          RGBColor(0xE2, 0x23, 0x1A),
    'doowon_red_soft':     RGBColor(0xF2, 0x6B, 0x65),
    # 중립
...
# 폰트
# ============================================================
FONT_KR = '현대하모니 M'   # 단일 폰트 (사용자 PC에 설치 필요)
...
def add_body_header(slide, header_text='{{HEADER}}', subtitle=None,
    ...
    # 횡선 분리선 우 (DOOWON 로고 + 날짜 영역만) — px(1767, 101, 153, 4)
...
def add_takeaway_bar(slide, label, body_text):
    """DOOWON 시사점 takeaway 바 — 모든 본문 슬라이드 하단 (v2 §7-1)."""
    # px(64, 855, 1792, 100)
    bar_left = px(64)
    bar_top = px(855)
    bar_w = px(1792)
    ...
    # 좌측 수직 룰 4px doowon-blue
    _add_left_rule(slide, bar_left, bar_top, bar_h, COLORS['doowon_blue'], width_pt=4)
    # 라벨 (위)
    _add_textbox(slide, bar_left + px(20), bar_top + px(6),
                 bar_w - px(20), px(22),
                 text='DOOWON 시사점',
                 font=FONT_KR, size_pt=11, bold=True,
                 color=COLORS['doowon_blue'],
                 align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP,
...
def build_cover_slide(prs, data, logo_path=None):
    ...
    _add_textbox(slide, px(80), px(1030), px(1760), px(28),
                 text='Doowon Climate Control Co., Ltd. : This information is exclusive property of Doowon Climate Control Co., Ltd.',
...
def build_2col_comparison_slide(prs, data, logo_path=None,
    ...
    _add_outlined_rect(slide, right_x, p2_top, right_w, panel_h,
                       line_color=COLORS['doowon_blue_soft'], line_width_pt=1.5,
    ...
    _add_textbox(slide, right_x + px(24), p2_top + px(20), right_w - px(48), px(22),
                 ...
                 font=FONT_KR, size_pt=11, bold=True, color=COLORS['doowon_blue'],
                 align=PP_ALIGN.LEFT, letter_spacing_pt=2.0)
    # 큰 숫자/값 — doowon-blue 32pt bold
    _add_textbox(slide, right_x + px(24), p2_top + px(60), right_w - px(48), px(80),
                 text=data.get('PANEL2_VALUE', '{{VALUE}}'),
                 font=FONT_KR, size_pt=32, bold=True, color=COLORS['doowon_blue'],
                 align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP,
                 letter_spacing_pt=-1.0)
    ...
    # 시사점 바
    add_takeaway_bar(slide, 'DOOWON 시사점',
...
def build_benchmark_tiles_slide(prs, data, logo_path=None,
    ...
    _add_outlined_rect(slide, bench_x, bench_y, bench_w, bench_h,
                       line_color=COLORS['doowon_blue_soft'], line_width_pt=1,
                       fill_color=COLORS['surface_blue_tint'])
    _add_textbox(slide, bench_x + px(24), bench_y + px(18), px(200), px(20),
                 text='BENCHMARK CASE',
                 font=FONT_KR, size_pt=10, bold=True, color=COLORS['doowon_red'],
                 align=PP_ALIGN.LEFT, letter_spacing_pt=2.0)
    _add_textbox(slide, bench_x + px(24), bench_y + px(42),
                 bench_w - px(48), px(36),
                 text=data.get('BENCH_TITLE', '{{BENCH_TITLE}}'),
                 font=FONT_KR, size_pt=18, bold=True, color=COLORS['doowon_blue'],
                 align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP)
    ...
    for i, t in enumerate(tiles[:4]):
        ...
        _add_outlined_rect(slide, x, tile_y, tile_w, tile_h,
                           line_color=COLORS['hairline'], line_width_pt=1,
                           fill_color=COLORS['canvas'])
        # top accent doowon-blue 3px
        _add_filled_rect(slide, x, tile_y, tile_w, Pt(3),
                         fill_color=COLORS['doowon_blue'])
        # KPI 큰 숫자
    ...
    for r_idx, row in enumerate(rows[:8]):
        ...
        for c in range(n_cols):
            ...
            _add_textbox(slide, table_x + col_w * c + px(12), y,
                         ...
                         color=(COLORS['doowon_blue'] if is_rec and c == 0
...
def build_4step_framework_slide(prs, data, logo_path=None,
    ...
    data 필드:
      ...
      - CONCLUSION_TITLE, CONCLUSION_BODY: 하단 결론 블록 (doowon-blue 배경)
    """
    blank = prs.slide_layouts[6]
    slide = prs.slides.add_slide(blank)
    add_body_header(slide,
    ...
    badge_colors = [COLORS['steel'], COLORS['danger_text'],
                    COLORS['doowon_blue'], COLORS['success_text']]
