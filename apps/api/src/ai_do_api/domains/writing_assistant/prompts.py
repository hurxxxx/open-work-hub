...
원본은 외부 Claude 를 썼지만 AI-DO 는 기본적으로 로컬 LLM(Qwen 계열)을 쓴다. 로컬 LLM 이
...
def build_draft_prompt(text: str, draft_type: str, lang: str) -> str:
    ...
    if lang == "en":
        return f"""You are a professional draft writer for Doowon HVAC (두원공조), a Korean automotive air-conditioning company.
...
{text}""" + _KO_TRANSLATION_ONLY_DIRECTIVE
    ...
    if lang == "zh":
        return f"""你是韩国斗源空调（Doowon HVAC，두원공조，一家汽车空调公司）的专业起草人。
...
{text}""" + _KO_TRANSLATION_ONLY_DIRECTIVE
    ...
    prompt = f"""당신은 두원공조의 전문 기안 작성자입니다.
