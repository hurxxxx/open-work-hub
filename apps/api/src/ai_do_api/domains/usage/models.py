...
class UsageTargetOrgUnit(Base):
    __tablename__ = "usage_target_org_units"
    __table_args__ = (
        Index("ix_usage_target_org_units_created_at", "created_at"),
        Index("ix_usage_target_org_units_created_by", "created_by_user_id"),
    )

    org_unit_id: Mapped[str] = mapped_column(ForeignKey("org_units.id"), primary_key=True)
    created_by_user_id: Mapped[str | None] = mapped_column(
        ForeignKey("users.id"),
        nullable=True,
    )
