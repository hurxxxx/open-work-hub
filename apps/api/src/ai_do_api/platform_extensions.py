...
from ai_do_api.core.asr import ensure_default_asr_backends_registered
...
from ai_do_api.domains.ai.local_gateway_tool_catalog import (
...
from ai_do_api.domains.ai.runtime.agent_catalog import default_agent_definitions
...
from ai_do_api.domains.rag.provider_factory import get_rag_provider_registry
...
from ai_do_api.domains.search.backend_factory import (
...
from ai_do_api.domains.search.entity_adapter_registry import search_entity_adapters
from ai_do_api.domains.search.entity_registry import search_entity_descriptors
from ai_do_api.domains.search.hook_registry import (
    get_search_index_hook_registration,
    search_index_hooks_registered,
)
from ai_do_api.domains.search.index_gateway import keyword_acl_query_field_names
from ai_do_api.domains.search.projection_registry import get_search_projection_adapters
...
from ai_do_api.domains.source_access.registry import get_source_access_adapters
...
def _validate_external_provider_settings(
    ...
    if not llm_allowed:
        problems.append(
            "AI_DO_LLM_EXTERNAL_ALLOWED_PROVIDERS must include at least one "
    ...
    if ai_unknown:
        problems.append(
            "AI_DO_AI_ALLOWED_EXTERNAL_PROVIDERS contains unsupported external "
            "provider(s): " + ", ".join(ai_unknown)
        )
    if not ai_allowed:
        problems.append(
            "AI_DO_AI_ALLOWED_EXTERNAL_PROVIDERS must include at least one "
            "supported external provider"
        )
    if _ai_external_llm_provider_settings_required(settings):
        if ai_default_llm is None:
            ...
            problems.append(
                ...
                f"AI_DO_AI_ALLOWED_EXTERNAL_PROVIDERS: {ai_default_llm}"
            )
    if ai_default is None:
        problems.append(
            "AI_DO_AI_DEFAULT_EXTERNAL_SEARCH_PROVIDER contains unsupported "
            f"external provider: {settings.ai_default_external_search_provider}"
        )
    elif ai_allowed and ai_default not in ai_allowed:
        problems.append(
            "AI_DO_AI_DEFAULT_EXTERNAL_SEARCH_PROVIDER must be listed in "
            f"AI_DO_AI_ALLOWED_EXTERNAL_PROVIDERS: {ai_default}"
        )
    return problems
...
def _callable_belongs_to_backend_domain(value: Any, backend_domain: str) -> bool:
    ...
    domain_module = f"ai_do_api.domains.{backend_domain}"
...
class _KeywordAclShapePolicy:
    ...
    user = SimpleNamespace(id="__user__", primary_org_unit_id=None)
