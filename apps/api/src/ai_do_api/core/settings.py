...
class Settings(BaseSettings):
    ...
    instance_id: str = Field(
        default="api",
        validation_alias="AI_DO_API_INSTANCE_ID",
    )
    api_prefix: str = Field(
        default="/api/v1",
        validation_alias="AI_DO_API_PREFIX",
    )
    ai_do_desktop_update_dirs: dict[str, str] = Field(default_factory=dict)
    ...
    drawio_bind_host: str = Field(
        default="127.0.0.1",
        validation_alias="AI_DO_DRAWIO_BIND_HOST",
    )
    drawio_image_tag: str = Field(
        default="30.2.5",
        validation_alias="AI_DO_DRAWIO_IMAGE_TAG",
    )
    drawio_port: int = Field(
        default=18082,
        ge=1,
        le=65535,
        validation_alias="AI_DO_DRAWIO_PORT",
    )
    drawio_server_url: str = Field(
        default="",
        validation_alias="AI_DO_DRAWIO_SERVER_URL",
    )
    dm_attachment_signing_key: str = Field(
        default=DEFAULT_DM_ATTACHMENT_SIGNING_KEY,
        validation_alias="AI_DO_DM_ATTACHMENT_SIGNING_KEY",
    )
    worker_broker_url: str = Field(
        default="redis://127.0.0.1:6379/0",
        validation_alias="AI_DO_WORKER_BROKER_URL",
    )
    worker_result_backend: str = Field(
        default="redis://127.0.0.1:6379/1",
        validation_alias="AI_DO_WORKER_RESULT_BACKEND",
    )
    mail_credential_encryption_key: str = Field(
        default="",
        validation_alias="AI_DO_MAIL_CREDENTIAL_ENCRYPTION_KEY",
    )
    ai_model_credential_encryption_key: str = Field(
        default="",
        validation_alias="AI_DO_AI_MODEL_CREDENTIAL_ENCRYPTION_KEY",
        repr=False,
    )
    mail_allowed_private_hosts: str = Field(
        default="",
        validation_alias="AI_DO_MAIL_ALLOWED_PRIVATE_HOSTS",
    )
    mail_allow_insecure_transport: bool = Field(
        default=False,
        validation_alias="AI_DO_MAIL_ALLOW_INSECURE_TRANSPORT",
    )
    mail_sync_max_messages: int = Field(
        default=50,
        ge=1,
        le=500,
        validation_alias="AI_DO_MAIL_SYNC_MAX_MESSAGES",
    )
    mail_sync_max_attempts: int = Field(
        default=3,
        ge=1,
        le=20,
        validation_alias="AI_DO_MAIL_SYNC_MAX_ATTEMPTS",
    )
    mail_sync_retry_backoff_seconds: int = Field(
        default=60,
        ge=1,
        le=3600,
        validation_alias="AI_DO_MAIL_SYNC_RETRY_BACKOFF_SECONDS",
    )
    ...
    mail_sync_dispatch_visibility_seconds: int = Field(
        ...
        ge=30,
        le=3600,
        validation_alias="AI_DO_MAIL_SYNC_DISPATCH_VISIBILITY_SECONDS",
    )
    hr_groupware_auth_url: str = Field(
        default="",
        validation_alias="AI_DO_HR_GROUPWARE_AUTH_URL",
    )
    hr_groupware_auth_timeout_seconds: float = Field(
        default=5.0,
        gt=0,
        le=30,
        validation_alias="AI_DO_HR_GROUPWARE_AUTH_TIMEOUT_SECONDS",
    )
    ...
    news_collect_queue_override: str = Field(
        default="",
        validation_alias="AI_DO_NEWS_COLLECT_QUEUE",
    )
    ...
    news_stale_after_hours: int = Field(
        ...
        ge=1,
        le=168,
        validation_alias="AI_DO_NEWS_STALE_AFTER_HOURS",
    )
    ...
    industry_report_upload_max_bytes: int = Field(
        ...
        ge=1024,
        le=512 * 1024 * 1024,
        validation_alias="AI_DO_INDUSTRY_REPORT_UPLOAD_MAX_BYTES",
    )
    ...
    collab_redis_url: str = Field(
        default="redis://127.0.0.1:6379/0",
        validation_alias="AI_DO_API_COLLAB_REDIS_URL",
    )
    realtime_redis_url: str = Field(
        default="redis://127.0.0.1:6379/0",
        validation_alias="AI_DO_API_REALTIME_REDIS_URL",
    )
    collab_acl_recheck_seconds: int = Field(
        default=15,
        ge=5,
        le=300,
        validation_alias="AI_DO_API_COLLAB_ACL_RECHECK_SECONDS",
    )
    collab_snapshot_debounce_ms: int = Field(
        default=2000,
        ge=250,
        le=30000,
        validation_alias="AI_DO_API_COLLAB_SNAPSHOT_DEBOUNCE_MS",
    )
    collab_max_user_room_connections: int = Field(
        default=2,
        ge=1,
        le=50,
        validation_alias="AI_DO_API_COLLAB_MAX_USER_ROOM_CONNECTIONS",
    )
    collab_max_room_clients: int = Field(
        default=100,
        ge=1,
        le=1000,
        validation_alias="AI_DO_API_COLLAB_MAX_ROOM_CLIENTS",
    )
    collab_cleanup_timeout_seconds: float = Field(
        default=5.0,
        ge=0.1,
        le=60.0,
        validation_alias="AI_DO_API_COLLAB_CLEANUP_TIMEOUT_SECONDS",
    )
    db_pool_size: int = Field(
        default=32,
        ge=1,
        le=100,
        validation_alias="AI_DO_API_DB_POOL_SIZE",
    )
    db_max_overflow: int = Field(
        default=64,
        ge=0,
        le=100,
        validation_alias="AI_DO_API_DB_MAX_OVERFLOW",
    )
    db_pool_timeout: int = Field(
        default=45,
        ge=1,
        le=300,
        validation_alias="AI_DO_API_DB_POOL_TIMEOUT",
    )
    recording_spool_dir: str = Field(
        default=str(WORKSPACE_ROOT / ".local-recording-spool"),
        validation_alias="AI_DO_API_RECORDING_SPOOL_DIR",
    )
    serve_frontend: bool = Field(
        default=False,
        validation_alias="AI_DO_API_SERVE_FRONTEND",
    )
    frontend_dist_dir: str = Field(
        default=DEFAULT_FRONTEND_DIST_DIR,
        validation_alias="AI_DO_API_FRONTEND_DIST_DIR",
    )
    recording_max_size_bytes: int = Field(
        default=1024 * 1024 * 1024,
        ge=1024 * 1024,
        validation_alias="AI_DO_API_RECORDING_MAX_SIZE_BYTES",
    )
    recording_staging_retention_hours: int = Field(
        default=24 * 7,
        ge=1,
        le=24 * 30,
        validation_alias="AI_DO_API_RECORDING_STAGING_RETENTION_HOURS",
    )
    video_chat_enabled: bool = Field(
        default=True,
        validation_alias="AI_DO_API_VIDEO_CHAT_ENABLED",
    )
    livekit_url: str = Field(
        default="ws://127.0.0.1:7880",
        validation_alias="AI_DO_LIVEKIT_URL",
    )
    livekit_public_url: str = Field(
        default="",
        validation_alias="AI_DO_LIVEKIT_PUBLIC_URL",
    )
    livekit_api_key: str = Field(
        default="devkey",
        validation_alias="AI_DO_LIVEKIT_API_KEY",
    )
    livekit_api_secret: str = Field(
        default="devsecret-devsecret-devsecret-0001",
        validation_alias="AI_DO_LIVEKIT_API_SECRET",
    )
    livekit_egress_api_url: str = Field(
        default="",
        validation_alias="AI_DO_LIVEKIT_EGRESS_API_URL",
    )
    video_chat_room_prefix: str = Field(
        default="ai-do",
        validation_alias="AI_DO_VIDEO_CHAT_ROOM_PREFIX",
    )
    video_chat_token_ttl_seconds: int = Field(
        default=3600,
        ge=60,
        le=86400,
        validation_alias="AI_DO_VIDEO_CHAT_TOKEN_TTL_SECONDS",
    )
    video_chat_recording_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_VIDEO_CHAT_RECORDING_ENABLED",
    )
    video_chat_captions_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_VIDEO_CHAT_CAPTIONS_ENABLED",
    )
    legacy_issue_compressor_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_LEGACY_ISSUE_COMPRESSOR_ENABLED",
    )
    dataviz_sysperf_storage_dir: str = Field(
        default=".local-sysperf",
        validation_alias="AI_DO_DATAVIZ_SYSPERF_STORAGE_DIR",
    )
    ...
    asr_inference_gateway_model: str = Field(
        ...
        validation_alias="AI_DO_API_ASR_INFERENCE_GATEWAY_MODEL",
    )
    asr_cohere_api_key: str = Field(
        default="",
        validation_alias="AI_DO_API_COHERE_API_KEY",
    )
    asr_cohere_model: str = Field(
        default="transcribe-v1",
        validation_alias="AI_DO_API_ASR_COHERE_MODEL",
    )
    asr_cohere_base_url: str = Field(
        default="https://api.cohere.com/v2",
        validation_alias="AI_DO_API_ASR_COHERE_BASE_URL",
    )
    ...
    asr_request_timeout_seconds: float = Field(
        ...
        gt=0,
        le=3600,
        validation_alias="AI_DO_API_ASR_REQUEST_TIMEOUT_SECONDS",
    )
    ...
    model_status_request_timeout_seconds: float = Field(
        ...
        gt=0,
        le=30,
        validation_alias="AI_DO_MODEL_STATUS_REQUEST_TIMEOUT_SECONDS",
    )
    model_status_diagnostic_targets_json: str = Field(
        default="[]",
        validation_alias="AI_DO_MODEL_STATUS_DIAGNOSTIC_TARGETS_JSON",
    )
    # LLM — Local pool (Apple Silicon mlx-lm by default)
    llm_local_provider: str = Field(
        default="mlx-lm",
        validation_alias="AI_DO_LLM_LOCAL_PROVIDER",
    )
    llm_local_base_url: str = Field(
        default="http://127.0.0.1:8080/v1",
        validation_alias="AI_DO_LLM_LOCAL_BASE_URL",
    )
    llm_local_api_key: str = Field(
        default="mlx",
        validation_alias="AI_DO_LLM_LOCAL_API_KEY",
    )
    llm_local_long_generation_timeout_seconds: float = Field(
        default=1200.0,
        gt=0,
        le=3600,
        validation_alias="AI_DO_LLM_LOCAL_LONG_GENERATION_TIMEOUT_SECONDS",
    )
    ...
    llm_external_long_generation_timeout_seconds: float = Field(
        ...
        gt=0,
        le=3600,
        validation_alias="AI_DO_LLM_EXTERNAL_LONG_GENERATION_TIMEOUT_SECONDS",
    )
    ...
    llm_request_timeout_seconds: float = Field(
        ...
        gt=0,
        le=3600,
        validation_alias="AI_DO_LLM_REQUEST_TIMEOUT_SECONDS",
    )
    llm_healthcheck_on_startup: bool = Field(
        default=True,
        validation_alias="AI_DO_LLM_HEALTHCHECK_ON_STARTUP",
    )
    llm_required: bool = Field(
        default=True,
        validation_alias="AI_DO_LLM_REQUIRED",
    )
    opf_enabled: bool = Field(
        default=True,
        validation_alias="AI_DO_OPF_ENABLED",
    )
    opf_checkpoint: str = Field(
        default=DEFAULT_OPF_CHECKPOINT,
        validation_alias="AI_DO_OPF_CHECKPOINT",
    )
    opf_cache_dir: str = Field(
        default="",
        validation_alias="AI_DO_OPF_CACHE_DIR",
    )
    opf_device: str = Field(
        default="cpu",
        validation_alias="AI_DO_OPF_DEVICE",
    )
    opf_timeout_ms: int = Field(
        default=10000,
        ge=100,
        le=60000,
        validation_alias="AI_DO_OPF_TIMEOUT_MS",
    )
    opf_max_input_chars: int = Field(
        default=50000,
        ge=100,
        le=500000,
        validation_alias="AI_DO_OPF_MAX_INPUT_CHARS",
    )
    opf_max_concurrency: int = Field(
        default=1,
        ge=1,
        le=8,
        validation_alias="AI_DO_OPF_MAX_CONCURRENCY",
    )
    opf_download_on_startup: bool = Field(
        default=True,
        validation_alias="AI_DO_OPF_DOWNLOAD_ON_STARTUP",
    )
    opf_healthcheck_on_startup: bool = Field(
        default=True,
        validation_alias="AI_DO_OPF_HEALTHCHECK_ON_STARTUP",
    )
    opf_required: bool = Field(
        default=True,
        validation_alias="AI_DO_OPF_REQUIRED",
    )
    opf_service_base_url: str = Field(
        default="http://127.0.0.1:18081",
        validation_alias="AI_DO_OPF_SERVICE_BASE_URL",
    )
    opf_service_timeout_ms: int = Field(
        default=10000,
        ge=100,
        le=60000,
        validation_alias="AI_DO_OPF_SERVICE_TIMEOUT_MS",
    )
    ai_tool_calling_enabled: bool = Field(
        default=True,
        validation_alias="AI_DO_AI_TOOL_CALLING_ENABLED",
    )
    ai_conversation_run_lock_ttl_seconds: int = Field(
        default=1800,
        ge=60,
        le=86400,
        validation_alias="AI_DO_AI_CONVERSATION_RUN_LOCK_TTL_SECONDS",
    )
    ai_local_tool_calling_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_AI_LOCAL_TOOL_CALLING_ENABLED",
    )
    ai_mcp_bridge_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_AI_MCP_BRIDGE_ENABLED",
    )
    ai_write_tools_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_AI_WRITE_TOOLS_ENABLED",
    )
    ai_runtime_graph_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_AI_RUNTIME_GRAPH_ENABLED",
    )
    ai_runtime_graph_execution_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_AI_RUNTIME_GRAPH_EXECUTION_ENABLED",
    )
    ai_runtime_shadow_write_enabled: bool = Field(
        default=True,
        validation_alias="AI_DO_AI_RUNTIME_SHADOW_WRITE_ENABLED",
    )
    ai_runtime_trace_payload_max_bytes: int = Field(
        default=32768,
        ge=1024,
        le=1048576,
        validation_alias="AI_DO_AI_RUNTIME_TRACE_PAYLOAD_MAX_BYTES",
    )
    ai_runtime_retention_days: int = Field(
        default=90,
        ge=1,
        le=3650,
        validation_alias="AI_DO_AI_RUNTIME_RETENTION_DAYS",
    )
    ...
    ai_external_search_enabled: bool = Field(
        default=False,
        validation_alias="AI_DO_AI_EXTERNAL_SEARCH_ENABLED",
    )
    ...
    ai_agent_max_turns: int = Field(
        ...
        ge=1,
        le=32,
        validation_alias="AI_DO_AI_AGENT_MAX_TURNS",
    )
    ai_agent_max_tool_calls: int = Field(
        default=16,
        ge=1,
        le=64,
        validation_alias="AI_DO_AI_AGENT_MAX_TOOL_CALLS",
    )
    ai_agent_max_consecutive_tool_errors: int = Field(
        default=3,
        ge=1,
        le=16,
        validation_alias="AI_DO_AI_AGENT_MAX_CONSECUTIVE_TOOL_ERRORS",
    )
    ...
    retrieval_unified_enabled: bool = Field(
        default=True,
        validation_alias="AI_DO_RETRIEVAL_UNIFIED_ENABLED",
    )
    ...
    plm_oracle_host: str = Field(
        default="",
        validation_alias="AI_DO_PLM_ORACLE_HOST",
    )
    plm_oracle_port: int = Field(
        default=1521,
        ge=1,
        le=65535,
        validation_alias="AI_DO_PLM_ORACLE_PORT",
    )
    plm_oracle_sid: str = Field(
        default="",
        validation_alias="AI_DO_PLM_ORACLE_SID",
    )
    plm_oracle_user: str = Field(
        default="",
        validation_alias="AI_DO_PLM_ORACLE_USER",
    )
    plm_oracle_password: str = Field(
        default="",
        validation_alias="AI_DO_PLM_ORACLE_PASSWORD",
    )
    plm_ojdbc_jar: str = Field(
        default="",
        validation_alias="AI_DO_PLM_OJDBC_JAR",
    )
    # Patent (AI 특허 작성) — KIPRIS (Korean patent office) search API.
    patent_kipris_api_key: str = Field(
        default="",
        validation_alias="AI_DO_PATENT_KIPRIS_API_KEY",
    )
    patent_kipris_base_url: str = Field(
        default="https://plus.kipris.or.kr/kipo-api/kipi/patUtiModInfoSearchSevice",
        validation_alias="AI_DO_PATENT_KIPRIS_BASE_URL",
    )
    ...
    event_loop_lag_warn_ms: int = Field(
        ...
        ge=100,
        le=60000,
        validation_alias="AI_DO_API_EVENT_LOOP_LAG_WARN_MS",
    )
    event_loop_lag_interval_seconds: float = Field(
        default=1.0,
        ge=0.1,
        le=60.0,
        validation_alias="AI_DO_API_EVENT_LOOP_LAG_INTERVAL_SECONDS",
    )

    model_config = SettingsConfigDict(
        env_prefix="AI_DO_API_",
    ...
    def _validate_runtime_config(self) -> "Settings":
        ...
        self.ai_do_desktop_update_dirs = ai_do_desktop_update_dir_values(_settings_env_values())
...
def _normalize_dm_attachment_signing_key(value: str, *, environment: str) -> str:
    ...
    if (
        ...
        raise ValueError("AI_DO_DM_ATTACHMENT_SIGNING_KEY must be set for preview/production.")
    return normalized
