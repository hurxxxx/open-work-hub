...
from aidoo_api.domains.rag.contracts import RagProviderHealth
...
from aidoo_api.domains.rag.providers.fake import FakeRerankClient, FakeVectorIndexClient
    ...
    if settings.rag_vector_index_provider == "qdrant":
        if not settings.rag_qdrant_url:
            raise RagProviderConfigurationError(
                "AIDOO_QDRANT_URL is required when rag_vector_index_provider=qdrant"
            )
