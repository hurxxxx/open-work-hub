...
from ai_do_api.core.settings import is_production_like_environment
...
def _load_readonly_harness():
    ...
    module_name = "ai_do_files_rag_readonly_harness_for_live_e2e"
...
def select_live_canaries(
    ...
    *,
        ...
        ordered_strata.append(
            sorted(
                ...
                key=lambda item: _sha256_bytes(
                    b"ai-do-files-live-canary-v1\0"
    ...
    run_tag = _sha256_bytes(b"ai-do-files-live-run-tag-v1\0" + nonce)[:16]
    ...
    while len(selected) < max_canaries and round_index < max_rounds:
        for items in ordered_strata:
            ...
            item_digest = _sha256_bytes(
                b"ai-do-files-live-upload-name-v1\0"
                + nonce
...
class HttpFilesApi:
    ...
    def preflight(self, workspace_slug: str) -> None:
        ...
        query = _sha256_bytes(b"ai-do-files-live-preflight-v1\0" + secrets.token_bytes(16))[:16]
...
class DevelopmentProjectionInspector:
    ...
    def _state(self, *, corpus_id: str, file_ids: Sequence[str]) -> dict[str, object]:
        ...
        from ai_do_api.domains.files.models import FileManagerCorpus, FileManagerFile
...
def run_live_e2e(
    *,
    ...
    query_id = _sha256_bytes(b"ai-do-files-live-query-v1\0" + query.encode("ascii"))
    ...
    try:
        ...
        content_query_id = _sha256_bytes(
            b"ai-do-files-live-content-query-v1\0" + content_query.encode("utf-8")
        )
        ...
        return {
            "schema_version": "ai-do.files-rag-live-e2e.v1",
            "status": "passed",
            "source_root_id": _sha256_bytes(
                b"ai-do-files-live-source-root-v1\0"
...
def assert_development_data_plane(settings: object) -> None:
    ...
    try:
        ...
        raise LiveE2EContractError("development_data_plane_required") from error
    if database_url.database != "ai_do_dev":
        raise LiveE2EContractError("development_data_plane_required")
    ...
    expected_names = {
        "minio_bucket": "ai-do-dev",
        "opensearch_index_prefix": "ai-do-dev",
        "rag_qdrant_collection_prefix": "ai-do-dev-rag",
    }
    if any(
    ...
    bindings = (
        ("AI_DO_POSTGRES_DSN", "postgres_dsn"),
        ("AI_DO_OPENSEARCH_URL", "opensearch_url"),
        ("AI_DO_RAG_QDRANT_URL", "rag_qdrant_url"),
