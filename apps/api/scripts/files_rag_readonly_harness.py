...
calling AI-DO services.  Standard output and optional manifests contain only
...
from ai_do_api.domains.document_processing.html_extractor import (
    HtmlExtractionError,
    extract_html_stream,
)

SCHEMA_VERSION = "ai-do.files-rag-readonly-manifest.v2"
...
def _source_id(relative_path: Path) -> str:
    ...
    return _sha256_bytes(b"ai-do-files-source-v1\0" + encoded)


def _source_root_id(source_root: Path) -> str:
    encoded = os.fsencode(str(source_root))
    return _sha256_bytes(b"ai-do-files-source-root-v1\0" + encoded)
...
def _query_id(query: str) -> str:
    ...
    if not normalized:
        raise HarnessContractError("invalid_evaluation_input")
    return _sha256_bytes(b"ai-do-retrieval-eval-query-v1\0" + normalized.encode("utf-8"))
