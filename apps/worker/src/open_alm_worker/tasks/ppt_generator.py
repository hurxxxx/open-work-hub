...
from open_alm_api.core.llm import LlmTaskContext  # noqa: E402
from open_alm_api.domains.ai.external_gateway import (  # noqa: E402
    AiExternalCapabilityPolicyViolation,
    AiExternalCapabilityRequest,
    begin_external_capability,
)
from open_alm_api.domains.ai.gateway import (  # noqa: E402
    LlmWorkloadContext,
    execute_llm,
    resolve_llm_workload_route,
)
from open_alm_api.domains.conversations import app_persistence  # noqa: E402
from open_alm_api.domains.conversations.models import Conversation  # noqa: E402
from open_alm_api.domains.ppt_generator import parsing, prompts  # noqa: E402
from open_alm_api.domains.ppt_generator import (  # noqa: E402
    PPT_DESIGN_WORKLOAD_ID,
    PPT_RESEARCH_WORKLOAD_ID,
)
from open_alm_api.domains.ppt_generator.anthropic_search_adapter import (  # noqa: E402
    AnthropicWebResearchAuthenticationError,
    run_anthropic_web_research,
)
from open_alm_api.domains.ppt_generator import service as ppt_service  # noqa: E402
from open_alm_api.domains.ppt_generator import source_images as src_images  # noqa: E402
from open_alm_api.domains.ppt_generator import ole_embed as ole  # noqa: E402
from open_alm_api.domains.ppt_generator.families import resolve_family  # noqa: E402
from open_alm_api.domains.ppt_generator.models import PptJob  # noqa: E402
...
def _normalize_house_tables(slides: list[dict]) -> None:
    ...
    """
    from open_alm_api.domains.ppt_generator.design.builders_house import _as_cells
...
def _generate_house_spec(session, job: PptJob, family_key: str, params: dict) -> tuple[dict, int]:
    """Open ALM 사내 진행보고 "하우스 스타일"(표 중심) — 블록 JSON 생성 → builders_house 렌더.
    ...
    # 표지: 세미나 참석 보고와 동일한 Open ALM A4 표지(a4-cover)를 맨 앞에 붙인다.
...
def _parse_corporate_body_html(raw_html: str) -> tuple[str, list[dict], str]:
    ...
    sections: list[dict] = []
    found = soup.select("section.corporate-body") or soup.find_all("section")
    for sec in found:
        sections.append(
            {
...
def _generate_corporate_free_spec(
    session, job: PptJob, family_key: str, params: dict
) -> tuple[dict, int]:
    """Open ALM 양식(자유 본문) — 보안 정책상 외부 본문 생성 대신 내부 LLM 파이프라인 사용.

    Claude 는 '주제(≤100자) 최신정보 검색' 용도로만 쓰이고(=_generate_corporate_qwen_spec
    내부 _research_block), 첨부 파일/본문은 외부로 나가지 않고 내부 Qwen 으로만 처리한다.
    """
    return _generate_corporate_qwen_spec(session, job, family_key, params)


def _generate_corporate_qwen_spec(
    session, job: PptJob, family_key: str, params: dict
) -> tuple[dict, int]:
    """Open ALM 양식(내부 LLM 자동) — 내용은 JSON, 디자인은 서버 §14 템플릿.

    파이프라인: (1) Qwen 아웃라인 1콜 → (2) 슬라이드별 Qwen 채움 N콜 →
    (3) corporate_templates 로 §14 본문 HTML 렌더 → (4) corporate_frame Open ALM 틀로 합성.
    반환: (slides_spec[format=html, chrome=corporate], n_slides). Claude 불필요.
    """
    from open_alm_api.domains.ppt_generator.design import corporate_frame, corporate_templates
    ...
    outline_msgs = prompts.build_corporate_qwen_outline_messages(
        content, n_body, language, tone, instructions
    )
    try:
        deck_title, outline = parsing.parse_corporate_qwen_outline(
    ...
    for idx, o in enumerate(outline, start=1):
        ...
        fill_msgs = prompts.build_corporate_qwen_fill_messages(
        ...
        try:
            slide = parsing.parse_corporate_qwen_slide(
        ...
        sections.append(corporate_templates.render_section(slide))

    cover_title = deck_title or (content[:40].strip() if content else "발표 자료")
    deck_html = corporate_frame.compose_deck(
        title=cover_title,
    ...
    spec = {
        ...
        "format": "html",
        "html": deck_html,
        "slide_w": corporate_frame.SLIDE_W_PX,
        "slide_h": corporate_frame.SLIDE_H_PX,
        "slide_w_cm": corporate_frame.SLIDE_W_CM,
        "slide_h_cm": corporate_frame.SLIDE_H_CM,
        "chrome": "corporate",
        "cover": {
            "title": cover_title,
            "author": params.get("author_name") or None,
        ...
        "body": sections,
        "body_style": "",
        "body_w": corporate_frame.BODY_W_PX,
        "body_h": corporate_frame.BODY_H_PX,
    }
...
def _build_seminar_ole_pptx(job: PptJob) -> bytes | None:
    """접은 출장 일정(params[fold_trip])을 Open ALM 양식 출장 일정 페이지 덱(.pptx)으로 렌더.
    ...
    try:
        from open_alm_api.domains.ppt_generator.design import corporate_frame, corporate_templates
        from open_alm_api.domains.ppt_generator.design.html_to_pptx import (
            corporate_deck_to_pptx_bytes,
        )

        _, fam = resolve_family(job.family)
        sec = corporate_templates.render_section(trip)
        return corporate_deck_to_pptx_bytes(
            {"title": str(trip.get("title") or "출장 일정")},
            [sec],
            "",
            logo_path=fam.get("logo_path"),
            body_w_px=corporate_frame.BODY_W_PX,
            body_h_px=corporate_frame.BODY_H_PX,
        )
    except Exception as e:  # noqa: BLE001
...
def _inject_seminar_ole_icon(session, job: PptJob, report_pages: list[dict]) -> bool:
    ...
    finalize `_maybe_embed_ole`(corporate-seminar 분기)가 이 자리에 일정표/출장 덱을 OLE 로 주입한다.
...
def _build_and_store_result_spec(
    ...
    스펙을 Open ALM pptx 로 변환해 OLE 로 임베드한다. 항목이 없으면 저장하지 않음(placeholder 폴백).
    ...
    toc = corporate_templates.render_section({"pattern": "toc", "title": subject, "items": items})
    body_sections = []
    for i, item in enumerate(items):
        ...
        body_sections.append(corporate_templates.render_section(sec))
    result_spec = {
        "cover": {
    ...
    # Open ALM 틀로 합성해 HTML 로도 돌려준다 → 프론트가 똑딱이 안 내용을 미리 보여준다.
    try:
        return corporate_frame.compose_deck(
...
def _generate_corporate_seminar_spec(
    session, job: PptJob, family_key: str, params: dict
) -> tuple[dict, int]:
    """세미나 참석 보고 — Open ALM 틀 + 내부 LLM 단일 본문(정보헤더+주요내용표+소감).

    파이프라인: (1) Qwen 단일 채움 콜 → (2) corporate_templates 로 본문 HTML 렌더 →
    (3) corporate_frame Open ALM 틀로 합성. 본문 1장 고정(아웃라인 불필요).
    """
    from open_alm_api.domains.ppt_generator.design import corporate_frame, corporate_templates
    ...
    fill_msgs = prompts.build_corporate_seminar_fill_messages(
    ...
    trip_msgs = prompts.build_corporate_seminar_trip_messages(
        content_excerpt, language, tone, instructions
    )
    trip_fallback = {"title": "출장 일정", "pattern": "trip_schedule"}
    try:
        trip = parsing.parse_corporate_qwen_slide(
            _call_freeform_llm(
                session,
                job,
                trip_msgs,
    ...
    if not fold_trip and prev_fold is not None:
        ...
        try:
            ...
            session.rollback()
    sections = [corporate_templates.render_section(p) for p in report_pages]
    ...
    if not merged_trip and not fold_trip and isinstance(trip, dict) and trip.get("days"):
        sections.append(corporate_templates.render_section(trip))
...
def _generate_corporate_education_spec(
    session, job: PptJob, family_key: str, params: dict
) -> tuple[dict, int]:
    """교육 보고서 — Open ALM 틀 + 내부 LLM 단일 본문(구분/내용/비고 표).

    파이프라인: (1) Qwen 단일 채움 콜 → (2) corporate_templates 로 본문 HTML 렌더 →
    (3) corporate_frame Open ALM 틀로 합성. 본문 1장 고정.
    """
    from open_alm_api.domains.ppt_generator.design import corporate_frame, corporate_templates
    ...
    fill_msgs = prompts.build_corporate_education_fill_messages(
        content_excerpt, language, tone, instructions
    )
    fallback = {
    ...
    sections = [corporate_templates.render_section(slide)]
    cover_title = str(slide.get("title") or "").strip() or (
...
def _transcribe_audio_uploads(session, job: PptJob) -> str:
    ...
    from open_alm_api.core.asr import get_asr_backend
...
def _generate_corporate_meeting_spec(
    session, job: PptJob, family_key: str, params: dict
) -> tuple[dict, int]:
    """회의록 — Open ALM 세로 A4 1장. 회의 메모/문서 → 머리표+회의내용 정리."""
    ...
    fill_msgs = prompts.build_corporate_meeting_fill_messages(
        content_excerpt, language, tone, instructions
    )
    ...
    deck_html = corporate_templates.render_meeting_minutes_deck(slide)
    spec = {
        "family": family_key,
        "format": "html",
        "html": deck_html,
        "slide_w": corporate_templates.MEETING_W_PX,
        "slide_h": corporate_templates.MEETING_H_PX,
        "slide_w_cm": corporate_templates.MEETING_W_CM,
        "slide_h_cm": corporate_templates.MEETING_H_CM,
...
def _build_pptx_bytes(family_key: str, slides: list[dict]) -> bytes:
    ...
    # 일부 양식(예: corporate-house/TEST)은 본문 빌더와 다른 표지(a4-cover)를 섞어 쓴다.
    # a4-cover 레이아웃은 양식 build_fn 대신 공용 Open ALM A4 표지 빌더로 그린다.
...
def generate(self, job_id: str) -> None:
    with _db_session_scope() as session:
        ...
        try:
            ...
            # Open ALM 양식(자유 본문) — Claude 본문 콘텐츠 + 서버 Open ALM 틀 합성.
            if _fam.get("design") == "corporate-free":
                spec, n_slides = _generate_corporate_free_spec(session, job, family_key, params)
                _set(
                    session,
                    job,
                    status="completed",
            ...
            # Open ALM 양식(Qwen 자동) — Qwen JSON 내용 + 서버 §14 템플릿 + Open ALM 틀 합성.
            if _fam.get("design") == "corporate-qwen":
                spec, n_slides = _generate_corporate_qwen_spec(session, job, family_key, params)
                _set(
                    session,
                    job,
                    status="completed",
            ...
            # 회의록 — 세로 A4 단일 회의록 양식.
            if _fam.get("design") == "corporate-meeting":
                spec, n_slides = _generate_corporate_meeting_spec(session, job, family_key, params)
                _set(
                    session,
                    job,
                    status="completed",
            ...
            # 세미나 참석 보고 — Open ALM 틀 + Qwen 단일 본문 채움.
            if _fam.get("design") == "corporate-seminar":
                spec, n_slides = _generate_corporate_seminar_spec(session, job, family_key, params)
                _set(
                    session,
                    job,
                    status="completed",
            ...
            # 교육 보고서 — Open ALM 틀 + Qwen 단일 본문 표(구분/내용/비고) 채움.
            if _fam.get("design") == "corporate-education":
                spec, n_slides = _generate_corporate_education_spec(session, job, family_key, params)
                _set(
                    session,
                    job,
                    status="completed",
...
def _build_result_report_pptx(job: PptJob, params: dict) -> bytes:
    ...
    generate 가 저장해 둔 result_spec(표지+목차+본문) 이 있으면 Open ALM pptx 로 변환해 반환.
...
def _maybe_embed_ole(job: PptJob, pptx_bytes: bytes) -> bytes:
    ...
    if fam.get("design") == "corporate-seminar":
    ...
    if fam.get("design") != "corporate-education" and not has_attach:
        return pptx_bytes
...
def finalize(self, job_id: str) -> None:
    with _db_session_scope() as session:
        ...
        try:
            ...
            if is_html and spec.get("chrome") == "corporate":
                # Open ALM 양식: 표지/본문 틀은 네이티브(builders_a4), 본문만 변환해 칸에 끼움.
                ...
                try:
                    ...
                    pptx_bytes = corporate_deck_to_pptx_bytes(
                        spec.get("cover") or {},
                        spec.get("body") or [],
                        spec.get("body_style") or "",
                        logo_path=logo,
                    ...
                    logger.exception(
                        "ppt_generator.finalize Open ALM 네이티브 변환 실패 → 이미지 폴백 (job=%s)",
                        job_id,
                    )
