...
from open_alm_worker.settings import Settings, get_settings as get_worker_settings
...
def _require_groupware_db_settings(settings: Settings) -> None:
    missing = [
        key
        for key, value in {
            "OPEN_ALM_HR_GROUPWARE_DB_HOST": settings.hr_groupware_db_host,
            "OPEN_ALM_HR_GROUPWARE_DB_USERNAME": settings.hr_groupware_db_username,
            "OPEN_ALM_HR_GROUPWARE_DB_PASSWORD": settings.hr_groupware_db_password,
        }.items()
        if not value.strip()
    ]
    if missing:
...
def _require_erp_db_settings(settings: Settings) -> None:
    missing = [
        key
        for key, value in {
            "OPEN_ALM_ERP_DB_IP": settings.erp_db_ip,
            "OPEN_ALM_ERP_DB_NAME": settings.erp_db_name,
            "OPEN_ALM_ERP_DB_ID": settings.erp_db_id,
            "OPEN_ALM_ERP_DB_PW": settings.erp_db_pw,
        }.items()
        if not value.strip()
    ]
    if missing:
