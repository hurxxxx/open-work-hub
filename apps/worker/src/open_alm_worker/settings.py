...
class Settings(BaseSettings):
    ...
    queue_group: str = Field(
        default="all",
        validation_alias="OPEN_ALM_WORKER_QUEUE_GROUP",
    )
    ...
    rag_backfill_batch_size: int = Field(
        ...
        ge=1,
        le=500,
        validation_alias="OPEN_ALM_RAG_BACKFILL_BATCH_SIZE",
    )
    rag_backfill_throttle_ms: int = Field(
        default=50,
        ge=0,
        le=60000,
        validation_alias="OPEN_ALM_RAG_BACKFILL_THROTTLE_MS",
    )
    rag_job_max_attempts: int = Field(
        default=3,
        ge=1,
        le=20,
        validation_alias="OPEN_ALM_RAG_JOB_MAX_ATTEMPTS",
    )
    rag_job_retry_backoff_seconds: int = Field(
        default=30,
        ge=1,
        le=3600,
        validation_alias="OPEN_ALM_RAG_JOB_RETRY_BACKOFF_SECONDS",
    )
    ...
    hr_groupware_db_host: str = Field(
        default="",
        validation_alias="OPEN_ALM_HR_GROUPWARE_DB_HOST",
    )
    hr_groupware_db_port: int = Field(
        default=1433,
        ge=1,
        le=65535,
        validation_alias="OPEN_ALM_HR_GROUPWARE_DB_PORT",
    )
    hr_groupware_db_name: str = Field(
        default="dw_visitor",
        validation_alias="OPEN_ALM_HR_GROUPWARE_DB_NAME",
    )
    hr_groupware_db_username: str = Field(
        default="",
        validation_alias="OPEN_ALM_HR_GROUPWARE_DB_USERNAME",
    )
    hr_groupware_db_password: str = Field(
        default="",
        validation_alias="OPEN_ALM_HR_GROUPWARE_DB_PASSWORD",
    )
    hr_groupware_db_timeout_seconds: int = Field(
        default=15,
        ge=1,
        le=120,
        validation_alias="OPEN_ALM_HR_GROUPWARE_DB_TIMEOUT_SECONDS",
    )
    ...
    erp_db_ip: str = Field(
        default="",
        validation_alias="OPEN_ALM_ERP_DB_IP",
    )
    erp_db_port: int = Field(
        default=1433,
        ge=1,
        le=65535,
        validation_alias="OPEN_ALM_ERP_DB_PORT",
    )
    erp_db_name: str = Field(
        default="",
        validation_alias="OPEN_ALM_ERP_DB_NAME",
    )
    erp_db_id: str = Field(
        default="",
        validation_alias="OPEN_ALM_ERP_DB_ID",
    )
    erp_db_pw: str = Field(
        default="",
        validation_alias="OPEN_ALM_ERP_DB_PW",
    )
    erp_db_timeout_seconds: int = Field(
        default=15,
        ge=1,
        le=120,
        validation_alias="OPEN_ALM_ERP_DB_TIMEOUT_SECONDS",
    )
    ...
    qna_board_category: str = Field(
        default="주요공지사항(관리팀)",
        validation_alias="OPEN_ALM_QNA_BOARD_CATEGORY",
    )
    ...
    qna_board_mnu_num: int = Field(
        default=10,
        ge=0,
        validation_alias="OPEN_ALM_QNA_BOARD_MNU_NUM",
    )
    # Groupware web access for board metadata, notice body, and attachments.
    qna_board_web_base_url: str = Field(
        default="http://gw.example.com",
        validation_alias="OPEN_ALM_QNA_BOARD_WEB_BASE_URL",
    )
    qna_board_web_username: str = Field(
        default="",
        validation_alias="OPEN_ALM_QNA_BOARD_WEB_USERNAME",
    )
    qna_board_web_password: str = Field(
        default="",
        validation_alias="OPEN_ALM_QNA_BOARD_WEB_PASSWORD",
    )
    qna_board_max_posts: int = Field(
        default=20,
        ge=1,
        le=200,
        validation_alias="OPEN_ALM_QNA_BOARD_MAX_POSTS",
    )
    qna_board_attachment_max_bytes: int = Field(
        default=50 * 1024 * 1024,
        ge=1024,
        le=512 * 1024 * 1024,
        validation_alias="OPEN_ALM_QNA_BOARD_ATTACHMENT_MAX_BYTES",
    )
    qna_board_attachments_max_total_bytes: int = Field(
        default=100 * 1024 * 1024,
        ge=1024,
        le=1024 * 1024 * 1024,
        validation_alias="OPEN_ALM_QNA_BOARD_ATTACHMENTS_MAX_TOTAL_BYTES",
    )
    ...
    image_request_timeout_seconds: float = Field(
        ...
        gt=0,
        le=3600,
        validation_alias="OPEN_ALM_IMAGE_REQUEST_TIMEOUT_SECONDS",
    )
