...
const apiProxyTarget = process.env.OPEN_ALM_WEB_API_PROXY_TARGET ?? 'http://127.0.0.1:8001';
const drawioProxyTarget =
  process.env.OPEN_ALM_WEB_DRAWIO_PROXY_TARGET ??
  `http://127.0.0.1:${process.env.OPEN_ALM_DRAWIO_PORT ?? 18082}`;
const webDevPort = Number(process.env.OPEN_ALM_WEB_DEV_PORT ?? 4200);
const webBuildOutDir = '../../dist/apps/web';
const drawioBrowserUrl =
  process.env.VITE_OPEN_ALM_DRAWIO_URL ?? process.env.OPEN_ALM_DRAWIO_SERVER_URL ?? '';
const drawioBrowserPort = String(process.env.OPEN_ALM_DRAWIO_PORT ?? 18082);
...
export default defineConfig(() => ({
  ...
  define: {
    ...
    'import.meta.env.VITE_OPEN_ALM_DRAWIO_PORT': JSON.stringify(drawioBrowserPort),
    'import.meta.env.VITE_OPEN_ALM_DRAWIO_URL': JSON.stringify(drawioBrowserUrl),
  },
  server: {
    port: webDevPort,
    host: '0.0.0.0',
  ...
  resolve: {
    alias: {
      '@open-alm/ui/styles.css': path.resolve(import.meta.dirname, '../../packages/ui/styles.css'),
