    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-4 px-2 space-y-6 custom-scrollbar">
          ...
          {/*
           * Learning sub-sidebar: collapsible course tree. Sits below the
           * "전체 학습 홈" NAV_ITEMS link so the landing page is always
           * reachable, while the per-lesson TOC stays in a column that never
           * covers the article and remains visible while the reader scrolls.
           */}
          {activeAppId === 'learning' ? (
            <LearningCourseTreeBlock currentPathname={location.pathname} />
          ) : null}
...
// Learning course tree — rendered inside the sub-sidebar for the Learning app.
// Each course has an independent expand/collapse state; the active course
// auto-expands so the current lesson is always visible without an extra click.

function parseLearningPathname(pathname: string): {
...
function LearningCourseTreeBlock({
  currentPathname,
}: {
  ...
  // Auto-expand when the user navigates into a different course.
  useEffect(() => {
