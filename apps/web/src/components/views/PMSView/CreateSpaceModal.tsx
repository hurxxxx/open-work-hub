...
export const CreateSpaceModal = ({
  ...
  const workspaceId = getWorkspaceRoleByKey(user, 'pms')?.workspace_id;
  ...
  async function handleCreate() {
    if (!token || !name.trim() || !workspaceId) return;
    setSubmitting(true);
    setError('');
    try {
    ...
    >
      <div className="space-y-5 text-clickup-text">
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium text-clickup-text/70">Space Name</label>
          <input
            type="text"
