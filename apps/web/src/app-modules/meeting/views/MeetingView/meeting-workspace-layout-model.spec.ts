...
import {
  MEETING_WORKSPACE_INITIAL_STATE,
  formatMeetingWorkspaceRange,
  meetingWorkspaceReducer,
  type MeetingWorkspaceState,
} from './meeting-workspace-layout-model';
...
function loadedState(
  overrides: Partial<MeetingWorkspaceState> = {},
): MeetingWorkspaceState {
  return {
    ...MEETING_WORKSPACE_INITIAL_STATE,
    meeting: meeting(),
    notesDoc: doc(),
...
describe('meeting workspace layout model', () => {
  it('marks loads as in-flight and clears stale errors', () => {
    const state = meetingWorkspaceReducer(
  ...
  it('clears loaded data and stores the error after a failed load', () => {
    const state = meetingWorkspaceReducer(loadedState({ loading: true }), {
      type: 'loadFailed',
  ...
  it('updates the detail drawer state', () => {
    expect(
      meetingWorkspaceReducer(loadedState(), {
        type: 'setDetailOpen',
        open: true,
      }).detailOpen,
    ).toBe(true);
    ...
    expect(
      meetingWorkspaceReducer(loadedState({ detailOpen: true }), {
        type: 'setDetailOpen',
        open: false,
      }).detailOpen,
    ).toBe(false);
  ...
  it('replaces the selected notes page', () => {
    ...
    const state = meetingWorkspaceReducer(loadedState(), {
      type: 'setNotesPage',
      notesPage: nextPage,
    });
  ...
  it('does not update notes content when no page is selected', () => {
    ...
    const nextState = meetingWorkspaceReducer(state, {
