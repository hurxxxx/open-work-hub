...
describe('DocsGroupSharing', () => {
  it('grants an app-owned role without changing content ownership', async () => {
    render(
      <DocsGroupSharing token="token" resourceId="resource-1" canManage />,
    );
    await waitFor(() =>
      expect(
    ...
    await waitFor(() =>
      expect(apiFetchJson).toHaveBeenCalledWith(
        '/api/v1/docs/items/resource-1/sharing/groups/group-1',
        'token',
  ...
  it('ignores stale sharing responses when the resource changes', async () => {
    ...
    result.rerender(
      <DocsGroupSharing token="token" resourceId="resource-2" canManage />,
    );
