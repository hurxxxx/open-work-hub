...
function htmlZoomStyle(resolvedZoom: number): string {
  return `<style data-open-work-hub-html-zoom>html{zoom:${resolvedZoom.toFixed(2)};}</style>`;
}

const HTML_WHEEL_BRIDGE_SCRIPT = `<script data-open-work-hub-wheel-bridge>(function(){if(window.__openWorkHubWheelBridge)return;window.__openWorkHubWheelBridge=true;function canScroll(el,dx,dy){if(!el||el===document)return false;var style=window.getComputedStyle(el);var canY=Math.abs(dy)>0&&/(auto|scroll|overlay)/.test(style.overflowY)&&el.scrollHeight>el.clientHeight;var canX=Math.abs(dx)>0&&/(auto|scroll|overlay)/.test(style.overflowX)&&el.scrollWidth>el.clientWidth;var yRoom=canY&&(dy<0?el.scrollTop>0:el.scrollTop+el.clientHeight<el.scrollHeight);var xRoom=canX&&(dx<0?el.scrollLeft>0:el.scrollLeft+el.clientWidth<el.scrollWidth);return yRoom||xRoom;}function findScrollable(start,dx,dy){var node=start;while(node&&node!==document.body&&node!==document.documentElement){if(canScroll(node,dx,dy))return node;node=node.parentElement;}return document.scrollingElement||document.documentElement;}window.addEventListener('message',function(event){var data=event.data;if(!data||data.type!=='open-work-hub-docs-html-wheel')return;var dx=Number(data.deltaX)||0;var dy=Number(data.deltaY)||0;var x=Number(data.clientX);var y=Number(data.clientY);var start=Number.isFinite(x)&&Number.isFinite(y)?document.elementFromPoint(x,y):document.scrollingElement;var target=findScrollable(start,dx,dy);if(target&&typeof target.scrollBy==='function'){target.scrollBy({left:dx,top:dy,behavior:'auto'});}else{window.scrollBy(dx,dy);}});})();</script>`;
