...
type Grant = ApiSchema<'SpaceGroupBindingResponse'>;
type Role = Grant['role'];
function SpaceGroupBindingsContent({
  token,
  resourceId,
  canManage,
  ...
  const endpoint = `/api/v1/pms/spaces/${encodeURIComponent(resourceId)}/groups`;
  useEffect(() => {
  ...
  const roles: Role[] = ['viewer', 'member'];
  if (canManageAdmins) roles.push('admin');
  return (
      ...
      <ul className="space-y-2">
        {items.map((item) => (
          <li key={item.group_id} className="flex flex-wrap items-center gap-2">
            ...
            <span>{t(`groupSharing.roles.${item.role}`)}</span>
            {canManage && (canManageAdmins || item.role !== 'admin') ? (
              <>
                <select
...
export function SpaceGroupBindings(
  props: Parameters<typeof SpaceGroupBindingsContent>[0],
) {
  return (
    <SpaceGroupBindingsContent
