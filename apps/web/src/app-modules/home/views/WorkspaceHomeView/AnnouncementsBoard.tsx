...
function AnnouncementEditorModal({
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!token || !workspaceSlug || !draftTitle.trim() || submitting) return;
    setSubmitting(true);
    setError(null);
    try {
      if (isEdit) {
        await updateAnnouncement(token, workspaceSlug, announcement.id, {
          title: draftTitle.trim(),
          body: body.trim(),
          isPinned: pinned,
        });
      } else {
        await createAnnouncement(token, workspaceSlug, {
          title: draftTitle.trim(),
          body: body.trim(),
          scope,
          isPinned: pinned,
...
function AnnouncementDetailModal({
  ...
  const remove = async () => {
    if (!token || !workspaceSlug || deleting) return;
    if (!window.confirm(t('home.announcementDeleteConfirm'))) return;
    setDeleting(true);
    setError(null);
    try {
      await deleteAnnouncement(token, workspaceSlug, announcement.id);
      onDeleted();
    } catch {
      ...
      <div className="flex max-h-[80vh] w-full max-w-md flex-col rounded-xl border border-app-border bg-app-surface p-5 shadow-xl">
        <div className="mb-3 flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-start gap-2">
            {announcement.isPinned ? (
              ...
              <Megaphone size={16} className="mt-0.5 shrink-0 text-app-ink/45" />
            )}
        ...
        <div className="app-text-body min-h-0 flex-1 overflow-y-auto whitespace-pre-wrap break-words text-app-ink">
          {announcement.body.trim() ? (
            ...
            <span className="text-app-ink/55">{t('home.announcementNoBody')}</span>
          )}
        </div>
        {error ? (
...
export function AnnouncementsBoard({
  ...
  const canManage =
    scope === 'company'
      ? hasAnySystemRole(user, ['platform_admin'])
  ...
  useEffect(() => {
    if (!token || !workspaceSlug) return undefined;
    let cancelled = false;
    setLoading(true);
    listAnnouncements(token, workspaceSlug, { scope, limit: 8 })
      .then((response) => {
  ...
  }, [token, workspaceSlug, scope, reloadKey]);
