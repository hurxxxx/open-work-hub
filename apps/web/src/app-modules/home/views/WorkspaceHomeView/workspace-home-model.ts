...
export type WorkspaceHomeSectionStatus = 'loading' | 'empty' | 'ready';

export type WorkspaceHomePriorityTone = 'danger' | 'warning' | 'muted';
...
export type WorkspaceHomeRow =
  ...
  | {
      ...
      to: string;
      trailing: string;
      priorityTone: WorkspaceHomePriorityTone;
    }
  | {
      kind: 'doc';
      id: string;
...
export type WorkspaceHomeSection = {
  ...
  emptyCtaTo: string;
  status: WorkspaceHomeSectionStatus;
  rows: WorkspaceHomeRow[];
};

export type WorkspaceHomeBriefing = {
...
function sectionStatus(
  loading: boolean,
  rows: readonly WorkspaceHomeRow[],
): WorkspaceHomeSectionStatus {
...
export function buildWorkspaceHomeSections({
  ...
  const taskRows = selectTopAssignedTasks(state.issues).map<WorkspaceHomeRow>(
    (issue) => ({
      kind: 'task',
      id: issue.id,
      title: issue.title,
  ...
  const docRows = selectTopRecentPages(state.recentPages).map<WorkspaceHomeRow>(
    (page) => ({
      kind: 'doc',
      id: page.page_id,
