...
} from './useWorkspaceHomeController';

describe('useWorkspaceHomeController', () => {
  ...
  it('does not load without token or workspace slug', () => {
    const client = clientWith();
    renderController({ client, token: null, workspaceSlug: '' });
...
function renderController(
  ...
  return renderHook(() =>
    useWorkspaceHomeController({
      ...
      workspaceSlug: options.workspaceSlug ?? 'hq',
    }),
  );
}

function clientWith(
  overrides: Partial<WorkspaceHomeClient> = {},
): WorkspaceHomeClient {
  return {
