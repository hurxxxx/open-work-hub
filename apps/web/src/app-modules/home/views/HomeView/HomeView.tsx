...
function resolveAiToolHref(item: BootstrapNavItem): string {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <h1 className="app-text-title-lg text-app-ink">{companyName}</h1>
      </div>
...
function BriefingBanner({ briefing }: { briefing: HomeBriefing }) {
...
function AiToolsMenu(_context: Record<string, never>) {
  const { t } = useTranslation('apps');
  const { t: tShell } = useTranslation('shell');
  const { aiToolAppIds, nav } = useAppBootstrapProjection();
...
function HomeSummaryRow({ row }: { row: HomeRow }) {
  if (row.kind === 'meeting') {
    return (
      <Link
        to={row.to}
...
function HomeSummarySection({ section }: { section: HomeSection }) {
  const { t } = useTranslation('apps');
      ...
      <div>
        {section.status === 'loading' ? (
          ...
          section.rows.map((row) => (
            <HomeSummaryRow key={`${row.kind}:${row.id}`} row={row} />
          ))
        )}
      </div>
    </section>
...
export const HomeView = () => {
  ...
  const { apps, loading: appsLoading } = useAppBootstrapProjection();
  ...
  const enabledAppIds = useMemo(
    () =>
      appsLoading
        ? null
        : apps.flatMap((app) => (app.enabled ? [app.app_id] : [])),
    [apps, appsLoading],
  );
  ...
  const sections = useMemo(
    ...
    [i18n.language, state, timeZone, t],
  );
  ...
  return (
    <div className="custom-scrollbar h-full overflow-y-auto">
      <div className="w-full space-y-6 px-8 py-10">
        <HomeHeader
          onPreferenceChanged={reload}
          userName={userName}
          companyName={companyName}
          timeZone={timeZone}
        />
        <BriefingBanner briefing={briefing} />
        <QuickActionsRow enabledAppIds={enabledAppIdSet} />
        ...
        <div className="grid auto-rows-fr grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
          <AnnouncementsBoard
            scope="company"
            title={`${companyName} ${t('home.announcements')}`}
          />
          {plannerSection ? (
            <HomeSummarySection section={plannerSection} />
          ) : null}
          {meetingsSection ? (
            <HomeSummarySection section={meetingsSection} />
          ) : null}
          {tasksSection ? <HomeSummarySection section={tasksSection} /> : null}
          <AnnouncementsBoard
            scope="company"
            title={t('home.announcementsCompany')}
          />
          {docsSection ? <HomeSummarySection section={docsSection} /> : null}
