...
export interface HomeClient {
  ...
  listRecentPages(token: string, limit: number): Promise<RecentPageItem[]>;
}
...
const defaultClient: HomeClient = {
  listAssignedTasks,
  listMeetings,
  listPlannerEvents,
  listRecentPages,
...
export function useHomeController({
  ...
  useEffect(() => {
    ...
    if (enabledApps.has('meeting')) {
      void client
        .listMeetings(token, { scope: 'upcoming' })
        .then((response) => {
          if (!cancelled) {
    ...
    if (enabledApps.has('pms')) {
      void client
        .listAssignedTasks(token, { limit: 10 })
        .then((response) => {
          if (!cancelled) {
  ...
  }, [client, enabledAppIds, timeZone, token]);
