...
interface DrawioBrowserEnv {
  DEV?: boolean;
  VITE_AI_DO_DRAWIO_PORT?: string;
  VITE_AI_DO_DRAWIO_URL?: string;
}
...
export function resolveDrawioServerUrl({
  ...
  const configuredUrl = env.VITE_AI_DO_DRAWIO_URL?.trim() ?? '';
  ...
  if (env.DEV || (!configuredUrl && isDevelopmentHost(location.hostname))) {
    return drawioDevUrlFromLocation(
      location,
      env.VITE_AI_DO_DRAWIO_PORT?.trim() || DRAWIO_DEV_DEFAULT_PORT,
    );
  }
