  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const sessionIds = Array.from(
        new Set([...localById.keys(), ...remoteById.keys()]),
      );
      const nextItems = await Promise.all(
        sessionIds.map(async (stagingId) => {
      ...
      setItems(
        nextItems.sort((left, right) => {
          const leftTs =
            left.localSession?.startedAt ??
            Date.parse(left.remoteStaging?.started_at ?? '0');
          const rightTs =
            right.localSession?.startedAt ??
            Date.parse(right.remoteStaging?.started_at ?? '0');
      ...
      setError(
        err instanceof Error
          ? err.message
          : t('recording.errors.recoveryCheckFailed'),
      );
    } finally {
      setLoading(false);
    }
  }, [initialTarget, scopeKey, t, token, userId]);
