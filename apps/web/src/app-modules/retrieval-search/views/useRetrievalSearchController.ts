...
const defaultClient: RetrievalSearchControllerClient = {
  listSources: listRetrievalSources,
  query: queryRetrieval,
};
...
export function useRetrievalSearchController({
  ...
  const [
    { error, loadingSources, response, searching, sourceError, sources },
    dispatch,
  ...
  const loadSources = useCallback(async () => {
    ...
    try {
      const result = await client.listSources(token, {
  ...
  }, [client, messages.authMissing, messages.sourcesLoadFailed, token]);
  ...
  const runSearch = useCallback(
    async (options?: {
      ...
      const nextSources =
        options?.selectedSources ?? selectedSourcesRef.current;
      ...
      try {
        ...
        if (
          caughtError instanceof RetrievalApiError &&
