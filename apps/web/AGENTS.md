...
- 현재 인증 후 웹 앱의 1차 UI 레퍼런스는 `ClickUp` 데스크톱 제품이다.
- 새 화면은 `좌측 앱 바 + 서브 사이드바 + 상단 작업 헤더 + 중앙 작업면` 구조를 기본으로 하고, 뷰 전환은 ClickUp처럼 `List / Board / Calendar / Gantt / Table` 밀도를 우선 참고한다.
...
- 브랜드 자산, 로고, 제품명, 고유 카피는 그대로 복제하지 말고 구조와 상호작용 패턴 중심으로 맞춘다.
