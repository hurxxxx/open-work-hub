...
Open ALM 문서중앙화 RAG는 “파일 업로드 후 embedding”으로 끝나지 않는다. 원본 파일과 검색용 파생 데이터를 분리하고, 부서/제품/문서등급/권한/버전이 검색 결과에 끝까지 따라오게 만들어야 한다.
...
| 단계 | 산출물 | 검수 기준 |
| --- | --- | --- |
| 1. 수집 | 원본 파일, PLM export, 회의록, 메일 템플릿 | 출처·소유자·문서등급이 있는가 |
...
_PDF, 스캔 문서, 표, 도면, PLM export는 같은 방식으로 색인하지 않고 parser, OCR, vision, SQL/API를 나눠 적용한다._
...
| PLM export | SQL/API 기준 원본 + 설명 필드만 RAG | 품번/BOM은 원천 시스템 우선 |
...
Open ALM의 metadata는 “있으면 좋은 설명”이 아니라 검색 품질과 보안의 핵심 입력이다.

| metadata | 쓰임 |
| --- | --- |
| `source_system` | 문서중앙화, PLM, 회의록, 메일 등 출처 구분 |
| `document_id`, `version` | 원문 추적과 증분 갱신 |
| `department`, `team` | 부서별 필터와 권한 |
| `product`, `part_no`, `project_code` | PLM/제품 검색 연결 |
...
| 계약 항목 | 설명 |
| --- | --- |
| 원천 시스템 | 문서중앙화, PLM, 메일, 회의록, 수동 업로드 등 |
...
Open ALM의 문서는 PDF, 스캔본, 표, 도면, PLM export가 섞인다. 파싱 품질은 자동 지표와 사람 샘플링을 함께 써야 한다.
...
| 정보 유형 | 처리 기준 |
| --- | --- |
| 개인정보 | 이름, 연락처, 사번, 이메일, 서명 이미지 등은 필요 최소한만 보존하고 masking 검토 |
