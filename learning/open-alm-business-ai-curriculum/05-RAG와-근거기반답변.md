...
Open ALM의 RAG는 문서중앙화, PLM, 회의록, 메일 초안, 보고서 작성과 연결된다. 이번 회차는 RAG의 기본 구조뿐 아니라 실제 운영에서 필요한 패턴과 실패 지점을 이해하는 데 초점을 둔다.
...
![Open ALM 근거 기반 답변 카드](./assets/diagrams/05a-grounded-answer-card.png)

_업무 답변은 결론, 근거 문서, 인용 위치, 경고, 후속 행동, 감사 정보를 함께 제공해야 검증 가능하다._

Open ALM 답변은 “그럴듯한 문장”이 아니라 검증 가능한 업무 산출물이어야 한다.
...
근거 기반 답변은 자연어 답변만 저장하면 운영하기 어렵다. Open ALM 답변은 UI와 평가가 읽을 수 있는 계약을 가져야 한다.
