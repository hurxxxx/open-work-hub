...
> **한 줄 요약.** Open ALM 품질은 데모 한 번의 인상이 아니라, 평가셋·trace·검색 지표·생성 지표·운영 지표로 계속 비교할 수 있어야 한다.

## 오늘의 목표

RAG/LLM 시스템은 모델을 바꾸고, 문서가 추가되고, 권한이 바뀌고, 사용자가 늘면서 품질이 계속 변한다. 이번 회차는 Open ALM PoC를 운영 가능한 시스템으로 만들기 위한 평가와 관측 체계를 설계한다.
