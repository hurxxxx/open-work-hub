...
**버킷(Bucket)**
객체 저장소의 최상위 폴더(실은 평면 컨테이너). MinIO의 `aidoo-uploads` 등(31장).
...
**아티팩트(Artifact)**
빌드 결과물. 이 교육 맥락에서는 Cowork의 "재방문 가능한 페이지"를 뜻하기도 함.
...
**이미지(Image, Docker)**
컨테이너를 만들기 위한 "설계도". Dockerfile로 굽는다.
