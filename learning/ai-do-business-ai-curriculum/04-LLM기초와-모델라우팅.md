...
이번 회차는 AI-DO PoC에서 사용할 수 있는 모델 선택 감각을 만든다. 최신 모델 이름을 외우는 것이 목적이 아니다. 모델 계열, 아키텍처, 강점, 배치 방식, 위험도를 구분할 수 있어야 한다.

| 배울 것 | AI-DO에서 연결되는 판단 |
| --- | --- |
...
AI-DO는 단순 챗봇이 아니다. 문서중앙화 RAG, PLM 통합검색, 문서 초안 작성, 메일 초안, 회의록, 승인 게이트가 함께 움직인다. 그래서 모델 선택도 “답변 품질” 하나가 아니라 다음 조건을 같이 본다.
...
![AI-DO 모델 지형과 라우팅](./assets/diagrams/04-model-landscape-routing.png)
...
| AI-DO 판단 | 사내 GPU에 안정적으로 올릴 작은 업무 모델 | GPU/메모리/동시성 검증 뒤 핵심 답변 모델 후보 |
...
| Code/coding agent | 코드 수정, 테스트, 리팩토링, PR 작성 | AI-DO 포털 기능 개발 보조 |
...
AI-DO에서는 이 변수들을 모델 카드에 넣어야 한다. “답변이 좋아 보인다”는 데모 감상이고, “우리 평가셋에서 citation accuracy가 6%p 올랐지만 p95 latency가 2.1초 증가했다”는 운영 판단이다.
...
AI-DO PoC에서는 처음부터 모든 요청을 대형 모델로 보내지 않는다. 문서 분류, query rewrite, tool input extraction은 작은 모델로 처리하고, 복잡한 근거 기반 답변이나 실패 진단은 reasoning 모델로 넘기는 cascade가 현실적이다.
