...
Open Work Hub는 검색만 하는 포털에서 문서 초안, 메일 초안, 업무 시스템 조회, 이슈 등록, 문서 생성으로 확장될 수 있다. 이때 모든 기능을 LLM에게 열어 주면 안 된다. capability 계약, read/write 구분, 승인 게이트, 감사 로그가 필요하다.
...
| 항목 | 예시 |
| --- | --- |
| 이름 | `records.lookup`, `documents.search`, `draft.email`, `issue.create` |
...
| 등급 | 설명 | Open Work Hub 예시 | 승인 정책 |
| --- | --- | --- | --- |
| Read-only | 데이터 조회만 수행 | 문서 검색, 업무 시스템 품번 조회 | 권한 확인 + 로그 |
...
| `records.lookup` | read | 오래된 업무 시스템 cache | 원천 시스템 timestamp 표시 |
| `records.search_changes` | read | 고객사/NDA 데이터 노출 | data grade routing |
...
| 방식 | 장점 | 주의점 | Open Work Hub 적용 |
| --- | --- | --- | --- |
| MCP tool | 모델/에이전트가 표준화된 tool 계약으로 기능 사용 | 서버별 권한, tool schema, 감사 로그 설계 필요 | 문서 검색, 초안 생성, 안전한 내부 tool 노출 |
| Backend API | 기존 서비스 권한 모델과 로직 재사용 | API 계약과 rate limit, idempotency 필요 | 업무 시스템 조회, 문서 원문 열기, 이슈 생성 |
...
| 대상 시스템 | 문서중앙화, 업무 시스템, PMS, 메일, 파일 저장소 |
