...
> **한 줄 요약.** Open Work Hub 품질은 데모 한 번의 인상이 아니라, 평가셋·trace·검색 지표·생성 지표·운영 지표로 계속 비교할 수 있어야 한다.

## 오늘의 목표

RAG/LLM 시스템은 모델을 바꾸고, 문서가 추가되고, 권한이 바뀌고, 사용자가 늘면서 품질이 계속 변한다. 이번 회차는 Open Work Hub PoC를 운영 가능한 시스템으로 만들기 위한 평가와 관측 체계를 설계한다.
...
| 유형 | 문항 수 예시 | 포함 이유 |
| --- | --- | --- |
| 사실 확인 | 10 | 특정 문서/업무 시스템 항목을 정확히 찾는지 |
...
| Representative coverage | 부서, 문서 유형, 업무 시스템 데이터, 보안 케이스를 균형 있게 포함 |
