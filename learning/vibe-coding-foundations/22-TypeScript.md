...
> 🔑 **한 마디로.** "엑셀 시트에 **셀 서식(숫자만, 날짜만)** 을 미리 지정해 놓고, 다른 사람이 **문자를 잘못 넣으면 빨간 경고가 뜨는** 기능을 프로그래밍에 도입한 것".

### 일상 비유로 확장하기

- **관공서 양식**: 주민번호 칸에 문자가 들어가면 접수가 안 된다. TypeScript는 **코드 제출 전 양식 검증**이다.
- **약 처방전**: "이 약은 성인 50mg, 소아 25mg" 라고 **단위·대상**이 찍혀 있어야 약사가 조제 실수를 막는다. 타입 = 단위·대상 표기.
- **레고 블록**: 블록에 돌기 모양·개수가 정해져 있어 **안 맞는 건 애초에 끼워지지 않는다**. TypeScript도 맞지 않는 타입은 편집기에서부터 막는다.
- **은행 거래명세 필드**: "계좌번호는 숫자만, 예금주는 글자, 금액은 소수점 2자리까지" — 이런 필드 규격을 **코드에 심어 둔 것**이 타입.
...
- **오해:** "TypeScript는 JavaScript보다 느리다."  
  **실제:** 브라우저가 실행하는 것은 **트랜스파일된 JavaScript**다. 타입은 빌드 때 사라지므로 **런타임 성능은 동일**.
- **오해:** "타입 선언이 코드량을 2배로 늘린다."  
  **실제:** 대부분은 **자동 추론**된다. 명시가 필요한 곳은 함수 경계·외부 데이터 정도.
- **오해:** "타입이 있으면 버그가 안 난다."  
  **실제:** 타입은 **"형태(shape)가 맞는지"** 만 검사한다. "값이 비즈니스 규칙상 맞는지"(예: 결제 금액이 음수인지)는 별도 검증이 필요. 타입은 **안전망**이지 **만능 방패**가 아니다.
- **오해:** "TypeScript를 배우려면 JavaScript부터 다 마스터해야 한다."  
  **실제:** AI 시대에는 둘을 같이 배우는 편이 빠르다. 에러 메시지·예제는 대부분 TS 맥락이다.
- **오해:** "타입만 고치면 버그가 고쳐진다."  
  **실제:** 타입 오류를 억지로 `any`로 덮으면 **버그를 숨기는 꼴**이다. 원인을 먼저 이해해야 한다.

---

## 3.6 🏢 업무 시나리오 — "AI 채팅 응답 포맷 변경"
...
- AI(Claude/Copilot)에게 "ChatResponse 바꿨으니 영향 받는 파일 전부 수정" 이라고만 지시해도, 타입이 명확할수록 AI가 정확히 해 준다.
...
1. VS Code 또는 Cursor에서 `apps/web/src/domains/` 아무 `.ts` 파일을 연다.
...
| **JavaScript(순수)** | 아예 타입이 없음 — **자유롭지만 대규모엔 위험**. | 스크립트·프로토타입엔 OK, 우리 규모엔 비추. |
| **ReScript, Elm** | 함수형 전통 언어. **문법이 낯설고 학습 곡선이 가파름**. | 생태계 작아 AI 지원도 약함. |

TypeScript는 2026년 기준 **웹 생태계의 사실상 표준**. 학습 투자 가치가 가장 큽니다. **React 19 생태계 전체가 TS 친화적**으로 정비되어 있고, LLM(대규모 언어 모델)의 코드 생성 정확도도 TS가 가장 높습니다.
...
- Flow: "페이스북이 밀었지만 업계가 따라주지 않음" — 지금 붙잡으면 **갈수록 외로워진다**.
- JSDoc + tsc: "주석으로만 타입 표기" — **경량이라 도구 의존을 피하고 싶을 때** 유용. 대규모엔 한계.
- 순수 JS: "타입 없이 달린다" — **소규모 스크립트·프로토타입**에는 여전히 적합.
- ReScript/Elm: "함수형 애호가의 천국" — **학습 곡선과 팀 전파 비용**이 높아 AX TF 같은 업무 도메인 팀엔 부담.

---

## 10.7 🏢 추가 업무 시나리오 — "신규 입사자의 첫 PR"
...
이 시나리오가 **"타입 = 살아 있는 문서 + 실수 방지 도구"** 라는 개념을 실감하게 합니다.
...
6. **비유로 답하기:** TypeScript의 타입 검사를 "관공서 양식 검증"에 비유할 때, `any`는 무엇에 해당할까요?
7. **타입이 있었다면 막을 수 있었을 사내 사고**를 하나 상상해 보세요(예: 결제 금액을 문자열로 저장해 계산이 어긋난 사례).
