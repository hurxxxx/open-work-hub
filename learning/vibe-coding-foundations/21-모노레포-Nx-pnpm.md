...
> 🔑 **한 마디로.** 하나의 저장소에 웹·API·워커 코드를 함께 두고, Nx가 빌드/테스트/캐시를 중앙에서 조율하며 pnpm이 의존성 설치를 효율화하는 방식이다.

### 모노레포 구조를 빠르게 이해하기

- `apps/web`, `apps/api` 같은 앱은 독립 유지하되, Nx가 공통 태스크(빌드/테스트) 실행 순서를 자동 조정합니다. pnpm은 전역 저장소 + 링크 방식으로 중복 설치를 줄입니다.
- ESLint/TSConfig 같은 공통 규칙은 루트에서 공유하고, 앱 기능 코드는 각 앱 폴더에서 독립적으로 변경합니다.
- 공통 도구는 공유하지만 배포 단위와 기능 책임은 앱 단위로 유지하는 것이 모노레포 운영의 핵심입니다.
...
### 2.3 ⚠️ pnpm에서 자주 하는 오해

- **"pnpm이든 npm이든 결과물은 같으니 아무거나 써도 된다"** — 모노레포에서 pnpm과 npm을 섞어 쓰면 `node_modules`가 엉켜 **"방금까지 되던 게 안 되는"** 재앙이 생긴다. 팀은 반드시 하나로 통일해야 한다.
- **"`pnpm install`이 오래 걸리니까 pnpm이 느린 것이다"** — 최초 1회 설치(글로벌 저장소 채우기) 이후로는 **새 프로젝트마다 거의 순간**에 끝난다. 디스크·네트워크 절약분이 크다.
- **"하드링크라 파일을 지우면 원본도 사라진다"** — 반대다. 여러 프로젝트가 **같은 파일을 공유**할 뿐, 한 쪽을 지워도 다른 쪽은 멀쩡하다.
...
## 3.5 ⚠️ Nx에서 바로잡고 싶은 오해

- **"Nx를 써야만 모노레포가 된다"** — pnpm 워크스페이스만 있어도 모노레포다. Nx는 **속도·일관성 최적화 도구**일 뿐이다. 다만 규모가 커질수록 Nx 없이는 빌드가 고통스러워진다.
- **"Nx 캐시가 이상하면 `rm -rf node_modules`로 풀어야 한다"** — `pnpm nx reset` 한 줄이면 Nx 캐시만 깨끗이 비워진다.
- **"Nx는 Angular 전용 도구다"** — 과거엔 그랬지만 현재는 React·Vite·Python·Go까지 플러그인이 있다. 우리도 **파이썬 앱**(`apps/api`, `apps/worker`)까지 Nx 타겟으로 관리한다.
- **"affected 빌드는 내부 동작을 몰라도 자동으로 맞게 계산된다"** — Nx는 **Git diff**를 기준으로 "어느 파일이 바뀌었는지"를 판단합니다. 커밋 품질이 낮으면 affected 계산 정확도도 떨어집니다.
...
| **Lerna** | **과거에 널리 쓰이던 도구**. 지금은 Nx가 인수해 유지 중. | 레거시. 새 프로젝트에는 권장하지 않음. |
...
> 📝 **비개발자용 힌트.** Turborepo는 설정이 단순한 JS 중심 도구, Nx는 다언어·대규모 태스크 관리에 강한 도구, Bazel은 초대형 조직용 고복잡도 도구로 이해하면 됩니다.
...
6. pnpm의 전역 저장소(store)와 각 프로젝트 `node_modules`의 관계를 설명해 보세요.
