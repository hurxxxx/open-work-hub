...
- **오해:** "에디터 하나 만드는 게 그렇게 어렵나? `<textarea>` 쓰면 되잖아."  
  **실제:** `<textarea>`는 **서식·이미지·실시간 협업이 불가능**하다. 블록 에디터는 IME, 복사·붙여넣기, 실행 취소까지 고려한 **소형 운영체제급** 복잡도다.
- **오해:** "블록 에디터는 마크다운과 같다."  
  **실제:** 블록 에디터는 **블록 단위 구조화 문서**(이미지·테이블·임베드가 1급 요소), 마크다운은 그냥 **텍스트 포맷**이다. 블록 에디터가 마크다운을 입/출력할 순 있지만 내부 모델이 다르다.
- **오해:** "BlockNote를 바꾸고 싶으면 소스를 뜯어야 한다."  
  **실제:** **슬래시 메뉴·단축키·커스텀 블록**은 공식 API로 확장 가능. 대부분은 설정으로 해결된다.
...
- **오해:** "CRDT는 충돌을 물리적으로 막는다."  
  **실제:** **의미상의 충돌**(두 사람이 서로 다른 의도로 같은 문장을 고침)은 막지 못한다. CRDT는 **"데이터 구조가 깨지지 않음"** 만 보장. 의도 충돌은 사람이 해결해야 한다.
- **오해:** "CRDT가 있으면 서버 저장이 필요 없다."  
  **실제:** 사용자가 모두 오프라인이 되면 문서가 사라진다. **주기적 스냅샷**을 DB/MinIO에 저장하는 것이 필수.
- **오해:** "Yjs는 작은 문서만 다룬다."  
  **실제:** 수천 블록까지 실무 가능. 다만 10만 라인 급은 UX 설계로 **여러 페이지 분할**이 현실적.
- **오해:** "undo/redo는 서버가 관리한다."  
  **실제:** Yjs는 **사용자별 undo 스택**을 로컬에 둔다. "내 undo는 내 편집만 되돌린다"가 원칙.
...
## 8.5 🏢 업무 시나리오 — "회의록 초안을 5명이 동시에 다듬기"
