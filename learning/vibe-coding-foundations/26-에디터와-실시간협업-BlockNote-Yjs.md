...
> 🔑 **한 마디로.** "**여러 명이 구글 독스처럼 동시에 편집**해도 글자가 섞이거나 덮이지 않도록, **수학적으로 충돌이 안 나는 자료구조**를 깔아 놓은 것". UI는 Notion과 비슷, 엔진은 CRDT, 통로는 WebSocket.

### 합주, 포스트잇 보드, 주방 주문서로 보기

- **음악 스튜디오 합주:** 각 연주자(사용자)가 자기 악기(로컬 사본)를 연주하고, **믹싱 콘솔(Yjs)** 이 모든 트랙을 **순서와 무관하게** 합쳐 최종 음원을 만든다. 누가 먼저 연주했든 결과가 같음.
- **포스트잇 보드:** 모든 포스트잇에 **고유 ID + 위치 좌표**가 있어서 두 사람이 같은 자리에 붙여도 **시스템이 알아서 정리**한다. 그게 CRDT의 감각.
- **식당 주방 주문서:** 홀 직원 여러 명이 동시에 주문을 추가해도 주방의 **주문 대장(Y.Doc)** 은 자동 병합된다.
- **블록식 메모지:** 각 메모지는 **블록(문단·제목·목록·표)** 하나에 대응. 드래그로 순서를 바꿔도 블록 자체는 그대로다.
...
- **"에디터 하나 만드는 게 그렇게 어렵나? `<textarea>` 쓰면 되잖아"** — `<textarea>`는 **서식·이미지·실시간 협업이 불가능**하다. 블록 에디터는 IME, 복사·붙여넣기, 실행 취소까지 고려한 **소형 운영체제급** 복잡도다.
- **"블록 에디터는 마크다운과 같다"** — 블록 에디터는 **블록 단위 구조화 문서**(이미지·테이블·임베드가 1급 요소), 마크다운은 그냥 **텍스트 포맷**이다. 블록 에디터가 마크다운을 입/출력할 수는 있지만 내부 모델이 다르다.
...
|---|---|
| **BlockNote** | **Notion과 가장 비슷한 UX**(슬래시 메뉴·드래그 블록), 협업 기본 내장. 우리 선택. |
| TipTap | **ProseMirror 위의 얇은 래퍼** — 유연하지만 블록 UX·슬래시 메뉴는 직접 구현해야 함. |
...
1. **Notion-like UX를 거의 공짜로 얻음** — 슬래시 메뉴·블록 드래그·테이블이 기본.
2. **Yjs 통합이 1급 지원** — 다른 에디터는 협업을 별도로 붙여야 함.
...
이 한 문서에 **BlockNote(UX) + Yjs(CRDT) + y-websocket(클라이언트) + ypy-websocket(서버) + DocsCollabHub(방 관리) + MinIO(스냅샷)** 이 모두 협연한다.
...
2. **"실시간이 공짜"** — 다른 에디터는 협업 기능을 별도로 붙여야 함.
...
6. **비유로 답하기:** CRDT를 **합주 중인 음악 스튜디오**에 비유하면, "방(room)"과 "스냅샷"은 각각 무엇에 해당할까요?
