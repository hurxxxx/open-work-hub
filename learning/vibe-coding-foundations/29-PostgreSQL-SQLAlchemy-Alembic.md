...
Alembic은 그 이력을 **파일로 버전 관리**합니다. Git 커밋처럼 DB 변경을 커밋하는 셈입니다.
...
책의 "색인"처럼, 특정 컬럼 값으로 빠르게 찾도록 도와주는 별도 자료구조. `B-Tree`가 기본.
...
1. ORM의 장점과 단점을 각각 두 가지씩 말해 보세요.
2. N+1 문제는 무엇이고, SQLAlchemy에서 어떻게 해결하나요?
