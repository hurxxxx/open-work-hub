...
- **"파일은 그냥 서버 디스크에 저장해도 충분하다"** — 서버가 여러 대가 되는 순간 "A 서버에는 있는데 B 서버에는 없는 파일"이 생깁니다. NFS로 공유하면 성능·장애 전파가 문제입니다. 객체 저장소가 정답입니다.
- **"업로드는 무조건 API 서버를 경유해야 한다"** — Presigned URL을 쓰면 클라이언트가 MinIO로 **직접** 업로드/다운로드합니다. API 서버는 "허가증"만 발급합니다.
