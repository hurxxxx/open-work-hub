...
> 🔑 **한 마디로.** "**이케아 완제 가구(Mantine) + 전문 목수 뼈대(Radix) + 페인트·시트지(Tailwind)** 를 섞어 사무실을 꾸미는 방식". 한 벤더 카탈로그에 갇히지 않고 **필요한 층위에서 최적 도구를 뽑는다**.

### 일상 비유로 확장하기

- **요리 현장:** Mantine = 밀키트(반조리 — 빠름), Radix = 정육점에서 받은 손질된 고기(맛내기는 내가 — 자유도 ↑), Tailwind = 양념·소스(마무리 조미).
- **인테리어:** Mantine = 시스템 가구 패키지, Radix = 붙박이장 뼈대만 받은 상태, Tailwind = 벽지·쿠션·조명 같은 소품.
- **옷차림:** Mantine = 기성복 정장(빠르고 일관), Radix = 재단만 된 옷감(핏을 내 맘대로), Tailwind = 타이·포켓치프(디테일 조정).
- **PPT 제작:** Mantine = 회사 표준 템플릿(제목·표·차트 슬라이드 완성형), Radix = 공백 슬라이드 위 정렬 가이드, Tailwind = 색·폰트·여백의 미세 조정.
...
쉽게 비유하면:
- **Mantine** = 이케아에서 산 완제품 가구
- **Radix** = 전문 목수가 짠 맞춤 가구의 **뼈대**
- **Tailwind** = 페인트·시트지 같은 **마감재**
...
**Mantine 8** 은 Vitaly Rtischev가 이끄는 오픈소스 프로젝트입니다. React 생태계에서 가장 완성도 높은 **컴포넌트 + 훅 모음** 중 하나입니다. 2025년 기준 GitHub 별 26k+, 활발한 유지보수.
...
- **오해:** "UI 라이브러리 여러 개를 쓰면 번들 크기가 폭발한다."  
  **실제:** Vite가 **Tree shaking(실제로 쓰지 않은 코드를 빌드에서 제거)** 을 해 주므로, 3개를 나눠 써도 안 쓰는 컴포넌트는 포함되지 않는다.
- **오해:** "Tailwind는 CSS의 대체제다."  
  **실제:** Tailwind는 **CSS 위의 유틸리티 레이어**다. CSS를 이해할수록 더 잘 쓸 수 있다.
- **오해:** "Mantine을 쓰면 디자인이 획일화된다."  
  **실제:** Mantine에는 **테마 시스템**이 있어 색·폰트·모서리 반경을 일괄 조정 가능. 추가로 Tailwind로 미세 조정.
- **오해:** "Radix도 예쁜 컴포넌트를 준다."  
  **실제:** Radix는 **"스타일 0%"** 다. 동작·접근성만 준다. 그래서 **Headless(머리 없는)** 라고 불린다.
- **오해:** "접근성은 장애인을 위한 것이지 나머지 사용자에겐 무관하다."  
  **실제:** 접근성 잘 된 UI는 **키보드 사용자·고령자·일시적 부상자** 모두에게 이득이고, **자동화 테스트(E2E)** 도 쉬워진다.
...
> 🔮 **2026 트렌드.** Tailwind v4는 **Oxide 엔진(Rust)** 으로 빌드 성능이 크게 개선되고, **CSS-first 설정**(JS config 없이 CSS 파일만으로 테마 정의)이 주류가 되는 흐름입니다. 이 프로젝트도 Vite 플러그인 + CSS-first 구조를 채택해 설정 파일 수를 최소화하고 있습니다.
...
이런 하이브리드는 **2025년의 주류 선택**입니다. 초보자에게는 "혼란스러워 보이지만" 실제로는 각 도구의 강점을 모두 취하는 현명한 전략입니다.
...
- Mantine과 Radix는 접근성을 **기본 원칙**으로 설계됨. 그냥 쓰면 대부분 OK.
- `eslint-plugin-jsx-a11y`가 접근성 위반을 **정적 검사(static check — 실행하지 않고 코드만 보고 문제를 잡기)** 함. 우리 설정에 포함.
...
이 프로젝트는 아이콘을 **lucide-react 0.546**에서 받습니다. Lucide는 **Feather Icons의 후계**로, 500+ 개의 오픈소스 아이콘을 제공합니다. Tree shaking 덕분에 **실제로 쓴 아이콘만** 번들에 포함됩니다. `import { Search, Plus } from 'lucide-react';` 한 줄로 끝.

---

## 7.2 🏢 업무 시나리오 — "대시보드 카드 한 벌 제작"
...
1. 한 UI 라이브러리로 통일하지 않고 세 개를 섞는 이유는?
2. "Headless UI"(Radix 스타일)가 무엇을 제공하고 무엇을 제공하지 않는지 한 줄로 설명하세요.
3. Tailwind에 대한 처음 인상("HTML 더러워짐")이 실제로는 어떻게 뒤집히는지 두 가지를 쓰세요.
4. `clsx`와 `tailwind-merge`가 각각 해결하는 문제를 구분해 설명하세요.
5. 접근성을 지키기 위해 팀이 할 수 있는 가장 쉬운 실천 두 가지를 들어 보세요.
6. **비유로 답하기:** Mantine / Radix / Tailwind를 **"식당의 어떤 역할"** 에 비유해 각각 한 문장씩 설명하세요.
7. AI 승인 모달(3.5 시나리오)에서 Mantine Modal 대신 Radix Dialog를 쓴 이유를 한 문장으로 요약하세요.
