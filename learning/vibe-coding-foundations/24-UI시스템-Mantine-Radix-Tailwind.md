...
| 라이브러리 | 특징 |
|---|---|
| **Mantine** | 컴포넌트 풍부, 사용 쉬움. 우리 선택. |
| MUI (Material UI) | 구글 머티리얼 디자인. 가장 유명. 스타일이 무겁고 테마 변경 번거로움. |
| Chakra UI | 개발 편함. 최근 v3에서 API 대격변으로 선호도 하락. |
| Ant Design | 기업용 폼이 강점. 중국권 대기업에서 인기. 디자인이 고유. |
| shadcn/ui | 복사-붙여넣기 식 컴포넌트. 최근 가장 뜨거움. 커스터마이징 자유도 최고. |
...
- **Vite 플러그인(`@tailwindcss/vite`) 통합** — 설정 파일 거의 필요 없음.
- **Oxide 엔진** — Rust 기반으로 10~100배 빠른 빌드.
- **CSS 안에서 테마 정의** — `@theme { --color-brand: ... }`.
- **컨테이너 쿼리 기본 지원** — 요즘 CSS의 핫토픽.
...
| 방식 | 특징 | 비고 |
|---|---|---|
| **Tailwind** | 유틸리티 퍼스트 | 우리 선택 |
| CSS Modules | 컴포넌트별 CSS 파일 | 전통적, 안전. |
| styled-components / Emotion | JS 안에 CSS | 런타임 오버헤드. 인기 하락. |
| vanilla-extract | 타입 안전 CSS-in-JS | 떠오름. |
| CSS 그대로 | 그대로 | 큰 규모엔 혼잡. |
...
**Accessibility(a11y)** 는 "장애가 있는 사용자도 쓸 수 있는가"입니다. 사내용이라고 소홀하면 안 됩니다(색맹, 키보드 전용 사용자, 스크린 리더 사용자 등).

- Mantine과 Radix는 접근성을 **기본 원칙**으로 설계됨. 그냥 쓰면 대부분 OK.
- `eslint-plugin-jsx-a11y`가 접근성 위반을 **정적 검사**. 우리 설정에 포함.
