...
- **Vite = 탑차 vs 컨테이너선(Webpack):** 컨테이너선은 화물 전부 실어야 출항(전체 번들). 탑차는 **필요한 짐만 골라 즉시 출발**(ES 모듈만 그때그때 제공). 재시동 시간이 비교가 안 됨.
...
- **오해:** "React는 프레임워크다."  
  **실제:** 공식적으로는 **라이브러리**다. 라우팅·폼·상태는 별도 라이브러리를 조합해야 한다. (우리는 React Router v7, Mantine, 자체 훅 조합 — 25장 참고.)
- **오해:** "컴포넌트를 잘게 쪼갤수록 성능이 좋아진다."  
  **실제:** 꼭 그렇지 않다. 과하게 쪼개면 **prop 전달 지옥**이 생기고, 상태 위치가 꼬인다. "**한 화면에서 함께 바뀌는 것은 한 컴포넌트**" 원칙이 기본.
- **오해:** "useState는 변수 선언이다."  
  **실제:** useState는 "**React에 변경을 알려 주는 통로**"다. 그냥 변수에 대입하면 화면이 안 바뀐다.
- **오해:** "React가 화면 전체를 다시 그린다."  
  **실제:** **가상 DOM(Virtual DOM — 실제 DOM의 가벼운 메모리 사본)** 비교를 거쳐 **달라진 부분만** 업데이트한다.
- **오해:** "Hook 안에서 조건문을 써도 된다."  
  **실제:** **Rules of Hooks** 위반이다. 훅은 반드시 **최상위에서 같은 순서로 호출**되어야 한다(React가 호출 순서로 훅을 식별하기 때문).
...
- **오해:** "Vite는 개발용이고 운영에는 못 쓴다."  
  **실제:** 개발 서버와 **운영 빌드**를 모두 담당한다. 운영 빌드는 **Rollup** 기반으로 검증된 최적화를 수행.
- **오해:** "Vite가 TypeScript 타입 검사를 해 준다."  
  **실제:** Vite는 **트랜스파일(언어 변환)만** 한다. 타입 검사는 `tsc --noEmit` 혹은 에디터가 별도로. 이 구분이 속도 비결이다.
- **오해:** "HMR(Hot Module Replacement, 코드 저장 시 새로고침 없이 반영)이 있으니 상태가 영원히 유지된다."  
  **실제:** 컴포넌트 구조를 크게 바꾸면 리로드가 필요할 수 있다.
...
## 9.5 🏢 업무 시나리오 — "회의 예약 폼에 '중복 예약 알림' 추가"
