...
> **한 줄 요약.** React는 **"화면을 데이터의 함수로 그리는"** 라이브러리이고, Vite는 React 앱의 개발·빌드 속도를 크게 높여 주는 도구다. 이 조합이 2025~2026 프런트엔드의 사실상 표준이다.

> 🔑 **한 마디로.** React는 상태 변화에 맞춰 UI를 선언적으로 재렌더링하는 라이브러리이고, Vite는 개발/빌드 속도를 높이는 프런트엔드 도구 체인이다.

### 핵심 개념 빠르게 정리

- React는 DOM 조작 명령을 직접 쓰기보다 상태를 변경하면 화면을 다시 계산해 반영하는 선언형 모델을 사용합니다.
- 상태(state)가 바뀌면 해당 상태를 참조하는 컴포넌트가 자동 재렌더링됩니다.
- Vite는 개발 중 전체 번들 대신 필요한 모듈만 즉시 제공해 초기 기동·리로드 시간을 크게 줄입니다.
...
- **2015~**: React + Redux, Vue, Angular의 3강 구도.
...
`setCount`가 호출되면 React가 **이 컴포넌트만 다시 그립니다**. 이 자동 업데이트가 React의 핵심 렌더링 메커니즘입니다.
...
- **"컴포넌트를 잘게 쪼갤수록 성능이 좋아진다"** — 꼭 그렇지 않다. 과하게 쪼개면 prop 전달 복잡도가 커지고 상태 위치가 꼬인다. "**한 화면에서 함께 바뀌는 것은 한 컴포넌트**" 원칙이 기본이다.
- **"useState는 변수 선언이다"** — useState는 "**React에 변경을 알려 주는 통로**"다. 일반 변수에 대입하면 화면이 바뀌지 않는다.
...
| **Svelte / SvelteKit** | **문법이 간결**, 가상 DOM 없음. 런타임 코드도 작음. | 유망한 대안이지만 생태계가 아직 얕음. |
...
5. 또 **JS·CSS** 필터로 보면 `index-<해시>.js`, `vendor-<해시>.js`가 초기 로드 시에만 내려온다.
...
7. React의 "상태 변경 시 자동 화면 갱신" 동작을 데이터 흐름 관점으로 설명해 보세요.
