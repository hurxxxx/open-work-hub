...
> **🔑 한 마디로.** 오래 걸리는 일을 "접수증만 끊고 뒤로 돌리는" 시스템. 접수 창구는 즉시 비고, 뒤편 작업자들이 대기줄에서 하나씩 꺼내 처리합니다.

### 일상 비유

- **세탁소**: 고객이 옷을 맡기면 접수증을 받고(즉시 응답), 실제 세탁은 뒤편에서 따로 진행(워커). 옷은 빨래통(큐=Redis)에 쌓였다가 차례로 돌아갑니다. 세탁이 끝나면 문자 알림(알림/폴링).
- **주방의 주문서 클립**: 홀 서버가 주문서를 클립에 꽂으면(큐에 적재), 주방이 순서대로 꺼내 요리. 홀(웹 서버)은 주문 받는 일만 빨리 처리.
- **택배 집하장**: 물건을 집하장(Redis)으로 보내면, 기사(워커)가 수거해 배송. 집하장이 막히지 않는 한 전체 시스템이 밀리지 않습니다.

### ⚠️ 흔한 오해

- **오해**: "FastAPI의 `BackgroundTasks`로 충분하다."
  **실제**: 같은 프로세스 내에서 실행되므로 서버 재시작 시 작업 유실, 부하 분산 불가, 재시도 없음. 짧은 "fire and forget"엔 쓸 수 있지만 장기 작업엔 부적합.
- **오해**: "async/await과 Celery는 같은 것이다."
  **실제**: async는 **한 프로세스 안의 동시성**, Celery는 **요청과 별개의 프로세스에서 나중에 실행**. 층위가 완전히 다릅니다.
- **오해**: "Redis는 메모리라서 언제든 데이터가 날아간다."
  **실제**: AOF(Append Only File, 모든 쓰기를 로그로 기록) 또는 RDB 스냅샷으로 영속화 옵션이 있습니다. 다만 캐시 용도는 사라져도 괜찮게 설계해야 합니다.
- **오해**: "Celery만 깔면 자동으로 신뢰성 있는 큐가 된다."
  **실제**: **멱등성**(같은 작업 여러 번 실행해도 결과가 같음)·재시도·DLQ(Dead Letter Queue)를 직접 설계해야 합니다. 그냥 쓰면 중복 결제·중복 발송 같은 사고가 납니다.
- **오해**: "Celery는 오래된 기술이라 피해야 한다."
  **실제**: 2026년에도 Python 비동기 큐의 기본 선택지입니다. Dramatiq·arq·RQ·Temporal·Prefect 같은 대안이 있지만, 생태계·예제·모니터링 도구(Flower)의 두께는 Celery가 우위.
...
**Celery 5.5** 는 2009년부터 존재하는 파이썬용 분산 **작업 큐(task queue, 할 일 대기줄)** 프레임워크입니다. 가장 오래되고 널리 쓰이는 선택지. 이 프로젝트의 워커 앱 코드 위치는 `apps/worker`.
