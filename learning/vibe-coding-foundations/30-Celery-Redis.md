...
- **주방의 주문서 클립**: 홀 서버가 주문서를 클립에 꽂으면(큐에 적재), 주방이 순서대로 꺼내 요리합니다. 홀(웹 서버)은 주문 받는 일만 빨리 처리하면 됩니다.
- **택배 집하장**: 물건을 집하장(Redis)으로 보내면, 기사(워커)가 수거해 배송합니다. 집하장이 막히지 않는 한 전체 시스템이 밀리지 않습니다.
...
- **"Celery만 깔면 자동으로 신뢰성 있는 큐가 된다"** — **멱등성**(같은 작업 여러 번 실행해도 결과가 같음)·재시도·DLQ(Dead Letter Queue)를 직접 설계해야 합니다. 그냥 쓰면 중복 결제·중복 발송 같은 사고가 납니다.
