...
Compose가 "옛 도구"라는 건 오해입니다. 단일 호스트 배포는 Compose로도 충분한 경우가 많고, 큰 규모는 Kubernetes, 그 중간은 Nomad·ECS 같은 대안을 검토합니다.
...
- **Apache httpd**: 전통 강자지만 이 프로젝트는 설정 단순성과 자료 접근성을 고려해 Nginx를 선택.
...
1. **팀 경험**: Nginx 설정은 인터넷에 자료가 많고, AI 도구도 비교적 잘 설명합니다. 누구에게 물어도 답을 얻기 쉽습니다.
