...
> 🔑 **한 마디로.** "**내 노트북에서는 돼요**" 문제를 기술로 없애는 것. 컨테이너는 앱의 실행 환경을 **통째로 포장**하는 방식이고, Nginx는 그 포장박스들 앞에 서 있는 **안내 데스크**다.

### 🧱 일상 비유 — 컨테이너, Compose, Nginx

1. **이사 상자**: 컨테이너는 "앱 + 그 앱이 의존하는 모든 것(파이썬, 라이브러리, 시스템 파일)"을 담은 박스다. 트럭을 바꿔도(서버를 바꿔도) 내용물은 흔들리지 않는다.
2. **케이터링 주문서**: Compose는 "상자 7개를 이 순서로 이렇게 연결해 테이블에 올려 주세요"라는 단일 주문서. 다음에 같은 주문서를 그대로 내면 같은 상 차림이 재현된다.
3. **빌딩 1층 안내 데스크**: Nginx는 건물에 들어오는 모든 방문객(HTTP 요청)을 받아 "이 방문은 3층 API 사무실, 저 방문은 5층 협업 서버"로 보내는 안내원. 손님은 사무실 번호를 몰라도 된다.

### ⚠️ 흔한 오해

1. **"Docker = 가상 머신(VM)"**: 다릅니다. VM은 운영체제 전체를 복제하고, 컨테이너는 커널은 공유하고 **프로세스와 파일시스템만 격리**합니다. 그래서 훨씬 가볍고 빠릅니다.
2. **"Compose는 옛날 도구"**: 아닙니다. 2024년 Compose V2(Go 재작성)로 현역이고, 단일 호스트 배포의 **가성비 최고 선택**입니다. Kubernetes가 무조건 상위 호환인 것이 아닙니다.
...
12 Factor App에서 말하는 핵심 가치. 내 노트북과 운영 서버가 **가능한 한 같은 그림**이어야 배포 사고가 줄어듭니다. Compose는 이 가치를 자연스럽게 구현하는 도구입니다.
...
6. "dev-prod parity"가 구체적으로 무엇을 의미하며, AIDOO Portal에서 이 원칙이 지켜지지 않으면 어떤 사고가 날 수 있을지 한 가지 예를 들어 보세요.
7. Caddy·Traefik이 매력적으로 보이지만 이 프로젝트가 Nginx를 유지하는 이유를 두 가지 이상 들어 보세요.
