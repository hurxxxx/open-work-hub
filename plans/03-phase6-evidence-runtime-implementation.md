...
- Current PR/stage: Phase 0-BC hidden graph node truncation handling after `502f83c`.
- Last completed: Hidden graph nodes that terminate with `finish_reason=length` now become failed node outputs with `finish_reason:length` in the summary instead of being treated as completed evidence. Added graph stream regression coverage to prove the failed node is reflected in SSE metadata and persisted invocation status.
- In progress: None.
...
- Files touched in Phase 0-BC: `apps/api/src/aidoo_api/domains/ai/router.py`, `apps/api/tests/test_ai_stream.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/router.py tests/test_ai_stream.py`; `cd apps/api && uv run pytest tests/test_ai_stream.py::test_chat_stream_graph_execution_marks_truncated_hidden_node_failed tests/test_ai_stream.py::test_chat_stream_graph_execution_adapter_runs_when_enabled tests/test_ai_stream.py::test_chat_stream_graph_execution_verifier_failure_degrades_writer_prompt -q`; `scripts/phase6-runtime-regression.sh -q`.
