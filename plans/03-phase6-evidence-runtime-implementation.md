...
- Current PR/stage: Phase 0-AA external mock provider evidence after `708dfda`.
- Last completed: Wired completed mock external search execution into `EvidencePacket` as trace-safe provider metadata. Query plan now carries `external_search_used`, `external_search_provider`, and `sanitized_query_ref`; the evidence item stores only query digest, result count, source kind, provenance, untrusted trust level, and public-web authority class. Mock external metadata alone does not mark a packet ready for grounded writing; node evidence is still required. Graph node inputs, writer prompts, and graph node execution summaries now receive the same provider evidence when explicit execution flags are enabled. No prompt/query body or raw provider output is persisted or rendered as provider evidence.
- In progress: None.
- Next exact task: Add deterministic mock result normalization/cache boundaries for external search so the same sanitized query digest yields stable provider evidence within a run before any real provider adapter is introduced.
- Files touched in Phase 0-AA: `apps/api/src/aidoo_api/domains/ai/router.py`, `apps/api/src/aidoo_api/domains/ai/runtime/contracts.py`, `apps/api/src/aidoo_api/domains/ai/runtime/graph_execution.py`, `apps/api/tests/test_ai_runtime_contracts.py`, `apps/api/tests/test_ai_stream.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/runtime/contracts.py src/aidoo_api/domains/ai/runtime/graph_execution.py src/aidoo_api/domains/ai/router.py tests/test_ai_runtime_contracts.py tests/test_ai_stream.py`; `cd apps/api && uv run pytest tests/test_ai_runtime_contracts.py::test_graph_evidence_packet_includes_external_mock_search_metadata tests/test_ai_runtime_contracts.py::test_external_mock_search_metadata_does_not_make_packet_ready_alone tests/test_ai_runtime_contracts.py::test_graph_node_input_builder_is_deterministic_and_scoped tests/test_ai_stream.py::test_chat_stream_graph_execution_uses_external_mock_evidence_when_enabled`; `cd apps/api && uv run pytest tests/test_ai_runtime_contracts.py tests/test_ai_stream.py`.
