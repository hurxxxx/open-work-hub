...
- Current PR/stage: Phase 0-Z external mock execution hooks after `820099c`.
- Last completed: Added opt-in mock execution hooks for `external_planner_v0` and `external_search_v0`. The execution path is disabled by default with `AIDOO_AI_EXTERNAL_PLANNER_EXECUTION_ENABLED=false` and `AIDOO_AI_EXTERNAL_SEARCH_EXECUTION_ENABLED=false`; when enabled, it still requires the existing egress allow decision and ready request envelope. Planner execution records only mock provider, status, planned-agent count, intent/output hints, and raw-output persistence state. Search execution records only mock provider, status, query digest, result count, source kinds, and raw-output persistence state. No OpenAI/Anthropic API is called and no prompt/query body is persisted in summaries.
- In progress: None.
- Next exact task: Make mock external planner/search outputs available to graph execution as trace-safe provider evidence, still behind explicit execution flags and without changing the default single-loop response path.
- Files touched in Phase 0-Z: `apps/api/src/aidoo_api/core/settings.py`, `apps/api/src/aidoo_api/domains/ai/agent.py`, `apps/api/src/aidoo_api/domains/ai/approvals.py`, `apps/api/src/aidoo_api/domains/ai/events.py`, `apps/api/src/aidoo_api/domains/ai/router.py`, `apps/api/src/aidoo_api/domains/ai/runtime/__init__.py`, `apps/api/src/aidoo_api/domains/ai/runtime/external_planner.py`, `apps/api/src/aidoo_api/domains/ai/runtime/external_search.py`, `apps/api/src/aidoo_api/domains/ai/runtime/persistence.py`, `apps/api/src/aidoo_api/domains/ai/runtime/routing.py`, `apps/api/tests/fixtures/envelope_schema.json`, `apps/api/tests/test_ai_runtime_external_planner.py`, `apps/api/tests/test_ai_runtime_external_search.py`, `apps/api/tests/test_ai_runtime_settings.py`, `apps/api/tests/test_ai_stream.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/core/settings.py src/aidoo_api/domains/ai/runtime/external_planner.py src/aidoo_api/domains/ai/runtime/external_search.py src/aidoo_api/domains/ai/runtime/__init__.py src/aidoo_api/domains/ai/runtime/routing.py src/aidoo_api/domains/ai/router.py src/aidoo_api/domains/ai/agent.py src/aidoo_api/domains/ai/approvals.py src/aidoo_api/domains/ai/events.py src/aidoo_api/domains/ai/runtime/persistence.py tests/test_ai_runtime_external_planner.py tests/test_ai_runtime_external_search.py tests/test_ai_stream.py tests/test_ai_runtime_settings.py`; `cd apps/api && uv run pytest tests/test_ai_runtime_external_planner.py tests/test_ai_runtime_external_search.py tests/test_ai_runtime_settings.py tests/test_ai_events.py::test_envelope_schema_snapshot_matches`; `cd apps/api && uv run pytest tests/test_ai_approvals.py tests/test_ai_runtime_persistence.py tests/test_ai_events.py::test_envelope_schema_snapshot_matches tests/test_ai_runtime_external_egress.py tests/test_ai_runtime_external_planner.py tests/test_ai_runtime_external_search.py tests/test_ai_runtime_routing.py`; `cd apps/api && uv run pytest tests/test_ai_stream.py`.
