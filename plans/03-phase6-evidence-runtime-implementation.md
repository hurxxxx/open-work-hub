...
- Current PR/stage: Phase 0-BD invocation sequence DB invariant after `d8aa579`.
- Last completed: Added an Alembic head migration and ORM check constraint so `ai_agent_invocations.invocation_seq` must be nonnegative, matching the runtime contract and trace event ordering invariants. Added persistence regression coverage for rejected negative invocation sequences.
- In progress: None.
...
- Files touched in Phase 0-BD: `apps/api/src/aidoo_api/domains/ai/runtime/models.py`, `apps/api/alembic/versions/f0a1b2c3d4e5_add_ai_invocation_seq_check.py`, `apps/api/tests/test_ai_runtime_persistence.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/runtime/models.py tests/test_ai_runtime_persistence.py alembic/versions/f0a1b2c3d4e5_add_ai_invocation_seq_check.py`; `cd apps/api && uv run alembic heads`; `cd apps/api && uv run pytest tests/test_ai_runtime_persistence.py::test_agent_invocation_rejects_negative_invocation_seq tests/test_ai_runtime_persistence.py::test_runtime_migration_downgrade_upgrade_round_trip -q`; `cd apps/api && uv run pytest tests/test_ai_runtime_persistence.py tests/test_alembic_migrations.py -q`; `scripts/phase6-runtime-regression.sh -q`.
