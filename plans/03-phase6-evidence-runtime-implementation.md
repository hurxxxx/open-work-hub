...
- Current PR/stage: Phase 0-W external planner trace wiring after `b9c570b`.
- Last completed: Wired `external_planner_v0` into graph-eligible routing as a trace-only planner candidate. Runtime metadata now carries `external_planner_summary` in done metadata, persisted assistant metadata, runtime shadow metadata, and graph candidate trace payloads. The summary never includes the sanitized prompt body; it records adapter id, status, provider, disabled reason, egress reason, and message count only. Previous Phase 0-V skeleton remains in place: the adapter consumes only `ExternalEgressDecision.sanitized_prompt`, stays disabled when egress denies or the decision is not for `planning`, and emits messages only when egress allows.
- In progress: None.
- Next exact task: Add an external search adapter skeleton that consumes only `ExternalEgressDecision.sanitized_query`, remains hard-disabled unless egress allows `search`, and emits summary-only metadata.
- Files touched in Phase 0-W: `apps/api/src/aidoo_api/domains/ai/agent.py`, `apps/api/src/aidoo_api/domains/ai/approvals.py`, `apps/api/src/aidoo_api/domains/ai/events.py`, `apps/api/src/aidoo_api/domains/ai/router.py`, `apps/api/src/aidoo_api/domains/ai/runtime/persistence.py`, `apps/api/src/aidoo_api/domains/ai/runtime/routing.py`, `apps/api/tests/fixtures/envelope_schema.json`, `apps/api/tests/test_ai_stream.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/router.py src/aidoo_api/domains/ai/agent.py src/aidoo_api/domains/ai/approvals.py src/aidoo_api/domains/ai/events.py src/aidoo_api/domains/ai/runtime/persistence.py src/aidoo_api/domains/ai/runtime/routing.py`; targeted stream/schema tests pending in this session.
- Known blockers: OpenAI/Anthropic-backed manager/search execution is still not enabled. The egress/planner contract only decides, sanitizes, and builds a request envelope; it does not call external APIs. `graph_node_runner_v0` remains an in-process runner, not a durable workflow backend. Hidden node outputs are not persisted verbatim; only status/count/error summary is persisted. High-risk / approval-preview graphs are intentionally not supported by this adapter.
