...
- Current PR/stage: Phase 0-X external search adapter skeleton after `69ea44a`.
- Last completed: Added `external_search_v0` skeleton. It consumes only `ExternalEgressDecision.sanitized_query`, stays disabled when egress denies or the decision is not for `search`, emits a request only when egress allows, and its summary excludes query payload. Tests lock that raw customer/product/order identifiers are not present in external search request metadata. Previous Phase 0-W planner trace remains in place: runtime metadata carries `external_planner_summary` in done metadata, persisted assistant metadata, runtime shadow metadata, and graph candidate trace payloads.
- In progress: None.
- Next exact task: Wire `external_search_v0` into graph-eligible routing metadata as summary-only trace data, still without external API calls.
- Files touched in Phase 0-X: `apps/api/src/aidoo_api/domains/ai/runtime/__init__.py`, `apps/api/src/aidoo_api/domains/ai/runtime/external_search.py`, `apps/api/tests/test_ai_runtime_external_search.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/runtime/external_search.py src/aidoo_api/domains/ai/runtime/__init__.py tests/test_ai_runtime_external_search.py`; `cd apps/api && uv run pytest tests/test_ai_runtime_external_search.py tests/test_ai_runtime_external_egress.py`.
