...
- Current PR/stage: Phase 0-AB external mock search cache boundary after `823f987`.
- Last completed: Added deterministic mock search normalization/cache metadata. Completed mock search execution now emits a stable `cache_key`, `cache_hit=false`, and deterministic `result_refs` derived from the sanitized query digest. `EvidencePacket` provider evidence is now result-ref based, so the same digest yields the same provider evidence refs without storing query text or raw provider output. Trace payload scrubbing preserves `result_refs` as safe metadata.
- In progress: None.
- Next exact task: Add provider invocation audit/metrics fields for mock external planner/search execution so future real adapters can report latency, error class, retry count, and provider-cost placeholders through the same contract.
- Files touched in Phase 0-AB: `apps/api/src/aidoo_api/domains/ai/runtime/external_search.py`, `apps/api/src/aidoo_api/domains/ai/runtime/graph_execution.py`, `apps/api/src/aidoo_api/domains/ai/runtime/persistence.py`, `apps/api/tests/test_ai_runtime_contracts.py`, `apps/api/tests/test_ai_runtime_external_search.py`, `apps/api/tests/test_ai_stream.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/runtime/external_search.py src/aidoo_api/domains/ai/runtime/persistence.py src/aidoo_api/domains/ai/runtime/graph_execution.py tests/test_ai_runtime_external_search.py tests/test_ai_runtime_contracts.py tests/test_ai_stream.py`; `cd apps/api && uv run pytest tests/test_ai_runtime_external_search.py tests/test_ai_runtime_contracts.py::test_graph_evidence_packet_includes_external_mock_search_metadata tests/test_ai_runtime_contracts.py::test_external_mock_search_metadata_does_not_make_packet_ready_alone tests/test_ai_stream.py::test_chat_stream_graph_gate_falls_back_without_graph_execution tests/test_ai_stream.py::test_chat_stream_external_mock_execution_runs_when_enabled tests/test_ai_stream.py::test_chat_stream_graph_execution_uses_external_mock_evidence_when_enabled`; `cd apps/api && uv run pytest tests/test_ai_runtime_external_search.py tests/test_ai_runtime_contracts.py tests/test_ai_stream.py`.
