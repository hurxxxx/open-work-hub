...
- Current PR/stage: Phase 0-U external egress routing trace after `dc4f32b`.
- Last completed: Wired `external_egress.v1` into graph-eligible runtime routing metadata. Graph candidates now carry an auditable `external_egress_summary` with planning/search decisions in done metadata, persisted assistant metadata, runtime shadow metadata, and graph candidate trace payloads. This still does not call OpenAI/Anthropic APIs; it records whether those future external paths would be allowed or denied and with which sanitized payload. Previous Phase 0-T contract remains in place: provider aliases normalize (`claude` -> `anthropic`), Phase 6 external flags and provider allowlists are enforced, PII is blocked, BOM/cost/contract context is blocked, and seed sanitizer cases are locked by tests.
- In progress: None.
- Next exact task: Add an external-planner adapter skeleton that can consume only `ExternalEgressDecision.sanitized_prompt` and is hard-disabled unless both `AIDOO_AI_EXTERNAL_LLM_ENABLED` and `AIDOO_AI_EXTERNAL_PLANNING_ENABLED` are true.
- Files touched in Phase 0-U: `apps/api/src/aidoo_api/domains/ai/agent.py`, `apps/api/src/aidoo_api/domains/ai/approvals.py`, `apps/api/src/aidoo_api/domains/ai/events.py`, `apps/api/src/aidoo_api/domains/ai/router.py`, `apps/api/src/aidoo_api/domains/ai/runtime/persistence.py`, `apps/api/src/aidoo_api/domains/ai/runtime/routing.py`, `apps/api/tests/fixtures/envelope_schema.json`, `apps/api/tests/test_ai_stream.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/router.py src/aidoo_api/domains/ai/agent.py src/aidoo_api/domains/ai/approvals.py src/aidoo_api/domains/ai/events.py src/aidoo_api/domains/ai/runtime/persistence.py src/aidoo_api/domains/ai/runtime/routing.py tests/test_ai_stream.py`; `cd apps/api && uv run pytest tests/test_ai_events.py::test_envelope_schema_snapshot_matches tests/test_ai_runtime_external_egress.py tests/test_ai_stream.py::test_chat_stream_graph_gate_falls_back_without_graph_execution`.
- Known blockers: OpenAI/Anthropic-backed manager/search execution is still not enabled. The egress contract only decides and sanitizes; it does not call external APIs. `graph_node_runner_v0` remains an in-process runner, not a durable workflow backend. Hidden node outputs are not persisted verbatim; only status/count/error summary is persisted. High-risk / approval-preview graphs are intentionally not supported by this adapter.
