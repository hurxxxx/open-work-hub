...
- Current PR/stage: Phase 0-T external egress contract after `60d959c`.
- Last completed: Added the first external-provider egress boundary for future OpenAI/Anthropic planner/search paths. `external_egress.v1` now normalizes provider aliases (`claude` -> `anthropic`), applies Phase 6 external capability flags, checks provider allowlists, blocks PII, blocks sensitive BOM/cost/contract context, and emits sanitized prompt/search payload metadata before any external call is allowed. The sanitizer is locked against the seed `sanitizer_leakage_cases.json` cases. Previous Phase 0-S fallback hardening remains in place: unsupported `graph_node_runner_v0` shapes now emit explicit `graph_execution_fallback_reason` and `graph_execution_fallback_policy` metadata.
- In progress: None.
- Next exact task: Wire `ExternalEgressDecision` into graph/runtime routing metadata so prompt-analysis/search candidates carry an auditable external-allowed or external-denied decision even while actual external calls remain disabled.
- Files touched in Phase 0-T: `apps/api/src/aidoo_api/domains/ai/runtime/__init__.py`, `apps/api/src/aidoo_api/domains/ai/runtime/external_egress.py`, `apps/api/tests/test_ai_runtime_external_egress.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/runtime/external_egress.py src/aidoo_api/domains/ai/runtime/__init__.py tests/test_ai_runtime_external_egress.py`; `cd apps/api && uv run pytest tests/test_ai_runtime_external_egress.py tests/test_ai_runtime_settings.py tests/test_ai_runtime_eval_fixtures.py`.
- Known blockers: OpenAI/Anthropic-backed manager/search execution is still not enabled. The new egress contract only decides and sanitizes; it does not call external APIs. `graph_node_runner_v0` remains an in-process runner, not a durable workflow backend. Hidden node outputs are not persisted verbatim; only status/count/error summary is persisted. High-risk / approval-preview graphs are intentionally not supported by this adapter.
