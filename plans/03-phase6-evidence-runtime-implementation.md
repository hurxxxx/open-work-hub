...
- Current PR/stage: Phase 0-V external planner adapter skeleton after `ed672eb`.
- Last completed: Added `external_planner_v0` skeleton. It consumes only `ExternalEgressDecision.sanitized_prompt`, stays disabled when egress denies or the decision is not for `planning`, emits a prompt only when egress allows, and its summary deliberately excludes prompt payload. Tests lock that raw customer/product/order identifiers are not present in external planner messages. Previous Phase 0-U routing trace remains in place: graph candidates carry `external_egress_summary` with planning/search decisions in done metadata, persisted assistant metadata, runtime shadow metadata, and graph candidate trace payloads.
- In progress: None.
- Next exact task: Wire `external_planner_v0` into manager candidate generation as an optional trace-only candidate path, still without calling external APIs. The deterministic local manager remains canonical unless an explicit future execution flag is added.
- Files touched in Phase 0-V: `apps/api/src/aidoo_api/domains/ai/runtime/__init__.py`, `apps/api/src/aidoo_api/domains/ai/runtime/external_planner.py`, `apps/api/tests/test_ai_runtime_external_planner.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/runtime/external_planner.py src/aidoo_api/domains/ai/runtime/__init__.py tests/test_ai_runtime_external_planner.py`; `cd apps/api && uv run pytest tests/test_ai_runtime_external_planner.py tests/test_ai_runtime_external_egress.py`.
