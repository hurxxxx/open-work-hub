...
- Current PR/stage: Phase 0-Y external search trace wiring after `319e92b`.
- Last completed: Wired `external_search_v0` into graph-eligible routing metadata as summary-only trace data. Runtime metadata now carries `external_search_summary` in done metadata, persisted assistant metadata, runtime shadow metadata, and graph candidate trace payloads. The summary never includes the sanitized query body; it records adapter id, status, provider, disabled reason, egress reason, and whether a query is present. Previous Phase 0-X skeleton remains in place: it consumes only `ExternalEgressDecision.sanitized_query`, stays disabled when egress denies or the decision is not for `search`, and emits a request only when egress allows.
- In progress: None.
- Next exact task: Add opt-in execution hooks for external planner/search adapters, but keep them disabled by default and gated by both egress allow decisions and explicit execution flags.
- Files touched in Phase 0-Y: `apps/api/src/aidoo_api/domains/ai/agent.py`, `apps/api/src/aidoo_api/domains/ai/approvals.py`, `apps/api/src/aidoo_api/domains/ai/events.py`, `apps/api/src/aidoo_api/domains/ai/router.py`, `apps/api/src/aidoo_api/domains/ai/runtime/persistence.py`, `apps/api/src/aidoo_api/domains/ai/runtime/routing.py`, `apps/api/tests/fixtures/envelope_schema.json`, `apps/api/tests/test_ai_stream.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
