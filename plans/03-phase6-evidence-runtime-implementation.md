...
- Current PR/stage: Phase 0-S graph execution fallback policy hardening after `404bb09`.
- Last completed: Added explicit `graph_execution_fallback_reason` values and `graph_execution_fallback_policy` metadata for graph shapes unsupported by `graph_node_runner_v0`: missing candidate summary, unsupported intent, approval-preview requirement, high-risk graph, unsupported output kind, disabled graph execution, and schedule-unavailable cases. The policy is propagated through done metadata, replay/shadow metadata, trace events, persisted assistant metadata, and the envelope schema snapshot. Previous Phase 0-R hardening remains in place: supported grounded-report schedules run hidden domain/search/verifier nodes in schedule order, pass only the structured EvidencePacket JSON plus policy to `writer.template`, stream only writer output to the user, expose `graph_node_execution_summary`, and shadow-write per-node runtime invocation status plus graph node trace events.
- In progress: None.
- Next exact task: Add the external-provider manager/search egress boundary before enabling OpenAI/Anthropic-backed prompt analysis or web/search calls. This should start with explicit sanitizer/egress policy metadata and tests, not direct external calls.
- Files touched in Phase 0-S: `apps/api/src/aidoo_api/domains/ai/agent.py`, `apps/api/src/aidoo_api/domains/ai/approvals.py`, `apps/api/src/aidoo_api/domains/ai/events.py`, `apps/api/src/aidoo_api/domains/ai/router.py`, `apps/api/src/aidoo_api/domains/ai/runtime/__init__.py`, `apps/api/src/aidoo_api/domains/ai/runtime/graph_execution.py`, `apps/api/src/aidoo_api/domains/ai/runtime/persistence.py`, `apps/api/src/aidoo_api/domains/ai/runtime/routing.py`, `apps/api/tests/fixtures/envelope_schema.json`, `apps/api/tests/test_ai_runtime_routing.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/runtime/graph_execution.py src/aidoo_api/domains/ai/runtime/routing.py src/aidoo_api/domains/ai/runtime/__init__.py src/aidoo_api/domains/ai/events.py src/aidoo_api/domains/ai/agent.py src/aidoo_api/domains/ai/router.py src/aidoo_api/domains/ai/approvals.py tests/test_ai_runtime_routing.py`; `cd apps/api && uv run pytest tests/test_ai_events.py::test_envelope_schema_snapshot_matches tests/test_ai_runtime_routing.py`; `cd apps/api && uv run pytest tests/test_ai_stream.py::test_chat_stream_graph_gate_falls_back_without_graph_execution tests/test_ai_stream.py::test_chat_stream_graph_schedule_failure_remains_fallback_metadata tests/test_ai_stream.py::test_chat_stream_graph_execution_adapter_runs_when_enabled`.
- Known blockers: `graph_node_runner_v0` is still an in-process runner, not a durable workflow backend. Hidden node outputs are not persisted verbatim; only status/count/error summary is persisted. High-risk / approval-preview graphs are intentionally not supported by this adapter. OpenAI/Anthropic-backed manager/search execution is still not enabled; it needs an external egress/sanitizer contract first.
