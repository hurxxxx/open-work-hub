...
- Current PR/stage: Phase 0-AC external mock provider audit fields after `eac9d39`.
- Last completed: Added provider invocation audit placeholders to mock external planner/search execution summaries: `latency_ms`, `retry_count`, `error_class`, and `estimated_cost_microunits`. Mock executions report zero latency/retries/cost and no error class, but the stream/runtime metadata contract now has the same fields real provider adapters will need for latency, retry, error, and cost reporting.
- In progress: None.
- Next exact task: Add a local fake adapter test harness that exercises graph + mock external execution end-to-end without real OpenAI/Anthropic keys, then use it as the default E2E guard before any real provider adapter work.
- Files touched in Phase 0-AC: `apps/api/src/aidoo_api/domains/ai/runtime/external_planner.py`, `apps/api/src/aidoo_api/domains/ai/runtime/external_search.py`, `apps/api/tests/test_ai_runtime_external_planner.py`, `apps/api/tests/test_ai_runtime_external_search.py`, `apps/api/tests/test_ai_stream.py`, `plans/03-phase6-evidence-runtime-implementation.md`.
- Tests/checks run: `cd apps/api && uv run ruff check src/aidoo_api/domains/ai/runtime/external_planner.py src/aidoo_api/domains/ai/runtime/external_search.py tests/test_ai_runtime_external_planner.py tests/test_ai_runtime_external_search.py tests/test_ai_stream.py`; `cd apps/api && uv run pytest tests/test_ai_runtime_external_planner.py tests/test_ai_runtime_external_search.py tests/test_ai_stream.py::test_chat_stream_graph_gate_falls_back_without_graph_execution tests/test_ai_stream.py::test_chat_stream_external_mock_execution_runs_when_enabled tests/test_ai_stream.py::test_chat_stream_graph_execution_uses_external_mock_evidence_when_enabled`; `cd apps/api && uv run pytest tests/test_ai_stream.py`.
