...
셋업은 GitLab 파이프라인 성공, 개발 시드 계정 로그인, 개인 앱에서 Codex 콘솔을 새 탭으로 열고 전용 비밀번호로 로그인하는 것까지 확인하면 완료됩니다.
Codex 콘솔은 현재 ChatGPT 구독을 사용하는 별도 웹 작업실입니다. [콘솔 설치 가이드](docs/apps/codex-console/README.md)를 함께 따릅니다.
...
콘솔을 세션 종료·재부팅 후에도 실행되는 별도 systemd 서비스로 등록하고, 개인 앱의 Codex 콘솔을 클릭하면 새 탭에서 열리도록 활성화해줘.
전용 HTTPS 주소를 콘솔에 직접 연결해 개발 Web 재시작에도 사용할 수 있게 해줘. 도메인이 없으면 서버 IP를 SAN에 넣은 인증서와 HTTPS 프록시·PC 신뢰 등록을 준비해줘. 앞단 프록시가 다른 호스트이면 콘솔 설치 문서의 사설망 연결 절차를 따르고, 콘솔의 loopback 수신·HTTPS·CSRF 검증을 유지해줘. 접속 주소와 비밀번호는 .auth_info에 기록해줘.
개인 앱 클릭→새 탭 열기→콘솔 로그인→ChatGPT 구독 연결 상태를 실제 브라우저로 확인해줘. 구독 로그인 자체가 없으면 필요한 사용자 인증만 안내하고 다른 설치 작업은 계속해줘.
...
Codex 콘솔의 파일 첨부도 함께 구성해줘. INSTALL.md 6.1절과 콘솔 소유 문서에 따라 파일 저장·백업·업로드 프록시 제한을 준비하고, 보관한 파일 중 선택한 자료만 메시지에 전달되는지와 전송 후 선택 해제·새로고침 복원을 확인해줘.
개발 앱과 Codex 콘솔은 셋업 세션이 끝난 뒤에도 실행되도록 관리하고, 접속 URL·파이프라인 링크와 커밋·검사 결과·미설정 기능·중지·재시작 방법을 알려줘. 운영 배포는 하지 마.
```
