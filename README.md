...
5. 검증용 MR의 최신 커밋에서 파이프라인이 성공했는지 확인합니다. lint 통과나 Runner 등록만으로 완료하지 않습니다.
6. 안내받은 `http://<서버-IP>:<개발-Web-포트>/login`에 SSH 터널 없이 접속하고, `.auth_info`의 개발 시드 계정으로 로그인해 앱 화면이 열리는지 확인합니다.
