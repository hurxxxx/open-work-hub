...
- 2026-05-01 환경 프로파일 분리: `AIDOO_ENV_PROFILE` 하나로 local/vm/prod를 구분한다. 기본값은 `local`이며 기존 로컬 dev/test 동작을 유지한다. `vm`일 때만 Docker 직접 접근 실패 시 `sudo -n docker` fallback을 쓰고, API Docker fixture가 host network 랜덤 포트를 사용한다. 운영은 아직 없으므로 `prod`는 dev/test 명령에서 거부한다. 신규 VM 명령: `AIDOO_ENV_PROFILE=vm ./dev.sh`, `pnpm test:api:vm`, `pnpm test:api:vm:collect`. `agent-browser` smoke는 기본 로컬 smoke를 유지하며 VM에서는 `AIDOO_ENV_PROFILE=vm pnpm agent-browser:smoke`로 실행한다.
...
> 업데이트: GitHub Actions와 live API Playwright smoke는 2026-05-01 테스트 경량화 작업에서 제거했다. 환경 구분은 `AIDOO_ENV_PROFILE=local|vm|prod` 하나로 한다. 기본은 `local`, VM 전용 Docker fallback/host network는 `vm`에서만 켜진다. 다음 작업은 로컬 검증 기준(`lint`, `typecheck`, focused unit/API tests, `pnpm nx e2e-shell web`)으로 진행한다.
