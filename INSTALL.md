...
- [실행 설정과 재개](docs/apps/codex-console/README.md#실행-설정과-중단-후-계속하기)에 따라
  구독 모델 목록·계획/구현 모드·승인/YOLO 선택과 탭 종료 후 백그라운드 작업 복원·중단 재개를 확인한다.
...
콘솔의 추가 동작 확인: 새 작업은 일반 대화로 시작하며 질문만으로 요구사항·계획을 갱신하지
않아야 한다. 작업 상단의 실제 브랜치/분리 HEAD·변경 상태와 **병합 요청** 대상·범위 선택을
[콘솔 동작 가이드](docs/apps/codex-console/README.md#브랜치-상태와-병합-요청)에 따라 확인한다.
