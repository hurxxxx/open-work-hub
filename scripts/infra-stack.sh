...
require_matching_checkout() {
  ...
  case "$environment" in
    prod)
      if [[ "$checkout_name" != "prod" && "${OPEN_WORK_HUB_ALLOW_NON_PROD_CHECKOUT_PROD_COMMANDS:-0}" != "1" ]]; then
        echo "Refusing to manage production infra from non-prod checkout: $ROOT_DIR" >&2
        exit 1
      fi
      ;;
    dev)
      if [[ "$checkout_name" == "prod" && "${OPEN_WORK_HUB_ALLOW_PROD_CHECKOUT_DEV_COMMANDS:-0}" != "1" ]]; then
...
case "$ENVIRONMENT" in
  prod)
    ...
    COMPOSE_FILE="$ROOT_DIR/ops/compose/open-work-hub-prod.infra.yml"
    ;;
  dev)
    require_matching_checkout dev
    COMPOSE_FILE="$ROOT_DIR/ops/compose/open-work-hub-dev.infra.yml"
    ;;
  *)
    usage
    exit 2
