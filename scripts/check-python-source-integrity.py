...
DEFAULT_SOURCE_ROOTS = (
    Path("apps/api/src/open_work_hub_api"),
    Path("apps/worker/src/open_work_hub_worker"),
)
...
def _direct_core_llm_imports(tree: ast.Module, path: Path) -> list[Finding]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            imports_core_llm_module = module == "open_work_hub_api.core.llm" or (
                node.level > 0 and module == "core.llm"
            )
            imports_core_package = module == "open_work_hub_api.core" or (
            ...
            for imported in node.names:
                if imported.name == "open_work_hub_api.core.llm":
                    findings.append(
                        Finding(
                            path=path,
                            line=node.lineno,
