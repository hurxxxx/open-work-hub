...
if [[ "${BASH_SOURCE[0]}" == "$0" ]] &&
   [[ -n "${OPEN_ALM_VALIDATION_REPO_ROOT:-}" ]]; then
  validation_image_repo_root="$OPEN_ALM_VALIDATION_REPO_ROOT"
fi

OPEN_ALM_VALIDATION_IMAGE="open-alm-validation:node25-python312-pg18-api-a3e8c22a3552-worker-3149583cef20-node-e7c57b3bacf9-484482bced42"
OPEN_ALM_VALIDATION_IMAGE_ID="sha256:89345c7907b296ea557a61e9a4c956ee9915c3fb9b611b3923f36bcc4c65d526"
OPEN_ALM_VALIDATION_DOCKERFILE_SHA256="484482bced42fc34de9efc211022aa4bc39c5cf533bf4d29e435f378b9ce4a14"
OPEN_ALM_API_DEPENDENCY_SHA256="a3e8c22a3552c93ab0108e3a8085a7a6581b21b06617ff03b66af07292a058b3"
OPEN_ALM_NODE_DEPENDENCY_SHA256="e7c57b3bacf997bd39ddb5532225ed427231a4dcc03fe189e75390edc7b0bef1"
OPEN_ALM_WORKER_DEPENDENCY_SHA256="3149583cef207cba735026353ddfa068de461899b45bead09ecd7cbd926d87ef"
...
verify_validation_image_source_contract() {
  ...
  if [[ "$actual_dockerfile_sha256" != "$OPEN_ALM_VALIDATION_DOCKERFILE_SHA256" ]]; then
  ...
  if [[ "$actual_api_dependency_sha256" != "$OPEN_ALM_API_DEPENDENCY_SHA256" ]]; then
    echo \
  ...
  if [[ "$actual_node_dependency_sha256" != "$OPEN_ALM_NODE_DEPENDENCY_SHA256" ]]; then
    echo \
  ...
  if [[ "$actual_worker_dependency_sha256" != "$OPEN_ALM_WORKER_DEPENDENCY_SHA256" ]]; then
    echo \
  ...
  if [[ ! "$OPEN_ALM_VALIDATION_IMAGE_ID" =~ ^sha256:[0-9a-f]{64}$ ]]; then
    echo \
...
print_validation_image_contract() {
  printf 'image=%s\n' "$OPEN_ALM_VALIDATION_IMAGE"
  printf 'image_id=%s\n' "$OPEN_ALM_VALIDATION_IMAGE_ID"
  printf 'dockerfile_sha256=%s\n' "$OPEN_ALM_VALIDATION_DOCKERFILE_SHA256"
  printf 'api_dependency_sha256=%s\n' "$OPEN_ALM_API_DEPENDENCY_SHA256"
  printf 'node_dependency_sha256=%s\n' "$OPEN_ALM_NODE_DEPENDENCY_SHA256"
  printf 'worker_dependency_sha256=%s\n' "$OPEN_ALM_WORKER_DEPENDENCY_SHA256"
}
...
build_and_verify_validation_image() {
  ...
  docker_config_dir="${OPEN_ALM_VALIDATION_DOCKER_CONFIG:-$validation_image_repo_root/.runtime/ci-validation-docker-config}"
  mkdir -p "$docker_config_dir"

  docker --config "$docker_config_dir" build \
    --pull=false \
    --provenance=false \
    --build-arg "API_DEPENDENCY_SHA256=${OPEN_ALM_API_DEPENDENCY_SHA256}" \
    --build-arg "NODE_DEPENDENCY_SHA256=${OPEN_ALM_NODE_DEPENDENCY_SHA256}" \
    --build-arg "WORKER_DEPENDENCY_SHA256=${OPEN_ALM_WORKER_DEPENDENCY_SHA256}" \
    --label \
      "ai.do.validation.dockerfile-sha256=${OPEN_ALM_VALIDATION_DOCKERFILE_SHA256}" \
    --label \
      "ai.do.validation.api-dependency-sha256=${OPEN_ALM_API_DEPENDENCY_SHA256}" \
    --label \
      "ai.do.validation.node-dependency-sha256=${OPEN_ALM_NODE_DEPENDENCY_SHA256}" \
    --label \
      "ai.do.validation.worker-dependency-sha256=${OPEN_ALM_WORKER_DEPENDENCY_SHA256}" \
    --file \
      "$validation_image_repo_root/ops/ci/validation-runner/Dockerfile" \
    --tag "$OPEN_ALM_VALIDATION_IMAGE" \
    "$validation_image_repo_root"

  built_image_id="$(
    docker --config "$docker_config_dir" image inspect \
      --format '{{.Id}}' "$OPEN_ALM_VALIDATION_IMAGE"
  )"
  ...
  if [[ "$built_image_id" != "$OPEN_ALM_VALIDATION_IMAGE_ID" ]] ||
     [[ "$built_dockerfile_sha256" != "$OPEN_ALM_VALIDATION_DOCKERFILE_SHA256" ]] ||
     [[ "$built_api_dependency_sha256" != "$OPEN_ALM_API_DEPENDENCY_SHA256" ]] ||
     [[ "$built_node_dependency_sha256" != "$OPEN_ALM_NODE_DEPENDENCY_SHA256" ]] ||
     [[ "$built_worker_dependency_sha256" != "$OPEN_ALM_WORKER_DEPENDENCY_SHA256" ]]; then
    echo \
  ...
  docker --config "$docker_config_dir" run --rm --pull=never \
    ...
    bash -lc '
      ...
      python --version
      uv --version
      test "$(<"$OPEN_ALM_API_IMAGE_DEPENDENCY_FILE")" = "'"$OPEN_ALM_API_DEPENDENCY_SHA256"'"
      test "$(<"$OPEN_ALM_API_IMAGE_NODE_DEPENDENCY_FILE")" = "'"$OPEN_ALM_NODE_DEPENDENCY_SHA256"'"
      test "$(<"$OPEN_ALM_WORKER_IMAGE_DEPENDENCY_FILE")" = "'"$OPEN_ALM_WORKER_DEPENDENCY_SHA256"'"
      UV_OFFLINE=1 "$OPEN_ALM_API_IMAGE_VENV/bin/python" -I -c \
        "import pytest, xdist; assert pytest.version_tuple[0] == 8"
      UV_OFFLINE=1 "$OPEN_ALM_WORKER_IMAGE_VENV/bin/python" -I -c \
        "import pytest; assert pytest.version_tuple[0] == 8"
      (
        cd /opt/open-alm/node-runtime
      ...
      ln -s "$OPEN_ALM_API_IMAGE_NODE_MODULES" \
  ...
  docker --config "$docker_config_dir" run --rm --pull=never \
    ...
    "$OPEN_ALM_VALIDATION_IMAGE" \
    bash -lc '
      UV_OFFLINE=1 "$OPEN_ALM_API_IMAGE_VENV/bin/python" \
        scripts/check-api-test-budget.py --output /tmp/api-test-report.json
    '

  echo \
    "Built and verified ${OPEN_ALM_VALIDATION_IMAGE} (${OPEN_ALM_VALIDATION_IMAGE_ID})."
}
