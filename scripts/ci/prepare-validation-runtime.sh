...
link_runtime() {
  ...
  if [[ -L "$link_path" ]]; then
    if [[ "$(readlink "$link_path")" != "$target" ]]; then
      echo "Validation ${label} link points to an unexpected target: ${link_path}" >&2
      exit 2
    fi
    return
  fi
  if [[ -e "$link_path" ]]; then
    echo "Validation ${label} link path already exists and is not the image runtime: ${link_path}" >&2
    exit 2
  fi
