...
dependency_mode="${AI_DO_API_DEPENDENCY_MODE:-offline}"
dependency_marker_name=".ai-do-api-dependency-sha256"

case "$dependency_mode" in
  offline)
    api_venv="${AI_DO_API_IMAGE_VENV:-/opt/ai-do/venvs/api}"
    image_dependency_file="${AI_DO_API_IMAGE_DEPENDENCY_FILE:-/opt/ai-do/locks/api/dependency.sha256}"
    ...
    ;;
  bootstrap)
    api_venv="${AI_DO_API_BOOTSTRAP_VENV:-}"
    if [[ -z "$api_venv" || "$api_venv" != /* || "$api_venv" == "/" ]]; then
      echo \
        "AI_DO_API_BOOTSTRAP_VENV must be an explicit absolute path in bootstrap mode." \
        >&2
      exit 2
    fi
    if [[ "$api_venv" == "${AI_DO_API_IMAGE_VENV:-/opt/ai-do/venvs/api}" ]]; then
    ...
    if [[ -r "$bootstrap_dependency_file" ]] &&
      ...
      if [[ -e "$api_venv" ]]; then
        echo \
          ...
          >&2
        echo "Use a clean digest-scoped AI_DO_API_BOOTSTRAP_VENV path." >&2
        exit 2
      fi
    ...
    ;;
  *)
    echo "AI_DO_API_DEPENDENCY_MODE must be offline or bootstrap." >&2
    exit 2
    ;;
esac
...
image_node_dependency_file="${AI_DO_API_IMAGE_NODE_DEPENDENCY_FILE:-/opt/ai-do/node-runtime/dependency.sha256}"
image_node_modules="${AI_DO_API_IMAGE_NODE_MODULES:-/opt/ai-do/node-runtime/node_modules}"
checkout_node_modules="${AI_DO_API_CHECKOUT_NODE_MODULES_LINK:-$repo_root/node_modules}"
...
if [[ "$checkout_node_modules" != /* || "$checkout_node_modules" == "/" ]]; then
  echo \
    "AI_DO_API_CHECKOUT_NODE_MODULES_LINK must be an explicit absolute path." \
    >&2
  exit 2
fi
...
if [[ "${AI_DO_API_CLEANUP_ONLY:-0}" == "1" ]]; then
  if [[ "$lane" != "external" || "$#" -ne 0 ]]; then
    echo "AI_DO_API_CLEANUP_ONLY=1 requires the external lane without test paths." >&2
    exit 2
  fi
  cd "$api_root"
...
if [[ "${AI_DO_API_PREPARE_ONLY:-0}" == "1" ]]; then
  "$api_python" -I -c \
...
if [[ "$lane" == "standard" ]]; then
  pytest_workers="${AI_DO_API_PYTEST_WORKERS:-8}"
  pytest_scheduler="${AI_DO_API_PYTEST_SCHEDULER:-worksteal}"
  if [[ ! "$pytest_workers" =~ ^[1-9][0-9]*$ ]]; then
    echo "AI_DO_API_PYTEST_WORKERS must be a positive integer." >&2
    exit 2
  fi
  case "$pytest_scheduler" in
    load)
      pytest_maxschedchunk="${AI_DO_API_PYTEST_MAXSCHEDCHUNK:-1}"
      if [[ ! "$pytest_maxschedchunk" =~ ^[1-9][0-9]*$ ]]; then
        echo "AI_DO_API_PYTEST_MAXSCHEDCHUNK must be a positive integer." >&2
        exit 2
      fi
      pytest_args+=(
        -n "$pytest_workers"
    ...
    worksteal)
      ...
      ;;
    *)
      echo "AI_DO_API_PYTEST_SCHEDULER must be load or worksteal." >&2
      exit 2
      ;;
  esac
...
if [[ "$lane" != "external" ]]; then
  cd "$api_root"
  if [[ "${AI_DO_API_ALLOW_EMPTY:-0}" != "1" ]]; then
...
if [[ -z "${AI_DO_API_TEST_RUN_ID:-}" ]]; then
  if [[ -n "${CI_JOB_ID:-}" ]]; then
    ...
    AI_DO_API_TEST_RUN_ID="$("$api_python" -I -c \
...
cleanup_external_resources() {
  ...
  if ! (
    ...
    echo \
      "warning: unable to clean shared API integration resources for run ${AI_DO_API_TEST_RUN_ID}" \
      >&2
    cleanup_status=1
  fi
  return "$cleanup_status"
...
pytest_pid=""
if [[ "$pytest_status" -eq 5 && "${AI_DO_API_ALLOW_EMPTY:-0}" == "1" ]]; then
