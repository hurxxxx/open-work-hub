...
if [[ -z "${OPEN_WORK_HUB_API_TEST_RUN_ID:-}" ]]; then
  if [[ -n "${CI_JOB_ID:-}" ]]; then
    OPEN_WORK_HUB_API_TEST_RUN_ID="$CI_JOB_ID"
  elif [[ -r /proc/sys/kernel/random/uuid ]]; then
    read -r OPEN_WORK_HUB_API_TEST_RUN_ID </proc/sys/kernel/random/uuid
  else
    OPEN_WORK_HUB_API_TEST_RUN_ID="$(python3 -c 'import uuid; print(uuid.uuid4())')"
  fi
fi
export OPEN_WORK_HUB_API_TEST_RUN_ID
...
cleanup_test_resources() {
  ...
  if ! (
    ...
    echo "warning: unable to clean shared API integration resources for run ${OPEN_WORK_HUB_API_TEST_RUN_ID}" >&2
    CLEANUP_STATUS=1
  fi
  return "$CLEANUP_STATUS"
}
...
echo "API test run id: ${OPEN_WORK_HUB_API_TEST_RUN_ID}" >&2
cd "$ROOT_DIR/apps/api"
