...
- Commit, push, MR mutation, merge, deploy, and unrelated or force cleanup require explicit current authorization. A user-visible Codex Console implementation request authorizes its required commit and GitLab `dev` integration, separate console release replacement, service restart, and public browser verification unless the user limits it to local changes or validation; do not stop after source integration or ask again for those steps. This standing scope does not authorize a `dev -> main` promotion or unrelated cleanup. An authorized MR delivery includes removing only its clean local worktree and verified-merged local branch.
...
- Codex Console uses a separate release and service: source sync or OWH deployment alone does not update it. For user-visible implementation work, follow [the console deployment checks](docs/apps/codex-console/README.md#배포-완료-확인), replace its release, and verify the running service and browser before reporting completion.
