...
export function Panel({
  ...
  return (
    <section
      className={cn(
        'rounded-[var(--ui-radius-lg)] border border-[var(--ui-color-border)] bg-[var(--ui-color-surface)] p-5 shadow-[var(--ui-shadow-sm)]',
        className,
      )}
      {...props}
    >
      {hasHeader ? (
        <header className="mb-4 flex items-start justify-between gap-3">
          <div className="grid gap-1.5">
            ...
            {title ? (
              <h2 className="m-0 text-[1.25rem] font-semibold tracking-[-0.02em] text-[var(--ui-color-ink)]">
                {title}
              </h2>
            ) : null}
            {description ? (
